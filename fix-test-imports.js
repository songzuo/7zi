...
function getLintWarningsForFile(filePath) {
  try {
    ...
    for (const line of lines) {
      ...
      const match = line.match(/(\d+):(\d+)\s+\w+\s+['"]([^'"]+)['"]\s+is\s+defined\s+but\s+never\s+used/);
      if (match) {
        warnings.push({
...
function fixTestFile(filePath) {
  try {
    ...
    const unusedPatterns = [
      ...
      { pattern: /import\s*\{\s*userEvent\s*\}\s*from\s*['"]@testing-library\/user-event['"];?\s*\n/g, replacement: '' },
...
function main() {
  ...
  for (const dir of testDirs) {
    ...
    const files = execSync(`find "${dir}" -name "*.test.ts" -o -name "*.test.tsx" -o -name "*.spec.ts" -o -name "*.spec.tsx"`, {
      encoding: 'utf8',
      timeout: 30000
    }).split('\n').filter(f => f);
