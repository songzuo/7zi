...
def update_translation_file(locale):
    """更新翻译文件"""
    file_path = f'/root/.openclaw/workspace/src/i18n/messages/{locale}.json'
    ...
    # 合并翻译
    merged = deep_merge(existing, translations[locale])

    # 写回文件
    with open(file_path, 'w', encoding='utf-8') as f:
        json.dump(merged, f, ensure_ascii=False, indent=2)

    print(f'✅ {locale}.json 更新完成')

    # 统计翻译数量
    count = 0
    for ns in translations[locale]:
        count += len(str(translations[locale][ns]))
    return count
...
if __name__ == '__main__':
    ...
    for locale in locales:
        count = update_translation_file(locale)
        total_count += count
        print(f'   已添加约 {count} 字符的翻译\n')

    print('✅ 所有翻译文件更新完成！')
    print(f'\n总计添加约 {total_count} 字符的翻译')
    print('\n已翻译的命名空间:', ', '.join(translations['ja'].keys()))
