...
function checkFile(filePath, maxSize, type) {
  try {
    ...
    if (stats.size > maxSize) {
      console.error(
        `${colors.red}❌ ${type}${colors.reset} ${relativePath}: ${formatSize(stats.size)} ${colors.red}exceeds${colors.reset} ${formatSize(maxSize)}`
      )
      return false
    }

    console.log(
      `${colors.green}✅${colors.reset} ${type} ${relativePath}: ${formatSize(stats.size)}`
    )
    return true
  } catch (error) {
...
function walkDirectory(dir, callback) {
  try {
    ...
    console.warn(
      `${colors.yellow}⚠️${colors.reset}  Could not read directory ${dir}: ${error.message}`
    )
  }
}
...
function main() {
  ...
  if (!hasChunks) {
    ...
    console.warn(
      `${colors.yellow}⚠️${colors.reset}  Make sure you have run the build first: \`npm run build\`\n`
    )
    process.exit(0)
  }
  ...
  if (allPassed) {
    ...
    console.error(
      `${colors.yellow}💡 Tip: Use \`npm run build:analyze\` to analyze and optimize your bundles.${colors.reset}`
    )
    process.exit(1)
  }
}
