...
function checkCriticalCSS() {
  ...
  if (result.exists) {
    ...
    if (isSmall) {
      ...
      log(
        `⚠️  Critical CSS 可能过大 (${(result.size / 1024).toFixed(2)} KB)，建议优化`,
        colors.yellow
      )
    }
  }
...
function checkOptimizedPage() {
  ...
  const result = checkFile('src/app/[locale]/page.optimized.example.tsx', '优化示例页面')
...
function generateReport() {
  ...
  const completionRate = (
    (implemented.length / (implemented.length + missing.length)) *
    100
  ).toFixed(1)

  log('\n📈 完成度:', colors.cyan)
  log(
    `   ${completionRate}% (${implemented.length}/${implemented.length + missing.length})`,
    completionRate >= 80 ? colors.green : colors.yellow
  )
