...
async function runTests() {
  ...
  logTest(
    ...
    connectedUsers === TEST_USERS.length,
      ...
      await new Promise((resolve, reject) => {
        ...
        client.once('room:user_list', data => {
          ...
          if (data.users.length === TEST_USERS.length) {
            logTest(`User List (${user.name})`, true, `Received ${data.users.length} users in room`)
          } else {
            logTest(
              `User List (${user.name})`,
              false,
  ...
  try {
    ...
    logTest(
      ...
      operationsReceived === receiverClientsWrappers.length,
  ...
  for (const { client, user } of clients) {
    ...
    client.on('cursor:update', data => {
      if (data.userId !== user.id) {
        ...
        console.log(
          `   👁️ ${user.name} sees ${data.userName}'s cursor at position ${data.position}`
        )
      }
    })
  }
  ...
  logTest(
    'Cursor Tracking',
    totalCursorUpdates === expectedCursorUpdates,
  ...
  logTest(
    'Typing Indicators',
    totalTypingUpdates === expectedTypingUpdates,
      ...
      await new Promise((resolve, reject) => {
        ...
        client.once('doc:sync', data => {
          ...
          if (data.roomId === TEST_ROOM_ID && data.document) {
            syncSuccessful++
            console.log(
              `   📄 ${client.user.name} synced document (revision: ${data.document.revision})`
            )
          }
