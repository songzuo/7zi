...
export class TeamPage {
  ...
  constructor(page: Page) {
    ...
    this.addMemberButton = page.locator('button:has-text("添加成员"), button:has-text("Add Member"), button:has-text("新建")');

    // Modal locators
    this.modal = page.locator('[role="dialog"], .modal, .dialog');
    this.nameInput = this.modal.locator('input[name="name"], input[placeholder*="名称"], input[placeholder*="Name"]');
    this.roleInput = this.modal.locator('input[name="role"], input[placeholder*="角色"], input[placeholder*="Role"]');
    this.providerSelect = this.modal.locator('select[name="provider"], [role="combobox"]');
    this.saveButton = this.modal.locator('button[type="submit"], button:has-text("保存"), button:has-text("Save")');
  }
  ...
  async deleteMember(index: number): Promise<void> {
    const member = this.memberCards.nth(index);
    const deleteButton = member.locator('.delete-button, button:has-text("删除"), button:has-text("Delete")');
  ...
  async editMember(index: number): Promise<void> {
    const member = this.memberCards.nth(index);
    const editButton = member.locator('.edit-button, button:has-text("编辑"), button:has-text("Edit")');
