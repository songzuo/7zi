...
export class RegistrationPage {
  ...
  constructor(page: Page) {
    ...
    // Initialize locators
    this.nameInput = page.locator('input[name="name"], input[placeholder*="姓名"], input[placeholder*="Name"]');
    ...
    this.submitButton = page.locator('button[type="submit"], button:has-text("注册"), button:has-text("Register")');
  ...
  async waitForError(): Promise<void> {
    await expect(this.page.locator('.error, .toast.error, .alert.error')).toBeVisible({ timeout: 5000 });
  }
}
