...
export class NotificationsPage {
  ...
  constructor(page: Page) {
    ...
    // Initialize locators
    this.notificationBell = page.locator('.notification-bell, [aria-label*="通知"], button:has-text("通知")');
    ...
    this.markAllReadButton = page.locator('button:has-text("全部已读"), button:has-text("Mark all as read")');
  ...
  async toggleEmailNotifications(): Promise<void> {
    ...
    const toggle = this.page.locator('input[type="checkbox"][name="email-notifications"], .toggle.email-notifications');
  ...
  async togglePushNotifications(): Promise<void> {
    await this.goToNotificationsSettings();
    const toggle = this.page.locator('input[type="checkbox"][name="push-notifications"], .toggle.push-notifications');
    if (await toggle.isVisible()) {
      await toggle.click();
    }
  }
  ...
  async configureNotificationType(type: string, enabled: boolean): Promise<void> {
    await this.goToNotificationsSettings();
    const checkbox = this.page.locator(`input[type="checkbox"][value="${type}"], .notification-type[data-type="${type}"]`);
