...
test.describe('User Management', () => {
  ...
  test('should search users by name', async ({ page }) => {
    ...
    // Find search input
    const searchInput = page.locator('input[placeholder*="搜索"], input[placeholder*="Search"], input[name="search"]');
  ...
  test('should open create user dialog', async ({ page }) => {
    ...
    const createButton = page.locator('button:has-text("新建用户"), button:has-text("Create User"), button:has-text("添加用户")');
  ...
  test('should create new user', async ({ page }) => {
    ...
    // Submit form
    await page.click('button:has-text("保存"), button:has-text("Save"), button:has-text("创建"), button:has-text("Create")');
  ...
  test('should edit existing user', async ({ page }) => {
    ...
    const editButton = page.locator('button:has-text("编辑"), button:has-text("Edit"), .edit-button').first();
  ...
  test('should delete user', async ({ page }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      // Confirm deletion
      const confirmButton = page.locator('button:has-text("确认"), button:has-text("Confirm"), button:has-text("确定")');
  ...
  test('should deactivate user', async ({ page }) => {
    ...
    const deactivateButton = page.locator('button:has-text("停用"), button:has-text("Deactivate"), .toggle-user').first();
  ...
  test('should handle pagination', async ({ page }) => {
    ...
    if (await pagination.isVisible()) {
      ...
      const prevButton = pagination.locator('button:has-text("上一页"), button:has-text("Previous")');
...
test.describe('User Management Permissions', () => {
  test('should restrict non-admin users from accessing user management', async ({ page }) => {
    ...
    const url = page.url();
    const isAccessDenied = await page.locator('text=访问被拒绝, Access Denied, 无权限').count() > 0;
