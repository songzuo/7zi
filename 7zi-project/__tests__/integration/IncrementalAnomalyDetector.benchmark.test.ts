...
describe('Incremental Anomaly Detection Performance Benchmarks', () => {
  ...
  describe('Basic Performance Tests', () => {
    ...
    test('StreamingAnomalyDetector should meet performance targets', () => {
      ...
      console.log(`StreamingAnomalyDetector: avg=${avgTime.toFixed(4)}ms, p95=${p95Time.toFixed(4)}ms`);
  ...
  describe('Large Scale Data Tests', () => {
    ...
    test('should handle streaming data with varying patterns', () => {
      ...
      for (const pattern of patterns) {
        ...
        const stats = detector.getStats();
        console.log(`${pattern.name}: ${duration.toFixed(2)}ms, anomaly rate: ${(stats.anomalyRate * 100).toFixed(2)}%`);
  ...
  describe('Configuration Optimization', () => {
    ...
    test('should balance accuracy and performance with different weights', () => {
      ...
      for (const config of configs) {
        ...
        console.log(`${config.name}: ${duration.toFixed(2)}ms, anomaly rate: ${(stats.anomalyRate * 100).toFixed(2)}%`);
...
describe('Benchmark Suite', () => {
  test('Run full benchmark suite', () => {
    ...
    const zscoreResult = benchmark('IncrementalZScore.update()', () => {
    ...
    const forestResult = benchmark('StreamingIsolationForest.addPoint()', () => {
      forest.addPoint(Math.random() * 100);
    }, 1000);

    // Full detector
    const detector = new StreamingAnomalyDetector();
    const detectorResult = benchmark('StreamingAnomalyDetector.detect()', () => {
