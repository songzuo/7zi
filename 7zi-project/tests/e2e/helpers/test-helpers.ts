...
export async function selectOptionByText(dropdown: Locator, optionText: string): Promise<void> {
  ...
  const option = dropdown.page().locator(`option:has-text("${optionText}"), [role="option"]:has-text("${optionText}")`);
  await option.click();
}
 ...
 * Type with delay
 */
export async function typeWithDelay(locator: Locator, text: string, delay: number = 100): Promise<void> {
 ...
 */
export async function getAttributes(locator: Locator, attributeNames: string[]): Promise<Record<string, string | null>> {
 ...
 */
export async function waitForElementCount(locator: Locator, count: number, timeout: number = 10000): Promise<void> {
