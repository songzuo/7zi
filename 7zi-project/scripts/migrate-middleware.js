...
function migrateRouteFile(filePath, dryRun = false) {
  ...
  if (!migrated.includes('@/lib/middleware/with-request-id')) {
    ...
    if (importMatch) {
      ...
      migrated = migrated.slice(0, lastImportEnd) + IMPORT_STATEMENT + '\n' + migrated.slice(lastImportEnd);
  ...
  migrated = migrated.replace(functionRegex, (match, isAsync, funcName) => {
    ...
    if (match.includes('export const') || match.includes('export let') || match.includes('export var')) {
      return match;
    }
  ...
  migrated = migrated.replace(
    ...
    (match, funcName, isAsync, params) => {
      ...
      if (match.includes('withRequestId') || match.includes('withAuth') || match.includes('withCors')) {
        return match;
      }
  ...
  if (migrated.includes('request.headers.get(\'x-request-id\')') || migrated.includes('request.headers.get("x-request-id")')) {
