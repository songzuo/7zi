...
function measurePerformance(name: string, fn: () => void, iterations: number = 1): BenchmarkResult {
  // 强制垃圾回收（如果可用）
  if (global.gc) {
    global.gc()
  }
...
function runBenchmark(testData: TestData, iterations: number = 1000): void {
  console.log('\n' + '='.repeat(80))
  ...
  console.log(
    `   IncrementalZScore detected: ${((incAnomalies / testData.data.length) * 100).toFixed(2)}%`
  )
  console.log(
    `   BatchZScore detected: ${((batchAnomalies / testData.data.length) * 100).toFixed(2)}%`
  )
  console.log(
    `   StreamingDetector detected: ${((streamAnomalies / testData.data.length) * 100).toFixed(2)}%`
  )
}
