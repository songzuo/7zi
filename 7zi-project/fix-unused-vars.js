  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        const varMatch = message.match(/'([^']+)'\s+is\s+(defined\s+but\s+never\s+used|assigned\s+a\s+value\s+but\s+never\s+used)/);
        if (varMatch) {
          warnings.push({
    ...
    for (const warning of sortedWarnings) {
      ...
      if (importMatch) {
          ...
          if (importMatch[2] === warning.varName) {
            ...
            if (bracesMatch) {
              ...
              if (newImports.length === 0) {
                ...
                const newImportLine = line.replace(bracesMatch[0], `import { ${newImports.join(', ')} } from`);
                lines[lineIdx] = newImportLine;
              }
              modified = true;
              console.log(`  Fixed: Removed unused import '${varName}' from ${path.relative(PROJECT_DIR, filePath)}`);
            }
          }
        }
      } else {
        ...
        if (varDeclMatch && varDeclMatch[1] === warning.varName) {
          ...
          lines[lineIdx] = line.replace(new RegExp(`\\b${varDeclMatch[1]}\\b`), `_${varDeclMatch[1]}`);
          modified = true;
          console.log(`  Fixed: Renamed unused variable '${varDeclMatch[1]}' to '_${varDeclMatch[1]}' in ${path.relative(PROJECT_DIR, filePath)}`);
        }
      }
    }
