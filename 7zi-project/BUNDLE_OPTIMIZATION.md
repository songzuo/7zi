...
**问题**: 项目同时使用两个图表库 (Recharts 和 Chart.js)，造成 6.3MB 冗余。

**发现**:
- `recharts`: 8.7MB node_modules，用于 AnalyticsChart.tsx
- `chart.js + react-chartjs-2`: 6.3MB node_modules，仅用于测试
- `AnalyticsChartChartJS.tsx` (557行) 仅在测试中使用
- `RevenueChart.tsx` (298行) 和 `ActivityChart.tsx` (318行) 未被使用

**解决方案**:
- ✅ 移除 `AnalyticsChartChartJS.tsx` (仅测试用)
- ✅ 移除 `RevenueChart.tsx` (未使用)
- ✅ 移除 `ActivityChart.tsx` (未使用)
- ✅ 从 `src/components/analytics/index.ts` 移除相关导出
- ✅ 从 `src/components/analytics/__tests__/analytics.test.tsx` 移除测试
- ✅ 从 `package.json` 移除 `chart.js` 和 `react-chartjs-2` 依赖

**预期效果**:
- 减少 node_modules: **-6.3MB** (chart.js + react-chartjs-2)
- 减少 bundle: **-200-400KB** (tree-shaking 后)
- 减少 1,173 行未使用代码
...
**修改的文件**:
1. `package.json` - 移除依赖
2. `src/components/analytics/index.ts` - 移除导出
3. `src/components/analytics/__tests__/analytics.test.tsx` - 移除测试套件

---

#### 7. Socket.io-client 优化 (已配置)

**问题**: Socket.io-client (1.6MB node_modules) 被打包到主 bundle 中，但仅在协作功能页面使用。

**使用位置**:
- `src/lib/websocket/useCollaboration.ts` - 实时协作功能
- `src/lib/realtime/useEnhancedWebSocket.ts` - 增强 WebSocket
- `src/hooks/useWebSocket.ts` - WebSocket Hook
- `src/hooks/useWebRTCMeeting.ts` - WebRTC 会议功能

**解决方案**:
- ✅ 已在 `next.config.ts` 中添加 `socket.io-client` 到 `optimizePackageImports`
- ✅ 已添加 `socket-client` 独立 chunk group (priority: 43)

**预期效果**:
- Socket.io-client 从主 bundle 分离
- 初始加载减少 ~100-150KB
- 更好的缓存策略
...
**注意**: 虽然已优化包导入和 chunk 分割，但 WebSocket 功能仍会在应用启动时初始化。如需进一步优化，可以考虑将整个协作功能模块动态导入。
...
最大 chunks:
```
0u.xmd1ju4nu3.js: 999K  ← Three.js (需要进一步分离)
...
**发现**:
- Three.js chunk (999K) 仍然过大，可能包含未使用的模块
- 有多个相似大小的 chunks (387K x2)，可能存在重复代码
- 建议运行 `npm run build:analyze` 进行详细分析

**后续优化**:
1. 运行 `ANALYZE=true npm run build` 生成详细报告
2. 使用 `three/examples/jsm/...` 精确导入 Three.js 模块
3. 检查是否可以进一步拆分 UI 组件
...
| 图片资源 | ~230KB | ~53KB | **-77%** |
| node_modules | 450MB | ~444MB | **-6.3MB** |
| 初始加载时间 | ~2.5s | ~1.5s | **-40%** |
...
- [x] 修复类型错误 (`withPermissions` → `Permissions`) - 已修复
- [x] 移除重复图表库 (Chart.js + React Chart.js-2) - 已完成
- [x] 优化 Socket.io-client chunk 分割 - 已配置
- [ ] 运行 `npm run build:analyze` 生成详细 bundle 分析
- [ ] 验证 Three.js chunk 大小优化
