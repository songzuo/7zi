...
export function multiFieldSearch<T extends object>(
  ...
  for (const item of items) {
    ...
    const meetsRequirements =
      (config.requireAllFields ? requiredMatched : true) &&
      matchedCount >= minMatchedFields;
 ...
 * 拼音匹配（简化版）
 */
function pinyinMatch(text: string, query: string): { match: boolean; score: number; start: number } {
  // 简单的拼音检测
 ...
 * 获取多字段搜索统计信息
 */
export function getMultiFieldSearchStats<T>(
  results: MultiFieldSearchResult<T>[]
): {
  totalResults: number;
  avgScore: number;
