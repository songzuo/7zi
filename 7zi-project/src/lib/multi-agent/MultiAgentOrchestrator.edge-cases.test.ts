...
import { MultiAgentOrchestrator, Task, WorkflowStep, ExecutionOptions } from './MultiAgentOrchestrator';
...
describe('MultiAgentOrchestrator - Edge Cases', () => {
  ...
  beforeEach(() => {
    ...
    a2a = {
      send: jest.fn().mockResolvedValue({ id: 'msg-1', from: 'orchestrator', to: 'agent-1', type: 'request', timestamp: Date.now(), payload: {} }),
  ...
  describe('extremely long inputs', () => {
    ...
    test('should handle task with many requiredCapabilities', async () => {
      registry.register(createMockAgent('agent-1', {
        capabilities: Array(100).fill(null).map((_, i) => `capability-${i}`),
      }));
      
      const task: Task = {
        id: 'many-caps',
        title: 'Many Capabilities Task',
        requiredCapabilities: Array(50).fill(null).map((_, i) => `capability-${i}`),
      };
    ...
    test('should handle workflow with many steps (100 steps)', async () => {
      ...
      const workflow: WorkflowStep[] = Array(100).fill(null).map((_, i) => ({
        taskId: `step-${i}`,
    ...
    test('should handle parallel execution with many agents (50 agents)', async () => {
      Array(50).fill(null).forEach((_, i) => {
      ...
      const agents = Array(50).fill(null).map((_, i) => createMockAgent(`agent-${i}`));
  ...
  describe('concurrent execution', () => {
    ...
    test('should handle concurrent executeSequential with separate workflows', async () => {
      ...
      const workflow1: WorkflowStep[] = [
        { taskId: 'w1-step1', task: { id: 'w1-step1', title: 'W1 Step 1', requiredCapabilities: ['research'] } },
        { taskId: 'w1-step2', task: { id: 'w1-step2', title: 'W1 Step 2', requiredCapabilities: ['research'] }, dependsOn: ['w1-step1'] },
      ];

      const workflow2: WorkflowStep[] = [
        { taskId: 'w2-step1', task: { id: 'w2-step1', title: 'W2 Step 1', requiredCapabilities: ['research'] } },
        { taskId: 'w2-step2', task: { id: 'w2-step2', title: 'W2 Step 2', requiredCapabilities: ['research'] }, dependsOn: ['w2-step1'] },
      ];
    ...
    test('should handle race condition with load updates', async () => {
      ...
      a2a.request.mockImplementation(() => new Promise(resolve => 
      ...
      const promises = Array(5).fill(null).map(() => orchestrator.assignDynamically(task));
      const results = await Promise.all(promises);
    ...
    test('should handle concurrent parallel executions', async () => {
      ...
      const agents1 = Array(5).fill(null).map((_, i) => createMockAgent(`agent-${i}`));
      const agents2 = Array(5).fill(null).map((_, i) => createMockAgent(`agent-${i + 5}`));
  ...
  describe('timeout handling', () => {
    ...
    test('should handle timeout in parallel execution', async () => {
      ...
      const agents = Array(5).fill(null).map((_, i) => createMockAgent(`agent-${i}`));
  ...
  describe('dependency edge cases', () => {
    test('should throw error for circular dependency (A -> B -> A)', async () => {
      ...
      const workflow: WorkflowStep[] = [
        { taskId: 'A', task: { id: 'A', title: 'Task A', requiredCapabilities: ['research'] }, dependsOn: ['B'] },
        { taskId: 'B', task: { id: 'B', title: 'Task B', requiredCapabilities: ['research'] }, dependsOn: ['A'] },
      ];
    ...
    test('should throw error for self dependency', async () => {
      ...
      const workflow: WorkflowStep[] = [
        { taskId: 'self-dep', task: { id: 'self-dep', title: 'Self Dependent', requiredCapabilities: ['research'] }, dependsOn: ['self-dep'] },
      ];
    ...
    test('should handle diamond dependency pattern (A -> B, A -> C, B -> D, C -> D)', async () => {
      ...
      const workflow: WorkflowStep[] = [
        ...
        { taskId: 'B', task: { id: 'B', title: 'Task B', requiredCapabilities: ['research'] }, dependsOn: ['A'] },
        { taskId: 'C', task: { id: 'C', title: 'Task C', requiredCapabilities: ['research'] }, dependsOn: ['A'] },
        { taskId: 'D', task: { id: 'D', title: 'Task D', requiredCapabilities: ['research'] }, dependsOn: ['B', 'C'] },
      ];
    ...
    test('should handle workflow with multiple dependencies', async () => {
      ...
      const workflow: WorkflowStep[] = [
        ...
        { taskId: 'D', task: { id: 'D', title: 'Task D', requiredCapabilities: ['research'] }, dependsOn: ['A', 'B', 'C'] },
      ];
    ...
    test('should throw error for non-existent dependency', async () => {
      ...
      const workflow: WorkflowStep[] = [
        { taskId: 'A', task: { id: 'A', title: 'Task A', requiredCapabilities: ['research'] }, dependsOn: ['nonexistent'] },
      ];
    ...
    test('should handle workflow with duplicate task IDs', async () => {
      ...
      const workflow: WorkflowStep[] = [
        { taskId: 'duplicate', task: { id: 'duplicate', title: 'Task 1', requiredCapabilities: ['research'] } },
        { taskId: 'duplicate', task: { id: 'duplicate', title: 'Task 2', requiredCapabilities: ['research'] } },
      ];
  ...
  describe('aggregation strategy edge cases', () => {
    test('should handle "first" strategy with all errors', async () => {
      ...
      const agents = Array(3).fill(null).map((_, i) => createMockAgent(`agent-${i}`));
      const result = await orchestrator.executeParallel(agents, task);
    ...
    test('should handle "vote" strategy with tied results', async () => {
      ...
      const agents = Array(4).fill(null).map((_, i) => createMockAgent(`agent-${i}`));
      const result = await orchestrator.executeParallel(agents, task);
  ...
  describe('miscellaneous edge cases', () => {
    ...
    test('should handle workflow with deeply nested dependencies', async () => {
      ...
      const workflow: WorkflowStep[] = [
        { taskId: 'A', task: { id: 'A', title: 'Task A', requiredCapabilities: ['research'] } },
        { taskId: 'B', task: { id: 'B', title: 'Task B', requiredCapabilities: ['research'] }, dependsOn: ['A'] },
        { taskId: 'C', task: { id: 'C', title: 'Task C', requiredCapabilities: ['research'] }, dependsOn: ['B'] },
        { taskId: 'D', task: { id: 'D', title: 'Task D', requiredCapabilities: ['research'] }, dependsOn: ['C'] },
        { taskId: 'E', task: { id: 'E', title: 'Task E', requiredCapabilities: ['research'] }, dependsOn: ['D'] },
        { taskId: 'F', task: { id: 'F', title: 'Task F', requiredCapabilities: ['research'] }, dependsOn: ['E'] },
      ];
    ...
    test('should handle retryOnFailure option (even if not implemented)', async () => {
      ...
      const result = await orchestrator.assignDynamically(task, { retryOnFailure: true, maxRetries: 3 });
      expect(result).toBeDefined();
    });
