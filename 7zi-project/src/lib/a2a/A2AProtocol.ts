...
export class A2AProtocol extends EventEmitter {
   ...
   * 发送通知
   */
  async notify(from: string, to: string, payload: unknown): Promise<A2AMessage> {
   ...
   * 发送错误
   */
  async error(from: string, to: string, correlationId: string, error: Error): Promise<A2AMessage> {
