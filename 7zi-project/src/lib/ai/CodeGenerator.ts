...
import {
  BaseProvider } from './providers/BaseProvider';
...
export class CodeGenerator {
  ...
  async generateCode(
    ...
    const userPrompt = this.provider.buildCodePrompt(
      prompt,
      lang,
    ...
    if (remainingPlaceholders && remainingPlaceholders.length > 0) {
      ...
      const response = await this.provider.chat({
        messages: [
          {
            role: 'system',
            content: 'You are a code template filler. Fill in placeholders with appropriate values.',
          },
