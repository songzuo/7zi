...
export class PerformanceMonitor implements Disposable {
  ...
  constructor(options: PerformanceMonitorOptions = {}) {
    ...
    this.activeOperations = new AutoCleanMap<string, Operation>({
      ...
      onExpire: (id: unknown, operation: unknown) => {
        ...
        console.warn(
          `[PerformanceMonitor] 操作 ${opId} (${op.name}) 超时未完成，已自动清理`
        );
        // 记录为失败
        op.status = 'failed';
        op.endTime = Date.now();
  ...
  getMetrics(): PerformanceMetrics {
    ...
    const completedCount = allOperations.filter(
      (op) => op.status === 'completed'
    ).length;
    const failedCount = allOperations.filter(
      (op) => op.status === 'failed'
    ).length;
  ...
  async dispose(): Promise<void> {
    ...
    for (const op of activeOps) {
      console.warn(
        `[PerformanceMonitor] 操作 ${op.id} (${op.name}) 未完成即被清理`
      );
    }
