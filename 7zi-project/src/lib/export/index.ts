...
export class DataExporter<T extends Record<string, unknown>> {
  ...
  private async exportExcel(data: T[]): Promise<ExportResult> {
    ...
    // 添加数据行
    transformedData.forEach((row) => {
      const values: (string | number | boolean | null | Date)[] = fields.map((f) => {
        const value = row[f.label];
    ...
    worksheet.columns.forEach((column, index) => {
      ...
      } else if (excelOptions.columnStyles?.[String.fromCharCode(65 + index)]?.width) {
        column.width = excelOptions.columnStyles[String.fromCharCode(65 + index)].width!;
      } else {
        // 自动计算宽度
        if (excelOptions.columnStyles?.[String.fromCharCode(65 + index)]?.autoWidth !== false) {
          let maxLength = 0;
          if (column.eachCell) {
    ...
    if (excelOptions.freezeRows && this.config.includeHeader) {
      worksheet.views = [{ state: 'frozen', ySplit: excelOptions.freezeRows }];
    }

    // 设置自动筛选
    if (excelOptions.autoFilter && worksheet.rowCount > 1) {
      const endColumnLetter = String.fromCharCode(64 + fields.length);
      worksheet.autoFilter = {
        from: 'A1',
        to: `${endColumnLetter}${worksheet.rowCount}`
      };
    }
...
export async function exportMultiSheet<T extends Record<string, unknown>>(
  ...
  try {
    ...
    config.sheets.forEach((sheet) => {
      ...
      // 表头
      const headerRow = worksheet.addRow(sheet.fields.map((f) => f.label));
      headerRow.font = { bold: true };
      headerRow.fill = {
        type: 'pattern',
        pattern: 'solid',
      ...
      sheet.data.forEach((row) => {
        const values: (string | number | boolean | null | Date)[] = sheet.fields.map((field) => {
          const value = row[field.key];
          if (field.formatter) {
      ...
      // 列宽
      worksheet.columns.forEach((column, index) => {
        const field = sheet.fields[index];
