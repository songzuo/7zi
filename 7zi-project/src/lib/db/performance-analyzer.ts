...
export async function analyzeSlowQueries(thresholdMs: number = 100): Promise<SlowQuery[]> {
  ...
  const queries = [
    ...
    { sql: 'SELECT * FROM agent_tokens WHERE agent_id = ? ORDER BY created_at DESC', tableName: 'agent_tokens' },

    // 数据访问表查询
    { sql: 'SELECT * FROM agent_data_access WHERE agent_id = ? ORDER BY timestamp DESC', tableName: 'agent_data_access' },
    { sql: 'SELECT * FROM agent_data_access WHERE resource_type = ? AND resource_id = ?', tableName: 'agent_data_access' },

    // 钱包表查询
    { sql: 'SELECT * FROM agent_wallets WHERE agent_id = ?', tableName: 'agent_wallets' },
    { sql: 'SELECT * FROM wallet_transactions WHERE wallet_id = ? ORDER BY created_at DESC', tableName: 'wallet_transactions' },
    { sql: 'SELECT * FROM wallet_transactions WHERE type = ? AND status = ?', tableName: 'wallet_transactions' },
  ];
...
function generateTableSuggestions(
  ...
  // 检查是否有覆盖索引
  const hasCoveringIndex = indexes.some(idx =>
    idx.columns.length > 2 && idx.columns.includes('created_at')
  );
...
export async function findMissingIndexes(): Promise<Array<{
  ...
  const patterns = [
    ...
    },
    {
      ...
      suggestedIndex: 'CREATE INDEX idx_data_access_resource ON agent_data_access(resource_type, resource_id)',
      priority: 'high' as const,
    },
    {
