...
describe('WebhookManager', () => {
  ...
  const createValidRequest = (
    overrides: Partial<CreateWebhookRequest> = {}
  ...
  describe('updateWebhook', () => {
    ...
    it('should reject update for non-existent webhook', async () => {
      await expect(manager.updateWebhook('non-existent', { enabled: false })).rejects.toThrow(
        'not found'
      )
    })
    ...
    it('should reject update with duplicate URL', async () => {
      ...
      await expect(manager.updateWebhook(another.id, { url: existingWebhook.url })).rejects.toThrow(
        'already exists'
      )
    })
  })
