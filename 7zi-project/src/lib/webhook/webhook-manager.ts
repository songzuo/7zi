...
export class WebhookManager {
  ...
  constructor(storage?: WebhookStorage, config?: Partial<WebhookConfig>) {
  ...
  async createWebhook(request: CreateWebhookRequest): Promise<WebhookEndpoint> {
    ...
    if (existing) {
      throw new WebhookError('invalid_url', 'Webhook with this URL already exists', {
        url: request.url,
      })
    }
   ...
   */
  async updateWebhook(id: string, request: UpdateWebhookRequest): Promise<WebhookEndpoint> {
  ...
  private validateUrl(url: string): void {
    try {
      ...
      if (!['http:', 'https:'].includes(parsed.protocol)) {
        throw new WebhookError('invalid_url', 'URL must use http or https protocol', { url })
      }
  ...
  private validateSecret(secret: string): void {
    if (!secret || secret.length < 8) {
      throw new WebhookError('invalid_secret', 'Secret must be at least 8 characters long', {
  ...
  private validateEvents(events: string[]): void {
    if (!events || events.length === 0) {
      throw new WebhookError('invalid_events', 'At least one event type is required')
    }
    ...
    if (invalid.length > 0) {
      throw new WebhookError('invalid_events', 'Invalid event types', { invalidEvents: invalid })
    }
  }
  ...
  private validateIpWhitelist(ips: string[]): void {
    ...
    for (const ip of ips) {
      if (!ipv4Regex.test(ip) && !ipv4CidrRegex.test(ip) && !ipv6Regex.test(ip)) {
        throw new WebhookError('invalid_url', 'Invalid IP address in whitelist', { ip })
      }
    }
  }
