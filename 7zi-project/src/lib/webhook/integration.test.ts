...
describe('Webhook System Integration', () => {
  ...
  describe('End-to-End Flow', () => {
    it('should create webhook and deliver event', async () => {
      ...
      // Emit event
      const eventId = await webhookSystem.dispatcher.emitAgentEvent(
        'agent.created',
        {
          agentId: 'agent-123',
          name: 'Test Agent',
    ...
    it('should filter events by webhook subscription', async () => {
      ...
      // Emit agent event
      const agentEventId = await webhookSystem.dispatcher.emitAgentEvent(
        'agent.created',
        { agentId: 'agent-1' }
      );

      // Emit task event
      const taskEventId = await webhookSystem.dispatcher.emitTaskEvent(
        'task.created',
      ...
      const agentWebhooks = await webhookSystem.webhookManager.getWebhooksForEvent(
        'agent.created'
      );
      expect(agentWebhooks).toHaveLength(1);
      expect(agentWebhooks[0].id).toBe(agentWebhook.id);

      const taskWebhooks = await webhookSystem.webhookManager.getWebhooksForEvent(
        'task.created'
      );
    ...
    it('should not deliver to disabled webhooks', async () => {
      ...
      const webhooks = await webhookSystem.webhookManager.getWebhooksForEvent(
        'agent.created'
      );
    ...
    it('should update webhook and reflect changes', async () => {
      ...
      // Update webhook
      const updated = await webhookSystem.webhookManager.updateWebhook(
        webhook.id,
        {
  ...
  describe('Error Handling', () => {
    ...
    it('should handle delete of non-existent webhook', async () => {
      await expect(
        webhookSystem.webhookManager.deleteWebhook('non-existent')
      ).rejects.toThrow('not found');
    });
  });
