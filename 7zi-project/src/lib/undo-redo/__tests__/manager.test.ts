...
describe('Undo-Redo Manager', () => {
  ...
  describe('Operation Grouping', () => {
    it('should group multiple operations', () => {
      ...
      expect((state.history[0].data as any)?.entries).toHaveLength(3);
    });
  ...
  describe('Timeline Management', () => {
    it('should clear future states on new operation', () => {
      ...
      let state = useUndoRedoManager.getState();
      // Calculate future states count
      const futureCount = state.history.length - state.currentIndex - 1;
      expect(futureCount).toBe(1); // entry2 is in future

      useUndoRedoManager.getState().push(entry3);

      state = useUndoRedoManager.getState();
      const futureCount2 = state.history.length - state.currentIndex - 1;
      expect(futureCount2).toBe(0);
