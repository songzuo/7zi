 ...
 * Optimized with React.memo, throttling, and batching
 */
...
import { throttle, debounce, rafThrottle, Batcher } from './throttle';
import { performanceMonitor, PerformanceTimer } from './performance';
...
function applyOperation(content: string, operation: Operation): string {
  switch (operation.type) {
    case 'insert':
      if (operation.content !== undefined && operation.position !== undefined) {
        return content.slice(0, operation.position) + operation.content + content.slice(operation.position);
      }
      return content;

    case 'delete':
      if (operation.length !== undefined && operation.position !== undefined) {
...
export function useCollaboration(config: CollaborationConfig): CollaborationState & CollaborationActions {
  ...
  // Performance optimization: memoized cursors and users
  ...
  // Batcher for operation batching
  const operationBatcherRef = useRef<Batcher<Operation> | null>(null);
  ...
  const connect = useCallback(() => {
    ...
    const connectionTimer = new PerformanceTimer();
    ...
    try {
      ...
      if (!operationBatcherRef.current) {
        operationBatcherRef.current = new Batcher<Operation>(
          (batch) => {
          ...
          { maxBatchSize: 10, maxWaitTime: 50 } // Batch up to 10 ops or 50ms
        );
      }
      ...
      socket.on('connect', () => {
        ...
        logger.info('WebSocket connected', { userId, userName, connectionTime });
      ...
      // Cursor updated - throttle state updates
      socket.on('cursor:update', (data) => {
        logger.debug('Cursor updated', { data });
        performanceMonitor.recordMessageReceived();

        // Update ref directly (faster than setState for high-frequency updates)
        cursorsMapRef.current.set(data.userId, {
          userId: data.userId,
      ...
      // Selection updated - throttle state updates
      socket.on('selection:update', (data) => {
        logger.debug('Selection updated', { data });
        performanceMonitor.recordMessageReceived();

        // Update ref directly
        const existing = cursorsMapRef.current.get(data.userId);
        cursorsMapRef.current.set(data.userId, {
          userId: data.userId,
          userName: data.userName,
          color: data.color,
        ...
        cursorUpdateCallbacksRef.current.forEach(cb => cb({
          ...
          userName: data.userName,
          color: data.color,
          position: cursorsMapRef.current.get(data.userId)?.position || 0,
          selection: data.selection,
          timestamp: Date.now(),
        }));
      });

      // Typing status updated - throttle state updates
  ...
  // Send operation - throttled to prevent flooding
  ...
  // Move cursor - throttled with RAF for smooth updates
  const moveCursor = useMemo(
    () => rafThrottle(((...args: unknown[]) => {
      const [position, selection] = args as [number, { start: number; end: number } | undefined];
      if (!currentRoomRef.current || !socketRef.current?.connected) {
        return;
      }
  ...
  const setTyping = useMemo(
    () => debounce(((...args: unknown[]) => {
      const [isTyping] = args as [boolean];
      if (!currentRoomRef.current || !socketRef.current?.connected) {
        return;
      }
