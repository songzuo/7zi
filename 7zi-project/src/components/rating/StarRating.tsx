      ...
      {getStarValues(displayRating, maxRating).map((starValue, index) => (
        <button
          ...
          className={cn(
            ...
            sizeClasses[size],
            interactive && !readonly && 'focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2 rounded',
            hoverColor
          )}
        ...
        >
          {starValue === 1 ? (
            ...
            <div className="relative">
              <Star
                className={cn('fill-current', emptyStarColor)}
                aria-hidden="true"
              />
              <StarHalf
                className={cn('absolute left-0 top-0 fill-current', starColor)}
                aria-hidden="true"
              />
            </div>
          ) : (
