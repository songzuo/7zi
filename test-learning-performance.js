...
async function runPerformanceTest() {
  ...
  if (adjustments.length > 0) {
    ...
    adjustments.slice(0, 3).forEach(adj => {
      console.log(
        `    - ${adj.agentId} (${adj.taskType}): ${adj.currentWeight.toFixed(2)} → ${adj.suggestedWeight.toFixed(2)}`
      )
  ...
  const avgScheduleTime =
    metrics.scheduleTime.reduce((a, b) => a + b, 0) / metrics.scheduleTime.length
  ...
  console.log(
    `Tasks Per Second: ${(metrics.totalTasks / (metrics.executionTime / 1000)).toFixed(2)}`
  )
  ...
  console.log(
    `  Scheduled: ${metrics.scheduledTasks} (${((metrics.scheduledTasks / metrics.totalTasks) * 100).toFixed(2)}%)`
  )
  console.log(`  Failed: ${metrics.failedTasks}`)
  console.log(`\nLearning Effectiveness:`)
  console.log(
    `  Agents with learning data: ${summary.agentsWithLearningData}/${summary.totalAgents}`
  )
