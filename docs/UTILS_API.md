...
const scroll = advancedThrottle(
  (e: Event) => console.log('Scrolling'),
  100
);
...
const cleanup1 = observeIntersection(
  element,
  (entries) => console.log('Intersection:', entries),
  { threshold: 0.5 }
);

// Resize Observer
const cleanup2 = observeResize(
  element,
  (entries) => console.log('Resize:', entries)
);
```
