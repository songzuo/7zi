...
class CapabilityAssessor implements CapabilityAssessment {
  ...
  private calculateSpeedScore(metrics: AgentLearningMetrics): {
    ...
    const score = Math.max(0, Math.min(100, 
      100 - ((avgTime - baselineTime) / baselineTime) * 50
    ));
  ...
  private detectChanges(
    ...
    for (const [taskType, typeMetrics] of Object.entries(metrics.byTaskType)) {
      ...
      const historicalSuccessRate = this.calculateHistoricalSuccessRate(agentId, taskType as TaskType);
...
class AdaptiveLearner {
  ...
  private applyAssessment(
    ...
    // 触发重新计算权重
    if (assessment.changes.improved.length > 0 || 
        assessment.changes.declined.length > 0) {
...
class ConvergenceChecker implements ConvergenceCriteria {
  ...
  checkConvergence(): ConvergenceStatus {
    ...
    // 整体收敛判定
    const converged = weightsConverged.converged && 
  ...
  private checkWeightsConvergence(): {
    ...
    // 计算权重变化的标准差
    const adjustments = recentWeights.map((w, i) => {
      if (i === 0) return 0;
    ...
    return {
      converged: avgAdjustment < this.thresholds.weightAdjustmentMax && 
  ...
  private checkPerformanceConvergence(): {
    ...
    return {
      converged: successRateVariance < Math.pow(this.thresholds.successRateVarianceMax, 2) &&
...
class LearningPersistenceManager implements LearningPersistence {
  ...
  private deserialize(data: string): LearningState {
    ...
    return {
      ...
      weightCache: new Map(parsed.weightCache.map(([k, v]: [string, any[]]) => [
        k, new Map(v)
      ])),
