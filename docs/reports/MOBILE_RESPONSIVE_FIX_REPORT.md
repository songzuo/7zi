...
// 添加自定义箭头和触摸优化
className =
  "w-full px-4 sm:px-6 py-4 sm:py-5 ... min-h-[56px] ... touch-active appearance-none bg-[url('data:image/svg+xml...')]"
```
...
<div className={`flex flex-wrap gap-2 sm:gap-3 ${className}`}>
  <Link className={`min-h-[44px] ...`}>
    <span className="hidden font-medium text-white sm:inline">{social.name}</span>
  </Link>
</div>
```
...
<div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-5 lg:gap-12">
  ...
  <div>
    <ul className="space-y-2 sm:space-y-3">
      <li>
        <Link className="block flex min-h-[44px] items-center py-1 text-sm ...">{link.name}</Link>
      </li>
    </ul>
  </div>
</div>
