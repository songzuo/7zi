...
```tsx
<div className="relative h-[calc(100vh-80px)] w-full">{/* 3D 容器 - 无响应式调整 */}</div>
```
...
```tsx
<div className="fixed top-4 right-4 ...">{/* 使用绝对定位,但可能在小屏幕上遮挡内容 */}</div>
```
...
;<div className="relative h-[calc(100vh-80px)] w-full sm:h-[calc(100vh-64px)]">
  {/* 响应式高度 */}
</div>
...
;<div className="absolute top-4 right-4 hidden sm:block ...">{/* 桌面端控件 */}</div>

{
  /* 移动端简化控件 */
}
;<div className="absolute right-4 bottom-4 sm:hidden ...">{/* 移动端按钮 */}</div>
```
...
// 解决方案:
test('应该应用自定义类名', () => {
  const { container } = render(<SimplePerformanceDashboard className="custom-class" />)
