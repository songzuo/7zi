...
manager._customRoles
  .get(r.id)(
    // After
    manager as any
  )
  ._customPermissions.get(p.id)(manager as any)
  ._customRoles.get(r.id)
```
...
| File                                                 | Errors | Status         |
| ---------------------------------------------------- | ------ | -------------- |
| src/app/api/users/batch/bulk/**tests**/route.test.ts | 11     | 🔄 Partial Fix |
| src/lib/**tests**/permissions.test.ts                | 24     | 🔄 Partial Fix |
| src/lib/auth/jwt.test.ts                             | 30     | ❌ Not Fixed   |
| src/lib/a2a/**tests**/\*.test.ts                     | 8      | ❌ Not Fixed   |
| src/lib/cache/**tests**/\*.test.ts                   | 15     | ❌ Not Fixed   |
...
- Test files (_.test.ts/_.test.tsx) often have looser type requirements in practice
