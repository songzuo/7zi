...
export class WebSocketMonitor {
  ...
  trackSocketIO(socket: Socket, namespace: string = 'default'): void {
    ...
    socket.on('connect', () => {
      ...
      monitor.trackCustomMetric(`ws_${namespace}_connect_time`, metrics.connectTime, 'ms', {
        namespace,
      })
    })
    ...
    socket.on('connect_error', error => {
      ...
      monitor.trackError('WebSocketConnectError', error.message, error.stack, { namespace })
    })
    ...
    const interval = setInterval(() => {
      if (socket.connected) {
        ...
        socket.once('pong', (data: { timestamp: number }) => {
          ...
          monitor.trackCustomMetric(`ws_${namespace}_latency`, latency, 'ms', { namespace })
        })
      }
    }, 5000) // 每 5 秒测试一次
...
export function initRouteTracking(): void {
  ...
  window.next.router.push = async (...args: Parameters<typeof originalPush>) => {
    ...
    try {
      ...
      monitor.trackCustomMetric('route_change', Date.now() - startTime, 'ms', {
        path: String(path),
      })
...
export function checkPerformanceBudget(
  ...
  for (const metric of metrics) {
    ...
    if (threshold && metric.value > threshold) {
      violations.push(`${metric.name}: ${metric.value.toFixed(0)}ms (budget: ${threshold}ms)`)
    }
  }
