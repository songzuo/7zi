...
import {
  ...
  PresenceIndicator,
  PresenceStats,
} from '@/components/MemberPresenceBoard'
```
...
```tsx
import { exportToPDF, exportTasksPDF, exportMembersCSV, exportCompleteReport } from '@/lib/export'
      ...
      <ul>
        {tasks.map(task => (
          <li key={task.id}>
            ...
            <button onClick={() => handleStatusChange(task.id, 'done')}>完成</button>
...
function MyComponent() {
  ...
  return <button onClick={toggleTheme}>当前: {resolvedTheme === 'dark' ? '🌙' : '☀️'}</button>
}
```
