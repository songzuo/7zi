  ...
  async createTask(tenantId: string, task: Omit<Task, 'id'>): Promise<Task> {
    ...
    await shard.execute(
      `INSERT INTO tasks (id, tenant_id, title, description, status, priority, assignee_id, creator_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, tenantId, task.title, task.description, task.status, task.priority, task.assignee_id, task.creator_id]
    );
...
export class ClusterManager {
  ...
  constructor(httpServer: any) {
    ...
    this.io.adapter(
      new RedisAdapter(this.pubClient, this.subClient)
    );
  ...
  private async saveMessage(data: { roomId: string; message: string; userId: string }): Promise<void> {
    const message = {
      id: uuidv4(),
      roomId: data.roomId,
      message: data.message,
  ...
  for (const task of tasks) {
    await mysqlConn.execute(
      `INSERT INTO tasks (id, tenant_id, title, description, status, priority, assignee_id, creator_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [task.id, task.tenant_id || 'default', task.title, task.description, task.status, task.priority, task.assignee_id, task.creator_id, task.created_at]
    );
  }
 ...
 */
export function tenantDataIsolationMiddleware(req: Request, res: Response, next: NextFunction): void {
...
export class RoomIsolation {
   ...
   * 租户加入房间
   */
  async joinTenantRoom(socket: any, tenantId: string, roomType: string, roomId: string): Promise<void> {
   ...
   * 向租户房间发送消息
   */
  async sendToTenantRoom(tenantId: string, roomType: string, roomId: string, message: any): Promise<void> {
    const tenantRoomId = this.getTenantRoomId(tenantId, roomType, roomId);
   ...
   * 检查租户房间权限
   */
  private async checkRoomPermission(tenantId: string, roomType: string, roomId: string): Promise<boolean> {
    // 从数据库查询房间归属
