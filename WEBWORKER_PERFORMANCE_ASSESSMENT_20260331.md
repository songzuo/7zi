...
export function useSearchWorker() {
  const searchWorker = useWorker<typeof import('@/workers/search-worker').workerAPI>(
    proxy(worker)
  );
...
function SearchComponent({ query }) {
  ...
  const results = useMemo(() => searchItems(data, deferredQuery), 
    [data, deferredQuery]);
}
```
