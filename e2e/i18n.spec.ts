...
test.describe('多语言 - 语言切换器测试', () => {
  ...
  test('切换到英文', async ({ page }) => {
    ...
    const englishOption = page.locator(
      'text=English, button:has-text("EN"), [role="menuitem"]:has-text("English")'
    ).first();
    ...
    if (await englishOption.isVisible()) {
      ...
      const url = page.url();
      const hasEnglish = url.includes('/en') || await page.locator('text=/Home|About/i').first().isVisible();
  ...
  test('切换到中文', async ({ page }) => {
    ...
    const chineseOption = page.locator(
      'text=中文，text=Chinese, button:has-text("中文")'
    ).first();
    ...
    if (await chineseOption.isVisible()) {
      ...
      const url = page.url();
      const hasChinese = url.includes('/zh') || await page.locator('text=/首页 | 关于/i').first().isVisible();
...
test.describe('多语言 - 内容本地化测试', () => {
  test('中文导航栏应该显示中文', async ({ page }) => {
    ...
    for (const item of chineseNavItems) {
      if (await nav.locator(`text=/${item}/`).first().isVisible().catch(() => false)) {
        foundItems++;
      }
    }
  ...
  test('英文导航栏应该显示英文', async ({ page }) => {
    ...
    for (const item of englishNavItems) {
      if (await nav.locator(`text=/${item}/i`).first().isVisible().catch(() => false)) {
        foundItems++;
      }
    }
...
test.describe('多语言 - 表单本地化测试', () => {
  ...
  test('英文联系表单应该显示英文标签', async ({ page }) => {
    ...
    for (const label of englishLabels) {
      if (await page.locator(`text=/${label}/i`).first().isVisible().catch(() => false)) {
        foundLabels++;
      }
    }
...
test.describe('多语言 - 响应式测试', () => {
  test('移动端语言切换器应该可用', async ({ page }) => {
    ...
    // 语言切换器应该在移动端也可用
    const langSwitcher = page.locator(
      'button:has-text("EN"), button:has-text("中文"), [aria-label*="language"]'
    ).first();
