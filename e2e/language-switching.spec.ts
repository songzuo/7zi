...
test.describe('语言切换器 - 界面显示', () => {
  ...
  test('语言切换器应显示当前语言标志', async ({ page }) => {
    ...
    // 中文页面应显示中文切换到英文的标志
    const hasUsFlag = await langSwitcher.evaluate(el =>
      el.textContent?.includes('🇺🇸') || el.textContent?.includes('EN')
    );
...
test.describe('语言切换 - 可访问性', () => {
  ...
  test('语言切换器应有键盘支持', async ({ page }) => {
    ...
    const isLangSwitcher = await focusedElement.evaluate(el =>
      el === langSwitcher.elementHandle()
    );
