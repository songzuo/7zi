...
test.describe('团队页面 - 基础测试', () => {
  ...
  test('成员卡片应该包含必要信息', async ({ page }) => {
    ...
    const memberCards = page
      .locator('[class*="member"], [class*="team-member"], [class*="card"]')
      .filter({ has: page.locator('text=/专家 | 师 | 员/i') })
...
test.describe('团队页面 - 成员展示测试', () => {
  ...
  test('成员卡片应该显示角色/职责', async ({ page }) => {
    ...
    const roles = page.locator(
      'text=/专家 | 咨询 | 架构 | 执行 | 管理 | 测试 | 设计 | 推广 | 销售 | 财务 | 媒体/i'
    )
...
test.describe('团队页面 - 导航测试', () => {
  ...
  test('从团队页面导航回首页', async ({ page }) => {
    ...
    // 点击首页链接
    const homeLink = page
      .locator('nav a[href="/"], nav a:has-text("7zi"), nav a:has-text("首页")')
      .first()
...
test.describe('团队页面 - 无障碍测试', () => {
  ...
  test('成员卡片应该可访问', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(count, 3); i++) {
      ...
      // 检查是否有可访问的标签
      const roleName = await card
        .locator('h3, h4, [class*="name"], [class*="title"]')
        .first()
        .textContent()
