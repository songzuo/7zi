...
test.describe('Role-Based Access Control', () => {
  test.describe('Admin Role', () => {
    ...
    test('should allow admin to manage roles', async ({ page }) => {
      ...
      const createRoleButton = page.locator('button:has-text("新建角色"), button:has-text("Create Role")');
  ...
  test.describe('User Role', () => {
    ...
    test('should deny user access to admin pages', async ({ page }) => {
      ...
      for (const pagePath of adminPages) {
        ...
        const url = page.url();
        const hasAccessDenied = await page.locator('text=访问被拒绝, Access Denied, 无权限').count() > 0;
    ...
    test('should show user-appropriate navigation', async ({ page }) => {
      ...
      const adminLinks = page.locator('a:has-text("用户管理"), a:has-text("User Management"), a[href="/admin"]');
...
test.describe('Permission Management', () => {
  ...
  test('should create new permission', async ({ page }) => {
    ...
    // Click create button
    const createButton = page.locator('button:has-text("新建权限"), button:has-text("Create Permission")');
  ...
  test('should revoke permission from role', async ({ page }) => {
    ...
    if (await roleEditButton.isVisible()) {
      ...
      // Check permission checkboxes
      const permissionCheckboxes = page.locator('input[type="checkbox"].permission-checkbox:checked');
...
test.describe('Role Management', () => {
  ...
  test('should delete role', async ({ page }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      // Confirm deletion
      await page.click('button:has-text("确认"), button:has-text("Confirm"), button:has-text("确定")');
  ...
  test('should show role permissions', async ({ page }) => {
    ...
    const viewButton = page.locator('button:has-text("查看权限"), button:has-text("View Permissions")').first();
...
test.describe('Permission Edge Cases', () => {
  ...
  test('should handle permission hierarchy', async ({ page }) => {
    ...
    const adminRow = page.locator('tr:has-text("Admin"), tr:has-text("管理员")');
    const viewPermissions = adminRow.locator('button:has-text("查看权限"), button:has-text("View Permissions")');
