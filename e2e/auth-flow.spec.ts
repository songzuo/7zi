...
test.describe('Authentication Flow', () => {
  ...
  test('should validate email format', async ({ page }) => {
    ...
    await expect(
      page.locator('text=邮箱格式错误, invalid email, Invalid email').first()
  ...
  test('should show error for empty fields', async ({ page }) => {
    ...
    await expect(page.locator('text=必填, required, Required').first()).toBeVisible({
      timeout: 2000,
    })
  })
  ...
  test('should handle invalid credentials', async ({ page }) => {
    ...
    await expect(
      page.locator('text=邮箱或密码错误, invalid credentials, Invalid email or password').first()
  ...
  test('should support "remember me" functionality', async ({ page }) => {
    ...
    const rememberCheckbox = page.locator(
      'input[type="checkbox"][name="remember"], input[type="checkbox"][value="remember"], label:has-text("记住我"), label:has-text("Remember me")'
    )
  ...
  test('should navigate to registration page', async ({ page }) => {
    ...
    const registerLink = page
      .locator('text=注册, Register, Sign Up, 创建账户, Create account')
      .first()
