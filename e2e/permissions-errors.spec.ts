  ...
  test('should restrict admin-only routes', async ({ page }) => {
    ...
    for (const route of adminRoutes) {
      ...
      const isAccessDenied =
        (await page.locator('text=未授权, Access Denied, 403, Forbidden').count()) > 0
...
test.describe('Error Handling', () => {
  test('should handle 404 page not found', async ({ page }) => {
    ...
    // Check for 404 page
    const notFoundElement = page
      .locator('h1, h2')
      .filter({ hasText: /404|not found|未找到|页面不存在/i })
  ...
  test('should handle network errors', async ({ page, context }) => {
    ...
    const networkError = page
      .locator('text=网络错误, Network Error, 连接失败, Connection failed')
      .first()
...
test.describe('Error Recovery', () => {
  ...
  test('should allow users to report errors', async ({ page }) => {
    ...
    const reportButton = page
      .locator('button:has-text("报告"), button:has-text("Report"), button:has-text("反馈")')
      .first()
...
test.describe('Loading States', () => {
  ...
  test('should show empty state when no data', async ({ page }) => {
    ...
    const emptyState = page
      .locator('.empty-state, [data-testid="empty"], text=暂无数据, text=No data')
      .first()
