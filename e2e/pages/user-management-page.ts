...
export class UserManagementPage {
  ...
  async exportUsers(format: 'csv' | 'xlsx' | 'pdf' = 'csv'): Promise<void> {
    const exportButton = this.page.locator(
      `button:has-text("导出${format.toUpperCase()}"), button:has-text("Export as ${format}")`
    )
  ...
  async bulkDelete(): Promise<void> {
    const bulkDeleteButton = this.page.locator(
      'button:has-text("批量删除"), button:has-text("Bulk Delete")'
    )
  ...
  async viewUserDetails(userEmail: string): Promise<void> {
    const userRow = this.userTable.locator(`tr:has-text("${email}")`)
    const viewButton = userRow.locator(
  ...
  async closeUserDetails(): Promise<void> {
    const closeButton = this.page.locator(
      'button:has-text("关闭"), button:has-text("Close"), [aria-label="Close"]'
    )
