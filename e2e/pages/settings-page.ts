...
export class SettingsPage {
  ...
  constructor(page: Page) {
    ...
    this.notificationsSection = page.locator('[data-section="notifications"], .notifications-section');
    ...
    // Profile form
    this.nameInput = this.profileSection.locator('input[name="name"], input[placeholder*="姓名"], input[placeholder*="Name"]');
    this.emailInput = this.profileSection.locator('input[type="email"], input[name="email"]');
    this.bioTextarea = this.profileSection.locator('textarea[name="bio"], textarea[placeholder*="简介"], textarea[placeholder*="Bio"]');
    this.avatarUpload = this.profileSection.locator('input[type="file"], .avatar-upload');
    this.saveProfileButton = this.profileSection.locator('button:has-text("保存"), button:has-text("Save")');
  ...
  async navigateToSection(section: string): Promise<void> {
    const navItem = this.settingsNav.locator(`a:has-text("${section}"), button:has-text("${section}")`).first();
  ...
  async enableEmailNotifications(): Promise<void> {
    ...
    const toggle = this.notificationsSection.locator('input[type="checkbox"][name="email"], .toggle.email-notifications');
  ...
  async enablePushNotifications(): Promise<void> {
    await this.navigateToSection('通知' || 'Notifications');
    const toggle = this.notificationsSection.locator('input[type="checkbox"][name="push"], .toggle.push-notifications');
    if (await toggle.isVisible()) {
      await toggle.click();
    }
  }
  ...
  async changePassword(currentPassword: string, newPassword: string): Promise<void> {
    ...
    const currentPassInput = this.securitySection.locator('input[name="currentPassword"], input[placeholder*="当前"]');
    const newPassInput = this.securitySection.locator('input[name="newPassword"], input[placeholder*="新密码"]');
    const confirmPassInput = this.securitySection.locator('input[name="confirmPassword"], input[placeholder*="确认"]');
    const saveButton = this.securitySection.locator('button:has-text("修改密码"), button:has-text("Change Password")');
  ...
  async enableTwoFactorAuth(): Promise<void> {
    ...
    const toggle = this.securitySection.locator('input[type="checkbox"][name="2fa"], .toggle.two-factor');
    if (await toggle.isVisible()) {
      await toggle.click();
    }
  }
