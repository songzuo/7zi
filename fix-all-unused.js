...
function getLintOutput() {
  try {
    const output = execSync('npx eslint src --format=compact', {
      cwd: PROJECT_DIR,
      encoding: 'utf8',
      timeout: 120000,
    })
    ...
    // eslint returns non-zero on errors, but we still want the output
    return error.stdout || ''
  }
}
...
function parseWarnings(output) {
  ...
  for (const line of lines) {
    ...
    // Parse format: /path/to/file.ts:line:col: warning  'var' is defined but never used  @typescript-eslint/no-unused-vars
    const match = line.match(
      /^([^:]+):(\d+):(\d+):\s+\w+\s+'([^']+)'\s+is\s+(defined\s+but\s+never\s+used|assigned\s+a\s+value\s+but\s+never\s+used)/
    )
    if (match) {
      warnings.push({
...
function fixUnusedVariable(content, lineNum, varName) {
  ...
  if (varMatch && varMatch[2] === varName) {
    ...
    lines[lineNum - 1] = line.replace(new RegExp(`\\b${varName}\\b`), `_${varName}`)
    return lines.join('\n')
  }
  ...
  try {
    ...
    for (const warning of sorted) {
      ...
      if (result !== null) {
        ...
        modified = true
        console.log(
          `  ✓ Removed unused import '${warning.varName}' from ${path.relative(PROJECT_DIR, filePath)}:${warning.lineNum}`
        )
        continue
      }
      ...
      if (varResult !== null) {
        ...
        modified = true
        console.log(
          `  ✓ Renamed unused variable '${warning.varName}' to '_${warning.varName}' in ${path.relative(PROJECT_DIR, filePath)}:${warning.lineNum}`
        )
      }
    }
