...
export async function waitForWebSocketMessage(page: Page, eventType: string, timeout = 5000): Promise<unknown> {
  return await page.evaluate(
    ({ eventType, timeout }) => {
      return new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => reject(new Error(`WebSocket message timeout: ${eventType}`)), timeout);
...
export async function measureActionTime<T>(fn: () => Promise<T>): Promise<{ result: T; duration: number }> {
  const startTime = Date.now();
