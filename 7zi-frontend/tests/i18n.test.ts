...
function findUntranslated(obj: TranslationFile, enObj: TranslationFile, prefix = ''): string[] {
  ...
  for (const key in obj) {
    ...
    } else if (typeof value === 'object' && typeof enValue === 'object' && value !== null && enValue !== null) {
      untranslated.push(...findUntranslated(value as TranslationFile, enValue as TranslationFile, fullKey));
    }
  }
  return untranslated;
}
...
function findToolCallArtifacts(obj: TranslationFile, prefix = ''): string[] {
  ...
  for (const key in obj) {
    ...
    if (typeof value === 'string') {
      if (value.includes('<minimax:tool_call>') || value.includes('<tool_call>') || value.includes('</tool_call>')) {
      ...
      const findMixedRecursive = (obj: TranslationFile, prefix = '') => {
        for (const key in obj) {
          ...
          if (typeof value === 'string') {
            ...
            const containsProblematicChinese = value.includes('年中无公害') ||
      ...
      const findMixedRecursive = (obj: TranslationFile, prefix = '') => {
        for (const key in obj) {
          ...
          if (typeof value === 'string') {
            ...
            const containsJapanese = /[\u3040-\u309f\u30a0-\u30ff]/.test(value) && !/[\u4e00-\u9faf]/.test(value);
            const containsChinese = /[\u4e00-\u9fa5]/.test(value);
            const containsMixed = value.includes('hybrid') || value.includes('現代') ||
              value.includes('メンバー') || value.includes('关于我们') ||
