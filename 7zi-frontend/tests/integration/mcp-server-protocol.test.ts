...
class MockMCPServer {
  ...
  private requestHandlers: Map<string, (params?: Record<string, unknown>) => Promise<unknown>> = new Map();
  ...
  registerTool(definition: MCPToolDefinition, executor: (params: Record<string, unknown>) => Promise<unknown>): void {
...
describe('MCP HTTP Transport', () => {
  ...
  it('should handle batch requests', async () => {
    ...
    const batch: MCPRequest[] = [
      ...
      { jsonrpc: '2.0', id: 'batch-3', method: 'tools/call', params: { name: 'read_file', arguments: { path: '/test' } } },
    ];

    const responses = await Promise.all(
      batch.map(r => server.handleRequest(r))
    );
