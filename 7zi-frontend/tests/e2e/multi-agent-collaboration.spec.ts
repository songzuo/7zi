...
class AgentDashboard {
  ...
  async getAgents(): Promise<Agent[]> {
    ...
    return Promise.all(agents.map(async (el) => {
      return {
        ...
        capabilities: (await el.locator('[data-testid="agent-capability"]').allTextContents()).filter(Boolean),
        status: (await el.locator('[data-testid="agent-status"]').textContent())?.toLowerCase() as Agent['status'],
      };
    }));
  }
  ...
  async getTasks(): Promise<Task[]> {
    ...
    return Promise.all(tasks.map(async (el) => {
      return {
        id: await el.getAttribute('data-task-id') || '',
        type: await el.locator('[data-testid="task-type"]').textContent() || '',
        priority: (await el.locator('[data-testid="task-priority"]').textContent())?.toLowerCase() as Task['priority'],
        status: (await el.locator('[data-testid="task-status"]').textContent())?.toLowerCase() as Task['status'],
        assignedAgent: await el.locator('[data-testid="assigned-agent"]').textContent() || undefined,
      };
    }));
  }
