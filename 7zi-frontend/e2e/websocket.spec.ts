...
test.describe('WebSocket 连接', () => {
  ...
  test('应该接收消息', async ({ page }) => {
    ...
    // 验证收到响应
    const receivedMessage = wsPage.messageList.getByTestId('received-message').filter({ hasText: 'Test message' });
...
test.describe('WebSocket 自动重连', () => {
  ...
  test('应该在超过最大重连次数后停止', async ({ page }) => {
    ...
    // 验证显示连接失败
    await expect(page.getByTestId('ws-connection-status')).toContainText(/连接失败|connection failed/i);
