...
export class AgentCollaborationPage {
  ...
  async selectAggregateStrategy(
    strategy: 'first' | 'last' | 'all' | 'majority' | 'best' | 'average' | 'merge' | 'custom'
  ) {
...
test.describe('Agent 协作流程', () => {
  ...
  test('TC-004-01: 单 Agent 执行任务', async ({ authenticatedPage }) => {
    ...
    // 等待完成
    await expect(authenticatedPage.getByTestId('task-status')).toHaveText(/已完成|completed/i, {
      timeout: 30000,
    })
  })
...
export async function waitForWebSocket(page: Page, timeout = 5000) {
  await page.waitForFunction(() => (window as any).wsConnected === true, { timeout })
}
