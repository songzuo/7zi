...
export const CardFooter: React.FC<CardFooterProps> = ({ children, className }) => {
  return (
    <div className={clsx('border-t border-gray-200 bg-gray-50 px-6 py-4', className)}>
      {children}
    </div>
  )
}
...
export const CardImage: React.FC<CardImageProps> = ({ src, alt, className }) => {
  return <img src={src} alt={alt} className={clsx('h-48 w-full object-cover', className)} />
}
...
export const CardText: React.FC<CardTextProps> = ({ children, className }) => {
  return <p className={clsx('mt-2 text-gray-600', className)}>{children}</p>
}
