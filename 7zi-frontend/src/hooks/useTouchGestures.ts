...
export function useTouchGestures(
  ...
  }, [enableZoom, enableDrag, enableLongPress, state.scale, state.isLongPressing, minZoom, maxZoom, onZoom, onDrag]);
  ...
  const handleTouchEnd = useCallback((e: TouchEvent) => {
    ...
    // 滑动检测
    if (enableSwipe && !state.isDragging && !state.isZooming && touchStartRef.current.touches === 1) {
  ...
  }, [enableSwipe, state.isDragging, state.isZooming, state.isLongPressing, swipeThreshold, onSwipe, onTap]);
...
export function usePinchToZoom(options: PinchToZoomOptions = {}) {
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    if (e.touches.length === 2) {
      ...
      const newScale = Math.min(
        Math.max(
          (currentDistance / startDistanceRef.current) * startScaleRef.current,
          minZoom
        ),
        maxZoom
      );
