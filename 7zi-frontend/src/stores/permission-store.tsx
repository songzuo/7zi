...
export interface PermissionState {
   ...
   */
  checkResourceAccess: (resourceOwnerId: string, requiredPermission: ContextPermission) => boolean
...
const ROLE_PERMISSIONS: Record<Role, ContextPermission[]> = {
  ...
  [Role.USER]: [ContextPermission.READ, ContextPermission.WRITE, ContextPermission.DELETE],
  [Role.GUEST]: [ContextPermission.READ],
}
...
export const usePermissionStore = create<PermissionState>()(
  persist(
    (set, get) => ({
       ...
       * 检查是否可以访问资源 (兼容 PermissionContext)
       */
      checkResourceAccess: (resourceOwnerId: string, requiredPermission: ContextPermission) => {
        const { user } = get()
        if (!user) return false
