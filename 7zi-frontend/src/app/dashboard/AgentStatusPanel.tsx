    ...
    <Card hoverable className={clsx('group relative', !agent.enabled && 'opacity-60')}>
      ...
      <CardBody padding="sm" className="pl-4">
        ...
        <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>
            {t('agent.lastActive')}: {lastActiveText}
          </span>
        </div>
      </CardBody>
...
export const AgentStatusPanel = memo(function AgentStatusPanel({
  ...
  if (agents.length === 0) {
    return (
      <div className={clsx('px-4 py-12 text-center', className)}>
        <div className="mb-4 text-6xl">🤖</div>
        <h3 className="mb-2 text-xl font-semibold text-gray-900 dark:text-gray-100">
          {t('agent.noAgents')}
        </h3>
        <p className="mb-4 text-gray-600 dark:text-gray-400">{t('agent.noAgentsDescription')}</p>
  ...
  return (
    <div className={clsx('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{t('agent.title')}</h2>
      ...
      {filteredAgents.length === 0 ? (
        <div className="py-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">{t('agent.noMatching')}</p>
        </div>
      ) : (
        <>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
