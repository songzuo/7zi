...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const agentsWithStats = (Array.isArray(allAgentStats) ? allAgentStats : [allAgentStats]).map(stats => {
      ...
      return {
        ...
        capabilities: Object.fromEntries(Array.from(stats.capabilityScores.entries()).map(([type, cap]) => [type, {
    ...
    const agentsWithoutStats = schedulerAgents.filter(a => !agentsWithStats.find(s => s.agentId === a.id)).map(agent => ({
      agentId: agent.id,
