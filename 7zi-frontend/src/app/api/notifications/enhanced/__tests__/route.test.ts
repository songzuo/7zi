...
describe('Enhanced Notifications API - GET /api/notifications/enhanced', () => {
  ...
  it('应该支持用户ID过滤（仅管理员）', async () => {
    ...
    const request = new NextRequest(
      'http://localhost:3000/api/notifications/enhanced?userId=user-2',
      {
        method: 'GET',
      }
    )
  ...
  it('应该支持优先级过滤', async () => {
    const request = new NextRequest(
      'http://localhost:3000/api/notifications/enhanced?priority=high',
      {
        method: 'GET',
      }
    )
  ...
  it('应该支持时间过滤', async () => {
    const request = new NextRequest(
      'http://localhost:3000/api/notifications/enhanced?since=1234567890',
      {
        method: 'GET',
      }
    )
  ...
  it('应该支持自定义邮件接收者', async () => {
    const request = new NextRequest('http://localhost:3000/api/notifications/enhanced', {
      ...
      body: JSON.stringify({
        title: 'Test',
        message: 'Test message',
        emailRecipients: [{ email: 'user@example.com', name: 'Test User' }],
      }),
    })
