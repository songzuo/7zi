...
const STATUS_CONFIG: Record<
  Task['status'],
  { label: string; color: 'blue' | 'yellow' | 'purple' | 'green' }
> = {
  ...
  return (
    ...
    >
      ...
      <CardHeader bordered={false} className="pl-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <CardBadge color={statusConfig.color} variant="soft" className="mb-2">
              {statusConfig.label}
            </CardBadge>
          ...
          <CardBadge color={priorityConfig.color} variant="outline" size="sm">
      ...
      <CardBody padding="sm" className="pl-4">
        <p className="line-clamp-3 text-sm text-gray-600 dark:text-gray-400">{task.description}</p>
  ...
  if (tasks.length === 0) {
    return (
      <div className="px-4 py-12 text-center">
        ...
        <h3 className="mb-2 text-xl font-semibold text-gray-900 dark:text-gray-100">暂无任务</h3>
        <p className="text-gray-600 dark:text-gray-400">创建第一个任务开始吧！</p>
      </div>
    )
  }
...
export const TaskStatusToggle = memo(function TaskStatusToggle({
  currentStatus,
  onStatusChange,
  disabled,
