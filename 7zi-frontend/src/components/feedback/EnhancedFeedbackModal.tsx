...
import { 
  X, Star, Upload, Camera, Send, Loader2, Sparkles, 
  AlertCircle, CheckCircle2, MessageSquare, Lightbulb,
  Wand2, Copy, ThumbsUp, ThumbsDown
} from 'lucide-react';
            ...
            <div className="grid grid-cols-3 gap-3">
              {TEMPLATES.map(template => (
                <button
                  ...
                  className="p-3 text-left bg-white border border-gray-200 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition-all"
                >
                  <div className="flex items-center space-x-2 mb-1">
                    <Lightbulb className="w-4 h-4 text-blue-600" />
                    <span className="text-sm font-medium text-gray-900">
                      {template.type === 'bug' ? '问题报告' : 
                       template.type === 'feature' ? '功能建议' : '性能问题'}
                    </span>
                  </div>
                  <p className="text-xs text-gray-500">
                    {template.type === 'bug' ? '结构化的问题描述模板' :
                     template.type === 'feature' ? '新功能建议模板' : '性能问题报告模板'}
                  </p>
                </button>
              ))}
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <div className="space-y-2 text-sm text-purple-700">
                  {aiSuggestion.type && (
                    <p>• 建议分类: <strong>{aiSuggestion.type}</strong></p>
                  )}
                  {aiSuggestion.priority && (
                    <p>• 建议优先级: <strong>{aiSuggestion.priority}</strong></p>
                  )}
                  {aiSuggestion.tags && aiSuggestion.tags.length > 0 && (
                    <p>• 建议标签: <strong>{aiSuggestion.tags.join(', ')}</strong></p>
                  )}
                </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {['low', 'medium', 'high', 'urgent'].map(priority => (
              <button
                key={priority}
                type="button"
                onClick={() => setFeedback(prev => ({ ...prev, priority: priority as FeedbackPriority }))}
                className={`px-4 py-2 text-sm font-medium rounded-lg border-2 transition-all ${
