...
describe('WebSocketAdvancedService Integration', () => {
  ...
  describe('Room and Message Integration', () => {
    ...
    it('should not allow sending messages without permission', async () => {
      ...
      await expect(
        service.sendMessage(room.id, 'user2', 'Jane', { text: 'Hello!' }, 'text')
    ...
    it('should retrieve room messages', async () => {
      ...
      await service.sendMessage(room.id, 'user1', 'Owner', { text: 'Message 1' }, 'text')

      await service.sendMessage(room.id, 'user2', 'Jane', { text: 'Message 2' }, 'text')
    ...
    it('should support message replies', async () => {
      ...
      await service.sendMessage(room.id, 'user1', 'Owner', { text: 'Original message' }, 'text')
  ...
  describe('Permission Integration', () => {
    ...
    it('should enforce message edit permissions', async () => {
      ...
      // Sender can edit
      const editResult = await service.editMessage(message.id, 'user2', { text: 'Edited' })
      expect(editResult.success).toBe(true)

      // Non-sender cannot edit
      const editResult2 = await service.editMessage(message.id, 'user1', { text: 'Edited again' })
