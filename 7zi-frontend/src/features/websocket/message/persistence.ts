...
export class MessagePersistence {
  ...
  async syncOfflineMessages(
    ...
    logger.info(
      `[MessagePersistence] Synced ${offlineMessages.length} offline messages for user ${userId}`
    )
  ...
  private async cleanupOldMessages(roomId: string): Promise<void> {
    ...
    if (filtered.length !== originalSize) {
      logger.info(
        `[MessagePersistence] Cleaned up ${originalSize - filtered.length} old messages in room ${roomId}`
      )
    }
  }
  ...
  async getUnreadCount(roomId: string, userId: string, since: number): Promise<number> {
    ...
    return messages.filter(m => m.createdAt > since && !m.readBy.includes(userId) && !m.deletedAt)
      .length
  }
