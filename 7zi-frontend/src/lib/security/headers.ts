...
export function generatePermissionsPolicy(config: PermissionsPolicyConfig): string {
  ...
  if (config['interest-cohort']) policies.push(`interest-cohort=${formatValue(config['interest-cohort'])}`);
 ...
 * 获取所有安全头部
 */
export function getSecurityHeaders(environment: Environment = 'production'): Record<string, string> {
 ...
 */
export function getCSPReportOnlyConfig(environment: Environment = 'production'): Record<string, string> {
  const config = getSecurityConfig(environment);
