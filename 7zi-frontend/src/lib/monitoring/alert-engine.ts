...
export class AlertEngine {
   ...
   */
  private evaluateTrend(condition: AlertCondition, metric: string, value: number): boolean {
   ...
   */
  private evaluateRateChange(condition: AlertCondition, metric: string, value: number): boolean {
    const trend = this.trendData.get(metric)
    if (!trend?.baseline) return false
   ...
   */
  private evaluateWebVital(condition: AlertCondition, metric: string, value: number): boolean {
   ...
   */
  private evaluateErrorRate(condition: AlertCondition, metric: string, value: number): boolean {
    const threshold = condition.threshold ?? 0
    return value > threshold
  }
   ...
   */
  private evaluateApiLatency(condition: AlertCondition, metric: string, value: number): boolean {
    const threshold = condition.threshold ?? 0
    return value > threshold
  }
   ...
   */
  private async shouldSuppress(rule: AlertRule, metric: string, value: number): Promise<boolean> {
    const { suppression } = this.config
    const now = Date.now()

    // Check max alerts in window
    const recentAlerts = this.alertHistory.filter(a => now - a.timestamp < suppression.windowMs)
  ...
  private async sendAlert(alert: Alert, rule: AlertRule): Promise<void> {
    const channelNames = rule.channels.length > 0 ? rule.channels : this.config.defaultChannels
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        console.error(`[AlertEngine] Failed to send to ${channelNames[index]}:`, result.reason)
      }
    })
  }
  ...
  private startEscalation(alert: Alert, rule: AlertRule): void {
    const policy = this.config.escalationPolicies.find(p => p.priority === rule.priority)
   ...
   * Send escalation notification
   */
  private async sendEscalation(alert: Alert, step: EscalationStep): Promise<void> {
   ...
   */
  updateTrendData(metric: string, value: number, timestamp: number = Date.now()): void {
