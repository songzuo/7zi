...
describe('AgentScheduler', () => {
  ...
  describe('Agent Registry', () => {
    it('should register a new agent successfully', () => {
      const agent = scheduler.registerAgent('agent1', 'Test Agent', 'processor', ['process', 'analyze']);
    ...
    it('should register agent with metadata', () => {
      ...
      const agent = scheduler.registerAgent('agent2', 'Agent with Meta', 'worker', ['task'], metadata);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle task with all priority levels', () => {
      scheduler.registerAgent('agent1', 'Test Agent', 'processor', ['process', 'analyze', 'transform', 'validate']);
      
      const priorities: Array<'low' | 'normal' | 'high' | 'urgent'> = ['low', 'normal', 'high', 'urgent'];
      
      priorities.forEach((priority) => {
        const response = scheduler.scheduleTask({
          type: priority === 'low' || priority === 'normal' ? 'process' : 
                priority === 'high' ? 'analyze' : 'transform',
          input: { data: priority },
          priority,
        });
        expect(response.success).toBe(true);
