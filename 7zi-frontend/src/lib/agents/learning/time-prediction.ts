...
import type { TaskType, AgentId, PredictionResult, TaskFeatures } from './types'
...
export class TaskTimePredictor {
  ...
  private weightedMovingAverage(history: TimeRecord[]): number {
    ...
    const weightedSum = history.reduce(
      (sum, record, i) => sum + record.executionTime * weights[i],
      0
    )
  ...
  private updatePriorKnowledge(taskType: TaskType, executionTime: number): void {
    ...
    if (!prior) {
      ...
      const newVariance =
        (prior.variance * prior.sampleSize + Math.pow(executionTime - prior.meanTime, 2)) /
        (prior.sampleSize + 1)
