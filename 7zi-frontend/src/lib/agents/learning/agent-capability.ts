...
export class AgentCapabilityAssessor {
  ...
  private calculateSpeedScore(
    ...
    const score = Math.max(0, Math.min(100, 100 - ((avgCompletionTime - baseline / 2) / (baseline * 1.5)) * 100));
  ...
  private detectChanges(
    ...
    for (const [taskType, capScore] of agentScores ? Array.from(agentScores.entries()) : []) {
      ...
      const recentSuccessRate = recentHistory.filter(h => h.status === 'completed').length / recentHistory.length;
      const previousSuccessRate = previousHistory.filter(h => h.status === 'completed').length / previousHistory.length;
  ...
  private generateRecommendations(
    ...
    if (dimensions.technical.score < 50) {
      recommendations.push(`技术能力较弱，建议培训：${dimensions.technical.weakTaskTypes.slice(0, 2).join(', ')}`);
    }
  ...
  private updateCapabilityScore(record: TaskHistoryRecord): void {
    ...
    capScore.avgCompletionTime =
      (capScore.avgCompletionTime * (capScore.sampleCount - 1) + record.executionTime) / capScore.sampleCount;
