...
export class AdaptiveLearner {
  ...
  recordTaskCompletion(
    ...
    const record: TaskHistoryRecord = {
      taskId, taskType, agentId, createdAt, startedAt, completedAt,
      queueWaitTime: startedAt - createdAt,
      executionTime: completedAt - startedAt, status, outputSize, errorType,
      retryCount, priority, inputSize, agentLoadAtStart,
    };
  ...
  private updateAgentStats(record: TaskHistoryRecord): void {
    ...
    if (!stats) {
      stats = {
        agentId, agentName: agentId, capabilityScores: new Map(),
        overallScore: 0.5, reliabilityScore: 0.5, speedScore: 0.5, qualityScore: 0.5,
        currentLoad: 0, avgResponseTime: executionTime, successRate: 1.0,
        lastUpdated: Date.now(), totalTasksCompleted: 0, totalTasksFailed: 0,
      };
    ...
    if (!capScore) {
      capScore = {
        taskType, avgCompletionTime: executionTime, successRate: 1.0,
        sampleCount: 0, lastTaskTime: record.completedAt, trend: 'stable',
      };
      stats.capabilityScores.set(taskType, capScore);
    }
    capScore.sampleCount++;
    capScore.avgCompletionTime = (capScore.avgCompletionTime * (capScore.sampleCount - 1) + executionTime) / capScore.sampleCount;
    capScore.lastTaskTime = record.completedAt;
    stats.totalTasksCompleted++;
    const recent = this.taskHistory.filter(h => h.agentId === agentId && h.completedAt > Date.now() - 86400000);
    stats.successRate = recent.length > 0 ? recent.filter(h => h.status === 'completed').length / recent.length : 1.0;
    stats.avgResponseTime = (stats.avgResponseTime * (stats.totalTasksCompleted - 1) + executionTime) / stats.totalTasksCompleted;
    this.recalculateAgentScores(stats);
    stats.lastUpdated = Date.now();
  }

  private recalculateAgentScores(stats: AgentLearningStats): void {
    const recent = this.taskHistory.filter(h => h.agentId === stats.agentId && h.completedAt > Date.now() - 7 * 86400000);
    ...
    stats.overallScore = stats.reliabilityScore * 0.4 + stats.speedScore * 0.3 + stats.qualityScore * 0.3;
  }
  ...
  predictCompletionTime(features: TaskFeatures): PredictionResult {
    ...
    return { estimatedTime: Math.round(Math.max(100, Math.min(3600000, estimatedTime))), confidence: Math.round(confidence * 100) / 100, factors };
  }
  ...
  adjustWeight(adjustment: WeightAdjustment): void {
    ...
    if (!capScore) {
      capScore = { taskType: adjustment.taskType, avgCompletionTime: 1000, successRate: 0.5, sampleCount: 0, lastTaskTime: Date.now(), trend: 'stable' };
  ...
  getSystemStats(): LearningSystemStats {
    ...
    return {
      ...
      activeAgents: Array.from(this.agentStats.values()).filter(s => Date.now() - s.lastUpdated < 3600000).length,
      totalTasksProcessed: total,
      avgCompletionTime: completed.length > 0 ? completed.reduce((s, h) => s + h.executionTime, 0) / completed.length : 0,
  ...
  getAggregatedStats(period: 'hour' | 'day' | 'week' | 'month'): AggregatedStats {
    const ms: Record<string, number> = { hour: 3600000, day: 86400000, week: 604800000, month: 2592000000 };
    const now = Date.now();
    ...
    return {
      period, startTime: start, endTime: now,
      tasksCompleted: completed.length,
      tasksFailed: relevant.filter(h => h.status === 'failed').length,
      avgExecutionTime: completed.length > 0 ? completed.reduce((s, h) => s + h.executionTime, 0) / completed.length : 0,
      avgQueueWaitTime: relevant.length > 0 ? relevant.reduce((s, h) => s + h.queueWaitTime, 0) / relevant.length : 0,
      avgAgentUtilization: 0,
      topPerformers: Array.from(this.agentStats.values()).sort((a, b) => b.overallScore - a.overallScore).slice(0, 3).map(s => s.agentId),
      strugglingAgents: Array.from(this.agentStats.values()).filter(s => s.totalTasksCompleted >= 5).sort((a, b) => a.overallScore - b.overallScore).slice(0, 3).map(s => s.agentId),
  ...
  exportData() {
    return {
      agentStats: Object.fromEntries(Array.from(this.agentStats.entries()).map(([id, s]) => [id, { ...s, capabilityScores: Object.fromEntries(s.capabilityScores) }])),
