...
export class DatabaseTracker {
  ...
  private normalizeQuery(query: string): string {
    return this.sanitizeQuery(query).toLowerCase().replace(/\s+/g, ' ').trim()
  }
  ...
  private detectIssue(query: SlowQuery): QueryIssue | null {
    ...
    if (query.duration > 5000 && query.rowCount > 10000) {
      return {
        ...
        severity: 'critical',
        suggestion:
          'Large result set detected. Implement pagination, add WHERE clauses, or consider batching',
        query: query.query,
      }
    }
    ...
    if (queryLower.includes('select *') || queryLower.includes('select\t*')) {
      return {
        ...
        severity: 'high',
        suggestion:
          'Avoid SELECT * - specify only required columns to reduce data transfer and improve query performance',
        query: query.query,
      }
    }
    ...
    if (queryLower.includes('where') && queryLower.includes('in (select')) {
      return {
        ...
        severity: 'medium',
        suggestion:
          'Potential N+1 query pattern. Consider using JOINs or batch loading instead of subqueries',
        query: query.query,
      }
    }
    ...
    if (queryLower.includes('where') && queryLower.includes('like') && queryLower.includes('%')) {
      return {
        ...
        severity: 'medium',
        suggestion:
          'Leading wildcard in LIKE clause prevents index usage. Consider full-text search or restructuring query',
        query: query.query,
      }
    }
    ...
    if (queryLower.includes('join') && query.duration > 3000) {
      return {
        ...
        severity: 'high',
        suggestion:
          'Slow JOIN operation. Review join conditions, add indexes on join columns, and consider denormalization',
        query: query.query,
      }
    }
  ...
  getSlowestQueries(count: number = 10): SlowQuery[] {
    return [...this.slowQueries].sort((a, b) => b.duration - a.duration).slice(0, count)
  }
    ...
    return {
      ...
      avgDuration:
        this.slowQueries.length > 0
          ...
          : 0,
      maxDuration:
        this.slowQueries.length > 0 ? Math.max(...this.slowQueries.map(q => q.duration)) : 0,
    }
  }
}
