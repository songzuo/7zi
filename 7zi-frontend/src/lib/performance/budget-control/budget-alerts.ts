...
import { BudgetCheckResult, BudgetViolation, PageBudget } from './types'
...
export class BudgetAlertManager {
  ...
  private determineAlertLevel(
    ...
    if (this.config.customAlertRules) {
      const customRule = this.config.customAlertRules.find(rule => rule.metric === violation.metric)
      if (customRule) {
  ...
  async createSummaryAlert(
    ...
    const worstSeverity = violations.some(v => v.violation.severity === 'critical')
      ? 'critical'
