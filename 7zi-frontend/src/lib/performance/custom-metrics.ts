...
export class CustomMetricsTracker {
  ...
  private trackResourceMetrics(): void {
    ...
    Object.entries(resourceStats).forEach(([type, stats]) => {
      monitor.trackCustomMetric(
        `resource_${type}_count`,
        stats.count,
        'count',
        { resourceType: type }
      );
      monitor.trackCustomMetric(
        `resource_${type}_total_size`,
        stats.totalSize,
        'bytes',
        { resourceType: type }
      );
      monitor.trackCustomMetric(
        `resource_${type}_avg_time`,
  ...
  private startMemoryTracking(): void {
    ...
    const checkMemory = () => {
      ...
      if (performance.memory) {
        ...
        this.metrics.memoryUsagePercent = (this.metrics.memoryUsage / this.metrics.memoryLimit) * 100;
        ...
        if (this.metrics.memoryUsagePercent > 90) {
          monitor.trackError('HighMemoryUsage', `Memory usage: ${this.metrics.memoryUsagePercent.toFixed(1)}%`, undefined, {
    ...
    // 定期检查
    this.memoryCheckIntervalId = setInterval(checkMemory, this.config.memoryCheckInterval) as NodeJS.Timeout;
  }
  ...
  trackWebSocketLatency(ws: WebSocket): void {
    ...
    ws.addEventListener('message', () => {
      ...
      if (timeSinceLastMessage > 1000) {
        ...
        monitor.trackCustomMetric('ws_messages_per_second', this.metrics.wsMessagesPerSecond, 'msg/s');
        ...
        const onMessage = (event: MessageEvent) => {
          try {
            ...
            if (data.type === 'pong' && data.timestamp === pingTime) {
              ...
              // 计算平均延迟
              const avgLatency = this.wsLatencyHistory.reduce((sum, val) => sum + val, 0) / this.wsLatencyHistory.length;
  ...
  async updateErrorMetrics(): Promise<void> {
    ...
    // 计算错误率 (错误数 / 总请求数)
    const totalRequests = metrics.apiMetrics.totalRequests + metrics.operationMetrics.totalOperations;
 ...
 * 初始化自定义指标追踪 (便捷函数)
 */
export function initCustomMetricsTracking(config?: Partial<MetricsTrackerConfig>): CustomMetricsTracker {
  if (config) {
