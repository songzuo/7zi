...
describe('BaselineManager', () => {
  ...
  beforeEach(() => {
    manager = new BaselineManager({
      enabled: true,
      algorithms: { zScore: { enabled: true, threshold: 3 }, isolationForest: { enabled: false, contamination: 0.1 }, threshold: { enabled: true } },
...
describe('RootCauseAnalyzer', () => {
  ...
  it('should analyze slow database queries', () => {
    const context = {
      timestamp: Date.now(),
      slowQueries: [
        { query: 'SELECT * FROM users', duration: 2000, rowCount: 1000, type: 'SELECT' as const, timestamp: Date.now() },
        { query: 'SELECT * FROM orders', duration: 3000, rowCount: 5000, type: 'SELECT' as const, timestamp: Date.now() },
      ],
    };
  ...
  it('should analyze slow API calls', () => {
    const context = {
      timestamp: Date.now(),
      slowApis: [
        { endpoint: '/api/users', method: 'GET', duration: 3000, statusCode: 200, timestamp: Date.now() },
        { endpoint: '/api/orders', method: 'POST', duration: 4000, statusCode: 500, timestamp: Date.now(), error: 'Timeout' },
      ],
    };
