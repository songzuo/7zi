 ...
 * 手机号验证（中国大陆，使用共享模式）
 */
export const phoneNumberSchema = z.string().regex(PATTERNS.phoneCN, '手机号格式无效')
 ...
 * IP 地址验证（IPv4，使用共享模式）
 */
export const ipv4Schema = z.string().regex(PATTERNS.ipv4, 'IPv4 地址格式无效')
...
export function sanitizeInput(
  input: unknown,
  type: 'sql' | 'nosql' | 'html' | 'command' | 'general'
): unknown {
