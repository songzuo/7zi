...
describe('useWebRTCMeeting', () => {
  ...
  describe('initialization', () => {
    ...
    it('should initialize socket with correct config', () => {
      ...
      expect(io).toHaveBeenCalledWith('/api/ws', expect.objectContaining({
  ...
  describe('joinMeeting', () => {
    ...
    it('should handle join-room event', async () => {
      ...
      const joinRoomCallback = mockSocket.on.mock.calls.find((call: any[]) => call[0] === 'join-room')?.[1];
      if (joinRoomCallback) {
        act(() => {
          joinRoomCallback({
            roomId: 'test-room',
  ...
  describe('leaveMeeting', () => {
    ...
    it('should cleanup local stream on leave', async () => {
      const { result } = renderHook(() => useWebRTCMeeting(defaultOptions));
  ...
  describe('toggleMute', () => {
    ...
    it('should emit mute-state event', async () => {
      const { result } = renderHook(() => useWebRTCMeeting(defaultOptions));
  ...
  describe('participant management', () => {
    it('should add participant on participant-joined event', async () => {
      ...
      const joinCallback = mockSocket.on.mock.calls.find((call: any[]) => call[0] === 'participant-joined')?.[1];
      if (joinCallback) {
        act(() => {
          joinCallback({
            id: 'user-456',
    ...
    it('should remove participant on participant-left event', async () => {
      ...
      // Remove participant
      const leaveCallback = mockSocket.on.mock.calls.find((call: any[]) => call[0] === 'participant-left')?.[1];
      if (leaveCallback) {
        act(() => {
    ...
    it('should update participant mute state on participant-muted event', async () => {
      ...
      const muteCallback = mockSocket.on.mock.calls.find((call: any[]) => call[0] === 'participant-muted')?.[1];
      if (muteCallback) {
        act(() => {
  ...
  describe('remote streams', () => {
    it('should add remote stream on track event', async () => {
      const { result } = renderHook(() => useWebRTCMeeting(defaultOptions));
      ...
      const pcCallback = mockSocket.on.mock.calls.find((call: any[]) => call[0] === 'webrtc-offer')?.[1];
  ...
  describe('audio elements', () => {
    it('should return null for non-existent audio element', async () => {
      const { result } = renderHook(() => useWebRTCMeeting(defaultOptions));
