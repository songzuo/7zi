...
describe('useDashboardData', () => {
  ...
  describe('基本功能', () => {
    ...
    it('应该成功获取 Issues 和 Commits', async () => {
      ...
      const mockCommits = [createMockCommit({ sha: 'abc123' }), createMockCommit({ sha: 'def456' })]
    ...
    it('应该过滤掉 Pull Requests', async () => {
      ...
      const mockIssues = [createMockIssue({ number: 1, title: 'Real Issue' })]
  ...
  describe('Token 支持', () => {
    it('应该调用内部 API 代理端点', async () => {
      ...
      const { result } = renderHook(() => useDashboardData('owner', 'repo', 'test-token'))
  ...
  describe('错误处理', () => {
    it('应该处理 404 错误（仓库不存在）', async () => {
      ...
      const { result } = renderHook(() => useDashboardData('nonexistent', 'repo'))
    ...
    it('应该处理 401 错误（Token 无效）', async () => {
      ...
      const { result } = renderHook(() => useDashboardData('owner', 'repo', 'invalid-token'))
  ...
  describe('活动格式化', () => {
    ...
    it('应该限制活动数量为 20 条', async () => {
      // 创建超过 20 条记录
      const mockIssues = Array.from({ length: 15 }, (_, i) => createMockIssue({ number: i + 1 }))
      const mockCommits = Array.from({ length: 15 }, (_, i) => createMockCommit({ sha: `sha${i}` }))
  ...
  describe('边界情况', () => {
    ...
    it('应该处理 Issues 获取失败但 Commits 成功', async () => {
      ...
      mockFetch.mockRejectedValueOnce(new Error('Issues failed')).mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve(mockCommits),
      })
