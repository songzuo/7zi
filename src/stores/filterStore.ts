...
export const useSearchActions = (namespace: string) => ({
  setSearchQuery: (query: string) => useFilterStore.getState().setSearchQuery(namespace, query),
  clearSearchQuery: () => useFilterStore.getState().clearSearchQuery(namespace),
})
...
export const useSortActions = (namespace: string) => ({
  setSort: (sort: SortCondition | null) => useFilterStore.getState().setSort(namespace, sort),
  toggleSort: (field: string) => useFilterStore.getState().toggleSort(namespace, field),
  clearSort: () => useFilterStore.getState().clearSort(namespace),
})
...
export const usePaginationActions = (namespace: string) => ({
  ...
  setPage: (page: number) => useFilterStore.getState().setPage(namespace, page),
  setPageSize: (pageSize: number) => useFilterStore.getState().setPageSize(namespace, pageSize),
  setTotal: (total: number) => useFilterStore.getState().setTotal(namespace, total),
})
...
export const useHasActiveFilters = (namespace: string) =>
  useFilterStore(state => {
    ...
    return filters && (filters.conditions.length > 0 || filters.searchQuery.trim().length > 0)
  })
