...
export function createMockFetch(): MockFetchImplementation {
  ...
  const mockFetch: MockFetchImplementation = async (
    ...
    const callback = responseCallbacks.get(url.pathname) ||
      responseCallbacks.get(urlStr);
    if (callback) {
      try {
...
export function setupCommonApiMocks(
  mockFn: MockFetchImplementation = mockFetch,
): void {
  // Health check
  ...
  mockFn.__mockResponseCallback("*", (req: MockFetchRequest) => {
    if (!mockFn.__mockResponses.has(req.url) &&
        !mockFn.__responseCallbacks.has(req.url)) {
 ...
 */
export function getLastFetchRequest(
  mockFn: MockFetchImplementation,
): MockFetchRequest | undefined {
  const calls = mockFn.__getMockedCalls();
...
export function createMockErrorResponse(
  ...
  return createMockResponse(
    { error: message || errorData.statusText },
    errorData,
  );
}
 ...
 */
export function setupDelayedFetch(
  delayMs: number,
