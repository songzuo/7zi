...
import { type User, type UserContext, type UserToken, UserRole } from '@/lib/auth/types'
...
export interface MockAuthContextValue {
  ...
  register: MockedFunction<(email: string, password: string, name: string) => Promise<void>>
...
export interface MockAuthService {
  ...
  register: MockedFunction<
    (
      ...
      password: string,
      name: string
    ) => Promise<{ user: MockUser; token: MockUserToken }>
  >
  ...
  changePassword: MockedFunction<(currentPassword: string, newPassword: string) => Promise<void>>
 ...
 */
export function checkMockPermission(user: MockUser, permission: string): boolean {
  return user.permissions.includes(permission) || false
}
...
export function createMockUsers(count: number): MockUser[] {
  return Array.from({ length: count }, (_, i) =>
    createMockUser({
      ...
      permissions: i === 0 ? ['read:all', 'write:all', 'delete:all'] : ['read:own', 'update:own'],
    })
  )
}
 ...
 */
export function createMockAuthState(user: MockUser | null = DEFAULT_MOCK_USER) {
  return {
    user,
