...
export interface MockAxiosInstance {
  ...
  post: MockedFunction<
    (url: string, data?: unknown, config?: Record<string, unknown>) => Promise<MockAxiosResponse>
  >
  put: MockedFunction<
    (url: string, data?: unknown, config?: Record<string, unknown>) => Promise<MockAxiosResponse>
  >
  ...
  delete: MockedFunction<
    (url: string, config?: Record<string, unknown>) => Promise<MockAxiosResponse>
  >
...
export function createMockAxios(
  responses: Map<string, MockAxiosResponse> = new Map()
): MockAxiosInstance {
  const instance = {
...
export const DEFAULT_AXIOS_RESPONSES = new Map<string, MockAxiosResponse>([
  ...
  [
    '/api/user',
    createAxiosResponse({ id: 'user-1', email: 'test@example.com', name: 'Test User' }),
  ],
])
