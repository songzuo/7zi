...
describe('useIntersectionObserver', () => {
  ...
  it('updates isIntersecting when element intersects', async () => {
    ...
    await waitFor(() => expect(mockObserve).toHaveBeenCalled(), { timeout: 1000 })
...
describe('useAnimateOnView', () => {
  ...
  it('accepts custom animation class parameter', () => {
    const { result } = renderHook(() => useAnimateOnView('custom-animation'))
  ...
  it('applies custom animation class when visible', async () => {
    const { result } = renderHook(() => useAnimateOnView('my-custom-animation'))
