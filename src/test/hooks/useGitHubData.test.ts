...
describe('useGitHubData', () => {
  ...
  it('fetches GitHub data successfully', async () => {
    ...
    ;(global.fetch as MockedFunction<typeof fetch>)
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify(mockCommits), { status: 200, headers: {} })
      ) // Commits
      .mockResolvedValueOnce(
        new Response(JSON.stringify(mockRepoData), { status: 200, headers: {} })
      ) // Stats
  ...
  it('fetches even when owner or repo is empty (hook does not guard)', async () => {
    ...
    ;(global.fetch as MockedFunction<typeof fetch>)
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ stargazers_count: 0, forks_count: 0, open_issues_count: 0 }),
          { status: 200, headers: {} }
        )
      )
  ...
  it('sets up auto-refresh interval', async () => {
    ...
    expect((global.fetch as MockedFunction<typeof fetch>).mock.calls.length).toBeGreaterThan(
      initialCallCount
    )
  })
