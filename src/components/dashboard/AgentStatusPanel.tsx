...
function getAgentCurrentTask(agentId: string, tasks: Task[]): Task | null {
  return tasks.find(task => task.assignedAgent === agentId && task.status === 'in_progress') || null
}
...
const AgentCard: FC<AgentCardProps> = ({ agentInfo, onClick, showMetrics = false }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative overflow-hidden rounded-xl border bg-white p-4 dark:bg-zinc-800/50 ${statusConfig.border} transition-all duration-300 hover:scale-[1.02] hover:shadow-lg ${onClick ? 'cursor-pointer' : 'cursor-default'} `}
    >
      {/* 顶部状态指示条 */}
      <div className={`absolute top-0 right-0 left-0 h-1 ${statusConfig.bg}`} />
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          {/* Agent 图标 */}
          <div
            className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100 dark:border-blue-700/30 dark:from-blue-900/30 dark:to-blue-800/20`}
          >
        ...
        <div className="flex flex-shrink-0 items-center gap-2">
          <span className={`h-2.5 w-2.5 rounded-full ${statusConfig.indicator}`} />
      ...
      {/* 当前任务 */}
      {currentTask && (
        <div className="mt-3 rounded-lg bg-zinc-50 p-2 dark:bg-zinc-700/30">
          <div className="flex items-center gap-2 text-sm">
            <Activity className="h-3.5 w-3.5 text-green-500" />
            <span className="truncate text-zinc-700 dark:text-zinc-300">{currentTask.title}</span>
          </div>
          <div className="mt-1 flex items-center gap-3 text-xs text-zinc-500 dark:text-zinc-400">
            <span>优先级: {currentTask.priority}</span>
            {currentTask.estimatedDuration && <span>预计 {currentTask.estimatedDuration}分钟</span>}
          </div>
        </div>
      )}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        {/* CPU 使用率 */}
        <div className="flex items-center gap-2 rounded-lg bg-zinc-50 p-2 dark:bg-zinc-700/30">
          ...
          <div className="flex-1">
            ...
            <div className="h-1 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-600">
              <div
      ...
      <div className="mt-3 space-y-2">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-zinc-700 dark:text-zinc-300">{agent.currentLoad}% 负载</span>
          </div>
        </div>
        ...
        {showMetrics && agent.metrics && (
          <div className="mt-3 border-t border-zinc-100 pt-3 dark:border-zinc-700/50">
    ...
    <div className={`${className}`}>
      ...
      <div className="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
          <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">智能体状态</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5">
              {(['all', 'idle', 'busy', 'offline', 'error'] as StatusFilter[]).map(filter => {
                ...
                const count = filter === 'all' ? extendedAgents.length : statusCounts[filter]
                  ...
                  <button
                    ...
                    className={`rounded-full px-2.5 py-1 text-xs font-medium transition-all duration-200 ${
                      statusFilter === filter
                        ...
                        : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-700/50 dark:text-zinc-400 dark:hover:bg-zinc-600/50'
                    } `}
                  >
                    {config.label} ({count})
                  </button>
      ...
      {maxDisplay && filteredAgents.length > maxDisplay && (
        <div className="mt-4 text-center">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            显示前 {maxDisplay} 个智能体，当前筛选共 {filteredAgents.length} 个，总计{' '}
            {agents.length} 个
          </p>
        </div>
      )}
      ...
      {agents.length > 0 && (
        <div className="mt-4 border-t border-zinc-100 pt-4 dark:border-zinc-700/50">
          <div className="flex items-center justify-between text-sm">
            <span className="text-zinc-500 dark:text-zinc-400">整体可用率</span>
            <div className="flex items-center gap-2">
              <div className="h-2 w-24 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-700">
                <div
