...
const ActivityItemComponent: React.FC<ActivityItemComponentProps> = React.memo(
  ({ activity, locale, variant, onClick }) => {
    ...
    const displayDescription = locale === 'en' && activity.descriptionEn 
    ...
    if (variant === 'minimal') {
      return (
        <div 
          className="flex items-center gap-3 py-2 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 rounded-lg cursor-pointer transition-colors"
          onClick={onClick}
        >
    ...
    if (variant === 'compact') {
      return (
        <div 
          className={cn(
            'flex items-start gap-3 p-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 rounded-lg cursor-pointer transition-colors',
            'border-b border-zinc-100 dark:border-zinc-700 last:border-0'
          )}
          onClick={onClick}
        >
          <div className={cn('p-2 rounded-lg bg-zinc-100 dark:bg-zinc-800', config.color)}>
          ...
          <span className="text-xs text-zinc-400 dark:text-zinc-500 flex-shrink-0 whitespace-nowrap">
            {timeLabel}
          </span>
        </div>
      );
    ...
    return (
      <div 
        className={cn(
          'flex items-start gap-4 p-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 rounded-lg cursor-pointer transition-colors',
          'border-b border-zinc-100 dark:border-zinc-700 last:border-0'
        )}
        onClick={onClick}
      >
        <div className={cn('p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-800', config.color)}>
...
const ActivitySkeleton: React.FC<{ count: number }> = ({ count }) => (
  <div className="space-y-3">
    {[...Array(count)].map((_, i) => (
      ...
      >
        <div className="w-10 h-10 bg-zinc-200 dark:bg-zinc-700 rounded-xl flex-shrink-0" />
        <div className="flex-1 space-y-2">
          <div className="w-3/4 h-4 bg-zinc-200 dark:bg-zinc-700 rounded" />
