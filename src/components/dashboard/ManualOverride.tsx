...
const ConfirmDialog: FC<ConfirmDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div
        className={`w-full max-w-md rounded-xl border-2 bg-white dark:bg-zinc-800 ${styles.border} shadow-2xl`}
      >
        <div className="p-6">
          <div className="mb-4 flex items-start gap-3">
            <AlertTriangle className={`mt-0.5 h-5 w-5 ${styles.icon}`} />
          ...
          <div className="mt-6 flex items-center justify-end gap-3">
            <button
              onClick={onCancel}
              className="rounded-lg bg-zinc-100 px-4 py-2 text-sm font-medium text-zinc-700 transition-colors duration-200 hover:bg-zinc-200 dark:bg-zinc-700/50 dark:text-zinc-300 dark:hover:bg-zinc-600/50"
            >
              {cancelText}
            </button>
            <button
              onClick={onConfirm}
              className={`rounded-lg px-4 py-2 text-sm font-medium text-white ${styles.confirmBtn} transition-colors duration-200`}
            >
              {confirmText}
            </button>
          </div>
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        <Clock className="mb-2 h-10 w-10 text-zinc-300 dark:text-zinc-600" />
        <p className="text-sm text-zinc-500 dark:text-zinc-400">暂无待处理任务</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      {tasks.map(task => (
        <div
          key={task.id}
          className={`rounded-lg border border-zinc-200 bg-zinc-50 p-3 transition-all duration-200 hover:bg-zinc-100 dark:border-zinc-700/50 dark:bg-zinc-800/30 dark:hover:bg-zinc-800/50`}
        >
          <div className="flex items-start justify-between gap-2">
            ...
            <button
              ...
              disabled={isLoading}
              className={`rounded-lg bg-red-50 p-1.5 text-red-600 transition-colors duration-200 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30`}
              title="取消任务"
            >
...
const SchedulePreviewCard: FC<SchedulePreviewCardProps> = ({ preview, priority }) => {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/30">
...
export const ManualOverride: FC<ManualOverrideProps> = ({
  ...
  const schedulePreview = useCallback((): SchedulePreview => {
    ...
    const estimatedStartTime =
      formData.executionMode === 'immediate' ? now : formData.scheduledTime || now
  ...
  const getSuitableAgents = () => {
    return agents.filter(agent => agent.capabilities.taskTypes.includes(formData.type))
  }
  ...
  return (
    <div className={className}>
      ...
      {/* 成功消息 */}
      {successMessage && (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-800/30 dark:bg-green-900/20">
      ...
      {/* 错误消息 */}
      {errors._form && (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-800/30 dark:bg-red-900/20">
          <XCircle className="h-4 w-4 text-red-600 dark:text-red-400" />
          <span className="text-sm text-red-700 dark:text-red-300">{errors._form}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">手动创建任务</h3>
          </div>

          {/* 任务标题 */}
          <FormField label="任务标题" required error={errors.title} helpText="简要描述任务内容">
            <input
              type="text"
              value={formData.title}
              onChange={e => handleInputChange('title', e.target.value)}
              placeholder="例如：优化首页加载性能"
              className={`w-full rounded-lg border bg-white px-4 py-2.5 dark:bg-zinc-800 ${errors.title ? 'border-red-300 dark:border-red-700' : 'border-zinc-200 dark:border-zinc-700'} text-zinc-900 placeholder-zinc-400 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:text-zinc-100 dark:placeholder-zinc-500`}
            />
          </FormField>
          ...
          <FormField label="任务描述" error={errors.description}>
            <textarea
              ...
              rows={3}
              className={`w-full rounded-lg border bg-white px-4 py-2.5 dark:bg-zinc-800 ${errors.description ? 'border-red-300 dark:border-red-700' : 'border-zinc-200 dark:border-zinc-700'} resize-none text-zinc-900 placeholder-zinc-400 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:text-zinc-100 dark:placeholder-zinc-500`}
            />
          </FormField>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <FormField label="任务类型" required error={errors.type}>
              <select
                ...
                className={`w-full rounded-lg border bg-white px-4 py-2.5 dark:bg-zinc-800 ${errors.type ? 'border-red-300 dark:border-red-700' : 'border-zinc-200 dark:border-zinc-700'} text-zinc-900 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:text-zinc-100`}
              >
            ...
            >
              <select
                ...
                className={`w-full rounded-lg border bg-white px-4 py-2.5 dark:bg-zinc-800 ${errors.agentId ? 'border-red-300 dark:border-red-700' : 'border-zinc-200 dark:border-zinc-700'} text-zinc-900 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:text-zinc-100`}
              >
          ...
          {/* 优先级和执行时间 */}
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <FormField label="优先级" required helpText="高优先级任务会影响现有调度">
              <select
                value={formData.priority}
                onChange={e => handleInputChange('priority', e.target.value as TaskPriority)}
                className={`w-full rounded-lg border border-zinc-200 bg-white px-4 py-2.5 text-zinc-900 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100`}
              >
            ...
            <FormField label="执行时间" error={errors.executionMode}>
              <select
                value={formData.executionMode}
                onChange={e =>
                  handleInputChange('executionMode', e.target.value as 'immediate' | 'scheduled')
                }
                className={`w-full rounded-lg border border-zinc-200 bg-white px-4 py-2.5 text-zinc-900 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100`}
              >
              ...
              <input
                ...
                value={
                  formData.scheduledTime
                    ? new Date(formData.scheduledTime).toISOString().slice(0, 16)
                    : ''
                }
                onChange={e => {
                ...
                className={`w-full rounded-lg border bg-white px-4 py-2.5 dark:bg-zinc-800 ${errors.scheduledTime ? 'border-red-300 dark:border-red-700' : 'border-zinc-200 dark:border-zinc-700'} text-zinc-900 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:text-zinc-100`}
              />
            </FormField>
          )}

          {/* 估计时长 */}
          <FormField label="估计时长（分钟）" required error={errors.estimatedDuration}>
            <input
              type="number"
              ...
              min={1}
              max={480}
              className={`w-full rounded-lg border bg-white px-4 py-2.5 dark:bg-zinc-800 ${errors.estimatedDuration ? 'border-red-300 dark:border-red-700' : 'border-zinc-200 dark:border-zinc-700'} text-zinc-900 transition-colors duration-200 focus:ring-2 focus:ring-blue-500/50 focus:outline-none dark:text-zinc-100`}
            />
          </FormField>
          ...
          <button
            ...
            className={`flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-3 font-medium text-white transition-all duration-200 hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-blue-500 dark:hover:bg-blue-600`}
          >
            {isSubmitting ? (
              <>
        ...
        <div className="space-y-6">
          ...
          {/* 统计信息 */}
          <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/30">
            <h4 className="mb-3 flex items-center gap-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">
              <RefreshCw className="h-4 w-4" />
              当前状态
            </h4>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg bg-white p-2 text-center dark:bg-zinc-800/50">
