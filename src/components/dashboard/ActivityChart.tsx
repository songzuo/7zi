...
const colors = {
  agents: { stroke: '#3b82f6', fill: '#3b82f6', fillOpacity: 0.3, icon: Zap, label: 'Active Agents' },
  users: { stroke: '#10b981', fill: '#10b981', fillOpacity: 0.3, icon: Users, label: 'Active Users' },
  tokens: { stroke: '#f59e0b', fill: '#f59e0b', fillOpacity: 0.3, icon: Cpu, label: 'Tokens Used' },
  requests: { stroke: '#8b5cf6', fill: '#8b5cf6', fillOpacity: 0.3, icon: Activity, label: 'Requests' },
...
export const ActivityChart: React.FC<ActivityChartProps> = ({
  ...
  // 计算统计信息
  const stats = metrics.reduce<Record<string, { total: number; avg: number; max: number }>>((acc, metric) => {
  ...
  const chartData = data.map(item => ({
    ...item,
    displayDate: item.date || new Date(item.timestamp).toLocaleDateString(locale, {
      month: 'short',
      day: 'numeric'
    })
  }));
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {metrics.slice(0, 4).map((metric) => {
          ...
          return (
            <div
              key={metric}
              className={`p-3 bg-${metric === 'errors' ? 'red' : metric === 'agents' ? 'blue' : metric === 'users' ? 'green' : metric === 'tokens' ? 'orange' : 'purple'}-50 dark:bg-${metric === 'errors' ? 'red' : metric === 'agents' ? 'blue' : metric === 'users' ? 'green' : metric === 'tokens' ? 'orange' : 'purple'}-900/20 rounded-lg cursor-pointer transition-all hover:scale-[1.02] ${
                selectedMetric === metric ? 'ring-2 ring-offset-2 ring-blue-500' : ''
              }`}
