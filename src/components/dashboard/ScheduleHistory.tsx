 ...
 * 根据时间范围筛选
 */
function filterByTimeRange(entries: HistoryEntry[], timeRange: TimeRange, startTime?: number, endTime?: number): HistoryEntry[] {
  const now = Date.now();
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          {/* 状态徽章 */}
          <div className={`px-2.5 py-1 rounded-lg text-xs font-medium flex items-center gap-1.5 ${statusConfig.badge}`}>
        ...
        {entry.description && !expanded && (
          <p className="text-sm text-zinc-600 dark:text-zinc-400 truncate mb-3">
            {entry.description}
          </p>
        )}
      ...
      {/* 任务 ID */}
      <div className="px-4 pb-3 pt-0 border-t border-zinc-100 dark:border-zinc-700/50">
...
export const ScheduleHistory: FC<ScheduleHistoryProps> = ({
  ...
  const statsInfo = useMemo(() => {
    ...
    const avgDuration = filteredEntries
      .filter(e => e.duration)
      .reduce((sum, e) => sum + (e.duration || 0), 0) / (filteredEntries.filter(e => e.duration).length || 1);
    ...
    <div className={`${className}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 rounded-lg bg-zinc-100 dark:bg-zinc-700/50 border border-transparent hover:bg-zinc-200 dark:hover:bg-zinc-600/50 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 w-40 sm:w-56"
            />
          </div>
      ...
      {filteredEntries.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <Clock className="w-12 h-12 text-zinc-300 dark:text-zinc-600 mb-3" />
      ...
      {filteredEntries.length > 0 && totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between">
          <div className="text-sm text-zinc-500 dark:text-zinc-400">
            显示 {(currentPage - 1) * pageSize + 1} - {Math.min(currentPage * pageSize, filteredEntries.length)} 条，共 {filteredEntries.length} 条
          </div>
