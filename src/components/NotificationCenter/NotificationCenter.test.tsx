...
describe('NotificationCenter', () => {
  ...
  it('应该显示未读徽章数量', () => {
    render(<NotificationCenter notifications={mockNotifications} showUnreadBadge={true} />)
  ...
  it('没有未读通知时不显示徽章', () => {
    ...
    render(<NotificationCenter notifications={readNotifications} showUnreadBadge={true} />)
  ...
  it('没有通知时显示空状态', async () => {
    render(<NotificationCenter notifications={[]} onClearAll={mockOnClearAll} />)
  ...
  it('点击全部已读应该调用回调', async () => {
    render(
      <NotificationCenter notifications={mockNotifications} onMarkAllAsRead={mockOnMarkAllAsRead} />
    )
  ...
  it('点击清空应该调用回调', async () => {
    render(<NotificationCenter notifications={mockNotifications} onClearAll={mockOnClearAll} />)
  ...
  it('应该限制显示的通知数量', async () => {
    const manyNotifications: Notification[] = Array.from({ length: 15 }, (_, i) => ({
      id: `notification-${i}`,
      title: `通知 ${i}`,
      message: `消息 ${i}`,
      type: 'info' as const,
    ...
    render(<NotificationCenter notifications={manyNotifications} maxVisible={10} />)
