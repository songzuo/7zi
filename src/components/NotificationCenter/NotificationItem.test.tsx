...
describe('NotificationItem', () => {
  ...
  it('点击标记已读按钮应该调用回调', () => {
    render(<NotificationItem notification={mockNotification} onMarkAsRead={mockOnMarkAsRead} />)
  ...
  it('点击删除按钮应该调用回调', () => {
    render(<NotificationItem notification={mockNotification} onDelete={mockOnDelete} />)
  ...
  it('不同类型应该显示不同的图标样式', () => {
    ...
    rerender(<NotificationItem notification={{ ...mockNotification, type: 'warning' }} />)
    expect(document.querySelector('.bg-yellow-50')).toBeInTheDocument()

    rerender(<NotificationItem notification={{ ...mockNotification, type: 'error' }} />)
    expect(document.querySelector('.bg-red-50')).toBeInTheDocument()

    rerender(<NotificationItem notification={{ ...mockNotification, type: 'info' }} />)
  ...
  it('支持自定义图标', () => {
    ...
    render(<NotificationItem notification={customIconNotification} />)
