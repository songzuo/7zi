...
function formatValue(value: number | string, format?: Statistic['format']): string {
...
const MetricCardBase: FC<MetricCardProps> = ({
  ...
  if (loading) {
    return (
      <div
        className={`${sizes.padding} rounded-xl border border-zinc-200 bg-gradient-to-br from-gray-50 to-gray-100 dark:border-zinc-700 dark:from-gray-800/50 dark:to-gray-700/30 ${className}`}
      >
        <div className="animate-pulse space-y-2">
          <div className="h-4 w-1/2 rounded bg-zinc-200 dark:bg-zinc-700"></div>
          <div className="h-8 w-3/4 rounded bg-zinc-200 dark:bg-zinc-700"></div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div
      onClick={onClick}
      className={`relative overflow-hidden ${sizes.padding} bg-gradient-to-br ${config.bg} dark:bg-gradient-to-br ${config.bg.split(' ').slice(-2).join(' ')} rounded-xl border ${config.border} transition-all duration-300 hover:scale-[1.02] hover:shadow-lg active:scale-[0.98] ${onClick ? 'cursor-pointer' : 'cursor-default'} ${className} `}
    >
      {/* Decorative background */}
      <div className="absolute top-0 right-0 -mt-4 -mr-4 h-24 w-24 rounded-bl-full bg-gradient-to-br from-white/20 to-transparent" />

      <div className="relative">
        {/* Top: Title and Icon */}
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h3 className={`${sizes.title} truncate font-medium text-zinc-600 dark:text-zinc-400`}>
              {label}
            </h3>
          </div>
          {Icon && (
            <div
              className={`${sizes.icon} rounded-xl ${config.icon} flex flex-shrink-0 items-center justify-center`}
            >
        ...
        {/* Middle: Value */}
        <div className={`mt-3 ${sizes.value} ${config.text} truncate`}>{displayValue}</div>
        ...
        {change && (
          <div className="mt-2 flex items-center gap-2">
            ...
            {change.period && (
              <span className="text-xs text-zinc-500 dark:text-zinc-400">({change.period})</span>
            )}
          </div>
        )}
      </div>
