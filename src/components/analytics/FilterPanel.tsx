        ...
        <div className="space-y-4 p-4">
          {FILTER_SECTIONS.map(section => {
            ...
            return (
              <div
                key={section.id}
                className="border-b border-zinc-100 pb-4 last:border-0 last:pb-0 dark:border-zinc-700/50"
              >
                <button
                  onClick={() => toggleSection(section.id)}
                  className="flex w-full items-center justify-between text-sm font-medium text-zinc-700 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white"
                >
                  {sectionTitle}
                  ...
                  <div className="mt-3 space-y-2">
                    {section.options.map(option => {
                      ...
                      return (
                        <label
                          key={option.value}
                          className="flex cursor-pointer items-center gap-3 rounded-lg p-2 hover:bg-zinc-50 dark:hover:bg-zinc-700/50"
                        >
                          <input
                            type="checkbox"
                            checked={isChecked}
                            onChange={() =>
                              handleCheckboxChange(
                                section.id as keyof AnalyticsFilters,
                                option.value
                              )
                            }
                          ...
                          {isChecked && (
                            <X
                              className="ml-auto h-4 w-4 cursor-pointer text-blue-600 hover:text-blue-700"
                              onClick={() =>
                                handleCheckboxChange(
                                  section.id as keyof AnalyticsFilters,
                                  option.value
