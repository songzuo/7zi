...
const TimingBar: React.FC<TimingBarProps> = ({ timing, totalDuration, locale }) => {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <div className="relative h-6 flex-1 overflow-hidden rounded bg-zinc-100 dark:bg-zinc-700">
        <div
...
const ResourceRow: React.FC<ResourceRowProps> = ({
  ...
  const color =
    RESOURCE_COLORS[resource.initiatorType as keyof typeof RESOURCE_COLORS] || RESOURCE_COLORS.other
  ...
  const IconComponent =
    RESOURCE_ICONS[resource.initiatorType as keyof typeof RESOURCE_ICONS] || RESOURCE_ICONS.other

  return (
    <div className="group flex items-center gap-2 rounded py-1 text-xs hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
      <div className="flex w-8 items-center justify-center" style={{ color }}>
        <IconComponent {...iconProps} />
      </div>
      <div className="min-w-0 flex-1 truncate text-zinc-700 dark:text-zinc-300">
        {resource.name}
      </div>
      {showDetails && (
      ...
      <div className="relative h-5 flex-1 overflow-hidden rounded bg-zinc-100 dark:bg-zinc-700">
        <div
          className="absolute h-full rounded-sm"
          style={{
            left: `${leftPercent}%`,
    ...
    return (
      ...
      >
        <div className="flex h-48 flex-col items-center justify-center text-center">
          <Clock className="mb-2 h-8 w-8 text-zinc-400" />
    ...
    >
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          <h4 className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{t.resources}</h4>
        ...
        {/* Resource header */}
        <div className="mb-2 flex items-center gap-2 px-2 text-xs text-zinc-500 dark:text-zinc-400">
          <div className="w-8" />
          <div className="flex-1">{locale === 'zh' ? '名称' : 'Name'}</div>
          {showDetails && <div className="w-24 text-right">{t.size}</div>}
        ...
        <div className="max-h-64 space-y-0.5 overflow-y-auto">
          {(isExpanded ? resourceTimings : resourceTimings.slice(0, maxResources)).map(
            (resource, index) => (
              <ResourceRow
                key={index}
                resource={resource}
