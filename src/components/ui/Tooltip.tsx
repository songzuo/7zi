...
const POSITION_CONFIG: Record<TooltipPosition, { placement: string; arrow: string }> = {
  top: {
    ...
    arrow:
      'bottom-0 left-1/2 -translate-x-1/2 translate-y-full border-l-transparent border-r-transparent border-b-transparent',
  },
  bottom: {
  ...
  left: {
    ...
    arrow:
      'right-0 top-1/2 -translate-y-1/2 translate-x-full border-t-transparent border-b-transparent border-r-transparent',
  },
  right: {
...
export interface SimpleTooltipProps extends Omit<
  TooltipProps,
  'position' | 'size' | 'showArrow' | 'delay' | 'hideDelay'
> {
  /** Tooltip position (default: 'top') */
  position?: TooltipPosition
}
