  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {toast.action && (
          <button
            ...
            className="mt-2 text-sm font-medium text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 dark:hover:text-cyan-300"
          >
            {toast.action.label}
          </button>
        )}
      ...
      {toast.type !== 'loading' && (
        <button
          onClick={handleClose}
          className="flex-shrink-0 p-1 rounded-lg text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors"
          aria-label="关闭通知"
        >
      ...
      {toast.duration && toast.duration > 0 && toast.type !== 'loading' && (
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-zinc-200 dark:bg-zinc-700 rounded-b-xl overflow-hidden">
          <div
...
export const ToastContainer: FC = memo(() => {
  ...
  return (
    <div
      className="fixed z-50 flex flex-col gap-2 p-4 max-w-sm w-full"
      aria-label="通知"
    >
      {toasts.map((toast) => (
        <ToastItem
...
export const PositionedToastContainer: FC<PositionedToastContainerProps> = memo(({
  ...
  return (
    <div
      className={`fixed z-50 flex flex-col gap-2 p-4 max-w-sm w-full ${POSITION_CLASSES[position]}`}
      aria-label="通知"
    >
      {toasts.map((toast) => (
        <ToastItem
