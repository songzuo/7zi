...
export function TaskEditorCollaboration({
  ...
  useEffect(() => {
    if (collaboration.isConnected && !collaboration.isInRoom && showCollaboration) {
      collaboration.joinRoom(`task:${task.id}`, 'task', String(task.id), task.title)
    }
  ...
  const calculateOperation = (oldContent: string, newContent: string, position: number) => {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {showCollaboration && (
        <div className="flex items-center justify-between border-b bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800/50">
          <div className="flex items-center gap-4">
            <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">{task.title}</h3>
            <span className="text-xs text-zinc-500 dark:text-zinc-400">#{task.number}</span>
          </div>
      ...
      <div className="relative flex-1 overflow-hidden">
        <textarea
          ...
          className="h-full w-full resize-none border-none p-4 font-mono text-sm focus:outline-none dark:bg-zinc-800 dark:text-white"
        ...
        {showCollaboration && collaboration.isConnected && (
          <div className="pointer-events-none absolute inset-0">
            {Array.from(collaboration.cursors.values()).map(cursor => (
              <RemoteCursor key={cursor.userId} cursor={cursor} currentUserId={userId} />
            ))}
          </div>
        )}
      </div>
      ...
      {/* Footer */}
      {showCollaboration && (
        <div className="flex items-center justify-between border-t bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800/50">
            ...
            {collaboration.typingUsers.length > 0 && (
              <TypingIndicator
                typingUsers={collaboration.users
                  .filter(u => collaboration.typingUsers.includes(u.id))
                  .map(u => ({
                    userId: u.id,
                    userName: u.name,
                    color: u.color,
...
export function CollaborationToggle({
  ...
  return (
    ...
    >
      <span
        className={`h-2 w-2 rounded-full ${isCollaborating ? 'bg-green-500' : 'bg-zinc-400'}`}
      />
      <span>{isCollaborating ? `Collaborating (${userCount})` : 'Enable Collaboration'}</span>
    </button>
  )
}
