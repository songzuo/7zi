...
const AnimatedProgressBar = memo(function AnimatedProgressBar({
  ...
  const barStyle = useMemo(() => ({ width: `${displayValue}%` }), [displayValue])
  ...
  return (
    ...
    >
          ...
          {showPercentage && (
            <span
              className={`text-sm font-bold transition-all duration-300 ${
                isComplete ? 'scale-110 text-green-500' : 'text-zinc-900 dark:text-white'
              }`}
            >
              {Math.round(displayValue)}%{isComplete && ' ✓'}
            </span>
          )}
        </div>
      )}
      <div
        className={`w-full overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700 ${sizeClass}`}
      >
        <div
          className={`${sizeClass} ${colorClass} ${striped ? 'bg-stripes' : ''} ${getAnimationClass()} relative overflow-hidden rounded-full transition-all duration-300 ease-out`}
          style={barStyle}
        >
          {/* 光效层 */}
          {animation === 'glow' && (
          ...
          {(animation === 'striped-animated' || striped) && (
            <div className="bg-stripes-animated absolute inset-0 opacity-50" />
          )}
        </div>
      </div>
      ...
      {/* 完成动画 */}
      {isComplete && (
        <div className="animate-fade-in mt-2 text-center">
          <span className="text-sm font-medium text-green-500">🎉 完成!</span>
        </div>
      )}
    </div>
  )
...
export const WaveProgress = memo(function WaveProgress({
  ...
  return (
    <div
      className="relative overflow-hidden rounded-full border-4 border-zinc-200 dark:border-zinc-700"
    ...
    >
      ...
      >
        ...
        <svg
          viewBox="0 0 120 12"
          className="animate-wave absolute top-0 left-0 h-3 w-[200%]"
          preserveAspectRatio="none"
        >
          <path d="M0,0 Q15,12 30,0 T60,0 T90,0 T120,0 V12 H0 Z" fill="white" fillOpacity="0.3" />
        </svg>
      </div>
      ...
      {showValue && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center">
          <span className="text-xl font-bold text-zinc-900 drop-shadow-sm dark:text-white">
            {Math.round(displayValue)}%
          </span>
          {label && <span className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">{label}</span>}
        </div>
      )}
    </div>
  )
      ...
      <div className={`flex gap-1 ${sizeClass}`}>
        {Array.from({ length: segments }, (_, index) => {
          ...
          return (
            <div
              key={index}
              className={`relative flex-1 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700 ${sizeClass}`}
            ...
            >
              ...
              />
              {isActive && <div className="absolute inset-0 animate-pulse bg-white/20" />}
            </div>
          )
        })}
      </div>
...
export const GradientProgress = memo(function GradientProgress({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`${sizeClass} relative overflow-hidden rounded-full transition-all duration-300 ${animated ? 'animate-gradient-shift' : ''}`}
          style={gradientStyle}
        >
          {/* 光泽效果 */}
      ...
      {steps.map((step, index) => {
        ...
        return (
          ...
          >
            {/* 步骤圆圈 */}
            <div
              className={`relative flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-all duration-300 ${
                isCompleted
                  ? `${colorClass} text-white`
                  : isCurrent
                    ? 'bg-white text-zinc-900 ring-2 ring-blue-500 ring-offset-2 dark:bg-zinc-800 dark:text-white'
            ...
            >
              ...
              {isCurrent && !isCompleted && (
                <div
                  className={`absolute inset-0 rounded-full ${colorClass} animate-ping opacity-30`}
                />
              )}
            </div>
            ...
            {index < steps.length - 1 && isHorizontal && (
              <div className="relative mx-2 h-0.5 min-w-[40px] flex-1 bg-zinc-200 dark:bg-zinc-700">
                <div
