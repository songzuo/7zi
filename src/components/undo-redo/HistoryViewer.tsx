...
export function HistoryViewer({
  ...
  const getTypeColor = (type: string) => {
    ...
    return colors[type] || 'bg-zinc-100 text-zinc-700 dark:bg-zinc-900/30 dark:text-zinc-400'
  }
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mb-1 flex items-center gap-2">
            <p className="truncate text-sm font-medium">{entry.description || '未命名操作'}</p>
            {showBadges && (
              <Badge
                variant="info"
                className={cn('text-xs', getTypeColor(entry.type || 'default'))}
              >
                {entry.type || 'default'}
              </Badge>
            )}
    ...
    <Card className={cn('p-4', className)}>
      ...
      <div className="space-y-2 overflow-y-auto pr-2" style={{ maxHeight }}>
        {filteredHistory.length === 0 ? (
          <div className="text-muted-foreground py-8 text-center text-sm">暂无操作历史</div>
        ) : (
...
export function HistoryMiniView({ className, limit = 5 }: HistoryMiniViewProps) {
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="truncate">
          {recentHistory.map((entry, i) => (
            <span
              key={entry.id}
              className={cn('mr-2 inline-block', i <= currentIndex ? 'opacity-100' : 'opacity-50')}
            >
              {entry.description}
            </span>
          ))}
