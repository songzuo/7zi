...
const ShimmerPlaceholder: React.FC<ShimmerPlaceholderProps> = memo(
  ({ className = '', rounded = 'rounded-lg' }) => {
    return (
      <div
        className={`absolute inset-0 overflow-hidden ${rounded} ${className} bg-gradient-to-r from-zinc-200 via-zinc-100 to-zinc-200 dark:from-zinc-700 dark:via-zinc-600 dark:to-zinc-700`}
        aria-hidden="true"
      >
        {/* Shimmer 动画效果 */}
        <div
          className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-transparent via-white/40 to-transparent dark:via-white/10"
          style={{
...
const BlurPlaceholder: React.FC<BlurPlaceholderProps> = memo(
  ({ blurDataURL, className = '', rounded = 'rounded-lg' }) => {
    return (
      <div
...
const SkeletonPlaceholder: React.FC<SkeletonPlaceholderProps> = memo(
  ({ className = '', rounded = 'rounded-lg', animate = true }) => {
    return (
      <div
        className={`absolute inset-0 ${rounded} ${className} bg-zinc-200 dark:bg-zinc-700 ${animate ? 'animate-pulse' : ''} `}
        aria-hidden="true"
      >
        {/* 图标 */}
...
const ColorPlaceholder: React.FC<ColorPlaceholderProps> = memo(
  ({ color = '#e5e5e5', className = '', rounded = 'rounded-lg' }) => {
    return (
      <div
...
const ErrorPlaceholder: React.FC<ErrorPlaceholderProps> = memo(
  ({ alt, onRetry, className = '', rounded = 'rounded-lg' }) => {
    return (
      <div
        className={`absolute inset-0 ${rounded} ${className} flex flex-col items-center justify-center bg-zinc-100 p-4 text-zinc-400 dark:bg-zinc-800 dark:text-zinc-500`}
        role="img"
      ...
      >
        ...
        <span className="mb-2 text-center text-sm">图片加载失败</span>
        {onRetry && (
          <button
            onClick={onRetry}
            className="rounded-lg bg-zinc-200 px-3 py-1 text-xs transition-colors hover:bg-zinc-300 dark:bg-zinc-700 dark:hover:bg-zinc-600"
          >
            重试
          </button>
        )}
      ...
      >
        ...
        {isInView && !hasError && (
          <Image
            ...
            quality={quality}
            className={`transition-all duration-500 ease-out ${objectFitClasses[objectFit]} ${isLoaded ? 'scale-100 opacity-100' : 'scale-105 opacity-0'} `}
            onLoad={handleLoad}
            onError={handleError}
...
export const ResponsiveLazyImage = memo(
  ({ aspectRatio = '16/9', className = '', ...props }: ResponsiveLazyImageProps) => {
    return (
      <div className={`relative overflow-hidden ${className}`} style={{ aspectRatio }}>
        <LazyLoadImage {...props} fill className="h-full w-full" />
      </div>
    )
  }
)
