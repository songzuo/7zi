...
export function SettingsPanel({ onClose, className = '' }: SettingsPanelProps) {
  ...
  return (
    <div className={`rounded-2xl bg-white shadow-xl dark:bg-zinc-900 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4 dark:border-zinc-700">
        <h2 className="text-xl font-bold text-zinc-900 dark:text-white">⚙️ 设置</h2>
        {onClose && (
          <button
            onClick={onClose}
        ...
        <section>
          ...
          <div className="space-y-3">
            ...
            {notifications.enabled && (
              <div className="ml-4 space-y-2 border-l-2 border-zinc-200 pl-4 dark:border-zinc-700">
                <NotificationToggle
                  icon="🔊"
                  label="声音"
                  description="播放通知声音"
        ...
        <section className="border-t border-zinc-200 pt-4 dark:border-zinc-700">
          {showResetConfirm ? (
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-zinc-100 px-4 py-3 font-medium text-zinc-600 transition-colors hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:hover:bg-zinc-700"
            >
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
...
function ToggleSwitch({ checked, onChange, disabled = false }: ToggleSwitchProps) {
  return (
    <button
      onClick={onChange}
      disabled={disabled}
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${checked ? 'bg-blue-500' : 'bg-zinc-300 dark:bg-zinc-600'} ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'} `}
      role="switch"
...
export function SettingsPanelCompact({ onClose, className = '' }: SettingsPanelProps) {
  return <SettingsPanel onClose={onClose} className={`max-w-md ${className}`} />
}
