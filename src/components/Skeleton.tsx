...
const SkeletonBase: React.FC<SkeletonBaseProps> = memo(({ 
  className = '', 
  animate = true 
}) => {
  return (
  ...
  return (
    <div 
      className={`
        ...
        rounded-xl bg-white dark:bg-zinc-800
        ${className}
      `}
    >
      {showAvatar && (
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {Array.from({ length: items }).map((_, i) => (
        <div 
          key={i}
          className="flex items-center gap-3 p-3 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
        >
...
export const SkeletonTable: React.FC<SkeletonTableProps> = memo(({ 
  ...
  return (
    <div className={`overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700 ${className}`}>
      {/* 表头 */}
      <div className="flex gap-4 p-4 bg-zinc-50 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700">
...
export const SkeletonStatCard: React.FC<SkeletonStatCardProps> = memo(({ 
  className = ''
}) => {
  return (
    <div 
      className={`
        p-6 rounded-2xl bg-white dark:bg-zinc-800 
        shadow-lg ${className}
      `}
    >
...
export const SkeletonNav: React.FC<SkeletonNavProps> = memo(({ 
  items = 5,
  className = ''
}) => {
  return (
    <nav className={`flex items-center gap-2 ${className}`}>
      ...
      <div className="flex items-center gap-1 ml-auto">
        {Array.from({ length: items }).map((_, i) => (
          <SkeletonBase key={i} className="w-16 h-8 rounded-lg" />
        ))}
      </div>
    </nav>
  );
...
export const SkeletonPage: React.FC<SkeletonPageProps> = memo(({ 
  ...
  return (
    <div className={`min-h-screen bg-zinc-50 dark:bg-zinc-900 ${className}`}>
      {showNav && (
        <header className="sticky top-0 z-50 bg-white dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 p-4">
          <SkeletonNav items={4} />
        </header>
      )}
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          <SkeletonBase className="h-12 w-3/4 mx-auto mb-4 rounded" />
          <SkeletonBase className="h-6 w-1/2 mx-auto mb-6 rounded" />
      ...
      {showFooter && (
        <footer className="bg-zinc-900 p-8 mt-auto">
