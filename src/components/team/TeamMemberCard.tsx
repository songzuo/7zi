...
export function TeamMemberCard({ member, translations }: TeamMemberCardProps) {
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${member.color} opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10 blur-sm`}
        aria-hidden="true"
      />
      <div
        className="absolute inset-0 rounded-2xl bg-white dark:bg-zinc-900 group-hover:bg-zinc-50 dark:group-hover:bg-zinc-800 transition-colors duration-300 -z-10"
        aria-hidden="true"
      />
