...
import type { Room, RoomType, RoomVisibility, RoomConfig, RoomParticipant, UserRole } from '@/lib/websocket/rooms';
...
export function RoomManager({
  ...
  const handleUpdateConfig = useCallback((roomId: string, config: Partial<RoomConfig>) => {
    updateRoom(roomId, {
  ...
  const handleChangeVisibility = useCallback((roomId: string, visibility: RoomVisibility) => {
    updateRoom(roomId, {
      visibility,
      updatedAt: new Date(),
    });
  ...
  const handleChangeRole = useCallback((roomId: string, targetUserId: string, newRole: UserRole) => {
    const room = rooms.find((r) => r.id === roomId);
    if (!room) return;
  ...
  const handleKickUser = useCallback((roomId: string, targetUserId: string) => {
  ...
  const handleBanUser = useCallback((roomId: string, targetUserId: string) => {
  ...
  const handleUnbanUser = useCallback((roomId: string, targetUserId: string) => {
    bannedUsers.delete(targetUserId);
  }, [bannedUsers]);

  const handleDestroyRoom = useCallback((roomId: string) => {
    const room = rooms.find((r) => r.id === roomId);
    if (!room) return;
  ...
  const ConnectionStatus = () => {
    if (isConnecting) {
      return (
        <div data-testid="connection-status" data-status="connecting" className="flex items-center gap-2 text-sm text-yellow-600 dark:text-yellow-400">
    ...
    if (isConnected) {
      return (
        <div data-testid="connection-status" data-status="connected" className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
    ...
    return (
      <div data-testid="connection-status" data-status="disconnected" className="flex items-center gap-2 text-sm text-red-600 dark:text-red-400">
  ...
  return (
    <div data-testid="room-manager" className="h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
      {/* Top Bar */}
      <div data-testid="room-manager-header" className="h-14 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-between px-4">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Room List Sidebar */}
        <div data-testid="room-list-sidebar" className="w-80 border-r border-gray-200 dark:border-gray-700 flex flex-col bg-white dark:bg-gray-800">
          <RoomList
        ...
        {/* Room View / Settings */}
        <div data-testid="room-view-container" className="flex-1 flex overflow-hidden">
