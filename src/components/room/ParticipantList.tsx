  ...
  if (layout === 'compact') {
    return (
      <div className="group relative">
        <div
          className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full text-sm font-semibold text-white"
  ...
  if (layout === 'grid') {
    return (
      <div className="flex flex-col items-center rounded-lg bg-gray-50 p-3 dark:bg-gray-800">
        ...
        <div className="w-full truncate text-center text-sm font-medium text-gray-900 dark:text-gray-100">
          {participant.name}
          {isCurrentUser && ' (你)'}
        </div>
        <div
          className={`rounded-full border px-2 py-0.5 text-xs ${getRoleBadgeColor(participant.role)}`}
        >
    ...
    <div data-testid="participant-item" data-user-id={participant.id} className={itemClasses}>
      ...
      <div className="relative">
        ...
        {!participant.isOnline && (
          <span
            data-status="offline"
            className="absolute right-0 bottom-0 h-3 w-3 rounded-full border-2 border-white bg-gray-400 dark:border-gray-900"
          />
        )}
      </div>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {isCurrentUser && <span className="text-xs text-gray-500 dark:text-gray-400">(你)</span>}
          {isOwner && <span className="text-xs">👑</span>}
        </div>
        <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
          <span
            className={`rounded-full border px-1.5 py-0.5 ${getRoleBadgeColor(participant.role)}`}
          >
            {getRoleLabel(participant.role)}
          </span>
          <span>•</span>
        ...
        <div className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          <div className="relative">
            <button
              ...
              aria-label="操作"
              className="rounded p-1.5 text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-100"
              title="操作"
            >
            ...
            {showRoleMenu && (
              <div className="absolute top-full right-0 z-10 mt-1 min-w-[100px] rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-gray-700 dark:bg-gray-800">
                {roles.map(role => (
                  <button
                    key={role}
                    onClick={() => {
                    ...
                    className={`w-full px-3 py-1.5 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-700 ${participant.role === role ? 'font-medium text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300'} `}
                  >
                    {getRoleLabel(role)}
                  </button>
                ))}
          ...
          <button
            ...
            aria-label="踢出用户"
            className="rounded p-1.5 text-gray-600 transition-colors hover:bg-orange-50 hover:text-orange-600 dark:text-gray-400 dark:hover:bg-orange-900/20 dark:hover:text-orange-400"
            title="踢出"
          >
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
      ...
      <div data-testid="participant-compact" className="flex items-center gap-1">
        ...
        {maxVisible && hiddenCount > 0 && !showAll && (
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-200 text-xs text-gray-600 dark:bg-gray-700 dark:text-gray-400"
          >
            +{hiddenCount}
          </button>
        )}
    ...
    return (
      <div>
        ...
        {maxVisible && hiddenCount > 0 && !showAll && (
          <button
            onClick={() => setShowAll(true)}
            className="mt-3 w-full rounded-lg bg-gray-100 py-2 text-sm text-gray-600 transition-colors hover:text-gray-900 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-gray-100"
          >
  ...
  return (
    <div data-testid="participant-list" role="list" aria-label="参与者列表">
        ...
        <div className="mt-4 border-t border-gray-200 pt-4 dark:border-gray-700">
          ...
          <div className="space-y-2">
            {bannedUsers.map(userId => (
              <div
                key={userId}
                className="flex items-center justify-between rounded bg-red-50 p-2 dark:bg-red-900/20"
              >
      ...
      {maxVisible && hiddenCount > 0 && !showAll && (
        <button
          onClick={() => setShowAll(true)}
          className="mt-3 w-full rounded-lg bg-gray-50 py-2 text-sm text-gray-600 transition-colors hover:text-gray-900 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-gray-100"
        >
          显示全部 {sortedParticipants.length} 位参与者
        </button>
      )}
