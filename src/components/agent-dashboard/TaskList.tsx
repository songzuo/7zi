    ...
    <div className={`rounded-xl border backdrop-blur-sm ${isDark ? 'bg-zinc-800/80 border-zinc-700/50' : 'bg-white/90 border-zinc-200/50'} ${className}`}>
      ...
      <div className="p-5 border-b border-zinc-200/50 dark:border-zinc-700/50">
        <h2 className={`text-lg font-semibold mb-4 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
          任务列表
          <span className={`ml-2 text-sm font-normal ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
      ...
      {/* Task List */}
      <div className="divide-y divide-zinc-200/50 dark:divide-zinc-700/50 max-h-[600px] overflow-y-auto">
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isExpanded ? 'bg-zinc-50/50 dark:bg-zinc-700/30' : 'hover:bg-zinc-50/30 dark:hover:bg-zinc-700/20'}
                `}
              >
                {/* Task Summary */}
                <div 
                ...
                >
                  <div className="flex items-start gap-3">
                    {/* Expand Icon */}
                    <span className="text-zinc-400 mt-1 transition-transform duration-200">
                      {isExpanded ? '▼' : '▶'}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className={`font-medium truncate ${isDark ? 'text-white' : 'text-zinc-900'}`}>
                          {task.title}
                        </h3>
                        <span className={`
                ...
                {isExpanded && (
                  <div className={`px-4 pb-4 pl-10 ${isDark ? 'text-zinc-300' : 'text-zinc-700'}`}>
                    ...
                    {task.metadata?.tags && Array.isArray(task.metadata.tags) && task.metadata.tags.length > 0 && (
