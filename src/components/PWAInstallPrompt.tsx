...
export function PWAInstallPrompt() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 fixed right-4 bottom-4 z-50 max-w-sm duration-300">
      <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-800">
        ...
        <div className="flex gap-2 px-4 pb-4">
          <button
            ...
            className="flex-1 rounded-lg bg-cyan-600 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-cyan-700"
          >
            立即安装
          </button>
          <button
            onClick={handleInstallLater}
            className="flex-1 rounded-lg px-3 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-700"
          >
            稍后再说
          </button>
        </div>
...
function IOSInstallGuide({ onClose }: { onClose: () => void }) {
  return (
    <div className="animate-in fade-in fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm duration-200">
      <div className="animate-in zoom-in-95 mx-4 max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl duration-200 dark:bg-zinc-800">
        {/* Header */}
        <div className="bg-gradient-to-r from-cyan-600 to-cyan-700 px-4 py-3">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-white">在 iOS 上安装</h3>
            <button onClick={onClose} className="text-cyan-100 transition-colors hover:text-white">
        ...
        <div className="space-y-4 p-6">
          <ol className="space-y-4">
            <li className="flex gap-3">
              <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-cyan-600 text-sm font-semibold text-white">
                1
              </div>
              <div className="flex-1">
                ...
                <div className="mt-1 flex justify-center">
                  <div className="rounded-lg bg-zinc-100 px-2 py-1 dark:bg-zinc-700">
                    <svg
        ...
        <div className="px-4 pb-4">
          <button
            onClick={onClose}
            className="w-full rounded-lg px-4 py-2 text-sm font-medium text-cyan-600 transition-colors hover:bg-cyan-50 dark:text-cyan-400 dark:hover:bg-cyan-900/20"
          >
            知道了
          </button>
        </div>
