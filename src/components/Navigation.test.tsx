...
vi.mock('./LanguageSwitcher', () => ({
  LanguageSwitcherCompact: ({ className }: { className?: string }) => (
    <button data-testid="language-switcher" className={className}>
      语言
    </button>
  ),
}))
...
vi.mock('./SettingsButton', () => ({
  SettingsButton: ({ compact }: { compact?: boolean }) => (
    <button data-testid="settings-button" data-compact={compact}>
      设置
    </button>
  ),
}))
...
describe('Navigation', () => {
  ...
  describe('活跃状态', () => {
    it('首页路径时，首页链接应该有活跃状态', () => {
      ...
      const hasActiveLink = homeLinks.some(link => link.className.includes('bg-cyan-50'))
    ...
    it('Dashboard 路径时，实时看板链接应该有活跃状态', () => {
      ...
      const hasActiveLink = dashboardLinks.some(link => link.className.includes('bg-cyan-50'))
      expect(hasActiveLink).toBe(true)
    })
    ...
    it('非活跃链接应该有默认样式', () => {
      ...
      const dashboardLinks = screen.getAllByRole('link', { name: /实时看板/ })
      const hasDefaultStyle = dashboardLinks.some(
        link => link.className.includes('text-zinc-600') || link.className.includes('text-zinc-600')
      )
