...
import { WorkflowEdge, EdgeType } from '@/types/workflow'
...
export function WorkflowEdgeComponent({
  ...
  return (
    <g className={cn('edge', 'group', !readOnly && 'cursor-pointer')}>
      {/* 箭头标记 */}
      <defs>{generateArrowMarker(markerId, style.color)}</defs>
      ...
      <path
        ...
        className={cn('transition-all', !readOnly && 'hover:stroke-w-[3px] hover:stroke-blue-500')}
      />
      ...
      {edge.type === EdgeType.DEFAULT && (
        <foreignObject
          ...
          height={20}
        >
          <div className="px-1 py-0.5 text-center text-xs text-gray-500">默认</div>
        </foreignObject>
      )}
