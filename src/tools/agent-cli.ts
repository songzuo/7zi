...
import { Task, TaskPriority, TaskType, createTask } from '../lib/agents/scheduler/models/task-model'
import {
  AgentCapability,
 ...
 */
function formatAgent(agent: AgentCapability): Record<string, unknown> {
  const availability = agent.availability ? '🟢 可用' : '🔴 不可用'
  const loadPercent = agent.currentLoad.toFixed(0)
  const loadBar =
    '█'.repeat(Math.floor(agent.currentLoad / 10)) +
...
taskCmd
  ...
  .action((options: Record<string, unknown>) => {
    ...
    if (options.urgent) {
      ...
      tasks = tasks.filter(
        t => t.priority === 'urgent' || (t.deadline && t.deadline - now < oneHour)
      )
    }
...
agentCmd
  ...
  .action((options: Record<string, unknown>) => {
    ...
    if (options.type) {
      agents = agents.filter(a => a.capabilities.taskTypes.includes(options.type as AgentTaskType))
    }
...
scheduleCmd
  ...
  .action((options: Record<string, unknown>) => {
    ...
    let decisions: ScheduleDecision[]
    if (options.agent) {
