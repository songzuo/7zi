...
import { getCsrfToken, clearCsrfToken, createCsrfHeaders, validateCsrfToken } from '../csrf'
...
describe('csrf.ts - CSRF 工具测试', () => {
  ...
  describe('综合场景测试', () => {
    it('应该完成完整的 CSRF 流程', async () => {
      ...
      // 3. 验证 token
      const isValid = validateCsrfToken(headers['X-CSRF-Token'] as string, mockToken)
