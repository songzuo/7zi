...
export async function validateUserToken(
  token: string
): Promise<{ user: User; token: UserToken } | null> {
  const db = await getDatabaseAsync()
  await initializeUserTables()
...
export function getDefaultPermissions(role: UserRole): string[] {
  ...
  switch (role) {
    ...
    case UserRole.MANAGER:
      return [...basePermissions, 'write:tasks', 'delete:tasks', 'manage:team', 'access:reports']

    case UserRole.MEMBER:
      return [...basePermissions, 'write:tasks', 'update:tasks']
