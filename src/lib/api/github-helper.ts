...
export function handleGitHubError(
  ...
  if (response.status === 404) {
    ...
    return NextResponse.json({ error: 'GitHub Token 无效或已过期' }, { status: 401 })
...
export async function fetchGitHubCommits(options: GitHubAPIOptions = {}): Promise<NextResponse> {
  return fetchFromGitHub(
    `/repos/${options.owner || process.env.NEXT_PUBLIC_GITHUB_OWNER}/${options.repo || process.env.NEXT_PUBLIC_GITHUB_REPO}/commits`,
    options
  )
}
