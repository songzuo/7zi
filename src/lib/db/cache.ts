...
class DatabaseCache {
  ...
  private evictForSize(requiredSize: number): void {
    let freed = 0;
    while (this.head && (freed < requiredSize || this.currentMemoryUsage >= this.config.maxMemoryUsage)) {
...
export class MemoizationCache {
   ...
   * );
   */
  memoizeSync<T extends (...args: unknown[]) => unknown>(
    fn: T,
    options: MemoizationOptions
  ): T {
    const {
  ...
  getStats(keyPrefix?: string): Map<string, MemoizationStats> | MemoizationStats {
    if (keyPrefix) {
      return this.stats.get(keyPrefix) || {
        hits: 0,
        misses: 0,
        totalCalls: 0,
        hitRate: 0,
