...
export async function batchInsert<T extends Record<string, unknown>>(
  ...
  try {
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      if (onProgress) {
        onProgress(Math.min(i + batchSize, records.length), records.length, result.batches)
      }
    }
  } catch (error) {
    if (!continueOnError) {
...
export async function batchUpdate<T extends Record<string, unknown>>(
  ...
  const { batchSize = 500, useTransaction = true, continueOnError = false, onProgress } = options
  ...
  try {
    for (let i = 0; i < updates.length; i += batchSize) {
      ...
      if (onProgress) {
        onProgress(Math.min(i + batchSize, updates.length), updates.length, result.batches)
      }
    }
  } catch (error) {
    if (!continueOnError) {
...
export async function batchExecute(
  ...
  if (useTransaction) {
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error)
      logger.error('[BatchExecute Error]', error, {
        category: 'db',
