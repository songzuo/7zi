...
export async function initializeFeedbackTables(): Promise<void> {
  ...
  db.exec(
    'CREATE INDEX IF NOT EXISTS idx_spam_detection_logs_user_id ON spam_detection_logs(user_id)'
  )
  db.exec(
  ...
  db.exec(
    'CREATE INDEX IF NOT EXISTS idx_feedback_notifications_read_at ON feedback_notifications(read_at, created_at DESC)'
  )
...
export async function getFeedbackStatistics(): Promise<FeedbackStatistics> {
  ...
  // Total feedbacks
  const totalResult = db.queryRows('SELECT COUNT(*) as count FROM feedbacks')[0] as {
    count: number
  }
  ...
  const by_priority = priorityResults.reduce(
  ...
  const rating_distribution = ratingResults.reduce(
...
export async function getRatingStatistics(filters?: {
  ...
  // Total ratings
  const totalResult = db.queryRows(
  ...
  // Average rating
  const avgResult = db.queryRows(
    `SELECT AVG(rating) as avg FROM ratings ${whereClause}`,
    params
  )[0] as {
    avg: number
  }
  ...
  const helpful_ratio =
    helpfulVotes.count + notHelpfulVotes.count > 0
