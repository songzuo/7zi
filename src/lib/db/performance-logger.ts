...
import { withPerformanceLogging, getQueryMetricsSummary, getQueryInsights } from '../middleware/db-performance';
...
class PerformanceLoggerManager {
  ...
  constructor() {
    this.config = {
      enabled: process.env.ENABLE_DB_PERFORMANCE_LOGGING === 'true' || process.env.NODE_ENV === 'development',
  ...
  getPerformanceSummary(): PerformanceSummary {
    ...
    return {
      ...
      errorQueryCount: dbMetrics.total - Math.floor((dbMetrics.successRate / 100) * dbMetrics.total),
      nPlus1Detections,
      insights,
  ...
  getHealthStatus(): {
    ...
    // 检查慢查询率
    const slowQueryRate = summary.totalQueries > 0
      ? (summary.slowQueryCount / summary.totalQueries) * 100
      : 0;
 ...
 * 快捷函数：包装数据库连接
 */
export function wrapDatabaseWithLogging(db: Database.Database | DatabaseConnection): DatabaseConnection {
