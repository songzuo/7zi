...
import { RoomManager, getRoomManager, resetRoomManager, RoomType } from '../rooms'
import { PermissionManager, resetPermissionManager } from '../permissions'
import { MessageStore, resetMessageStore, StoredMessage } from '../message-store'
...
describe('WebSocket 高级集成测试', () => {
  ...
  describe('权限系统复杂场景', () => {
    ...
    it('应正确处理无效权限操作', async () => {
      // 对不存在的房间授予权限
      permissionManager.grantPermission('user', 'non-existent-room', 'send_message' as any, 'admin')
  ...
  describe('消息存储复杂场景', () => {
    ...
    it('应正确处理消息反应', async () => {
      ...
      const message = messageStore.get(messageId)
      expect(message?.reactions?.some(r => r.emoji === '👍' && r.userId === 'user-react')).toBe(
        true
      )
    })
