...
export class RoomManager {
   ...
   * Join a room
   */
  join(roomId: string, options: JoinRoomOptions): {
    success: boolean;
    room?: Room;
    ...
    const offlineMessages = this.messageStore.getOfflineMessages(userId)
      .map(m => m.message)
   ...
   */
  kick(roomId: string, userId: string, kickedBy: string, reason?: string): {
    success: boolean;
    error?: string;
  } {
    const room = this.rooms.get(roomId);
   ...
   */
  ban(roomId: string, userId: string, bannedBy: string, reason?: string): {
    success: boolean;
    error?: string;
  } {
    const room = this.rooms.get(roomId);
   ...
   */
  unban(roomId: string, userId: string, unbannedBy: string): {
    success: boolean;
    error?: string;
  } {
    const room = this.rooms.get(roomId);
   ...
   */
  changeRole(roomId: string, userId: string, newRole: UserRole, changedBy: string): {
    success: boolean;
    error?: string;
    oldRole?: UserRole;
  } {
   ...
   */
  invite(roomId: string, userId: string, invitedBy: string): {
    success: boolean;
    error?: string;
  } {
    const room = this.rooms.get(roomId);
   ...
   */
  updateCursor(roomId: string, userId: string, cursor: {
    position: number;
    selection?: { start: number; end: number };
  }): boolean {
    const room = this.rooms.get(roomId);
