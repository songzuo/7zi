    ...
    (set, get) => ({
      ...
      removeRoom: (roomId) => {
        set((state) => ({
          ...
          participants: new Map(Array.from(state.participants.entries()).filter(([id]) => id !== roomId)),
        }));
      },
      ...
      removeMessage: (roomId, messageId) => {
        set((state) => {
          const newMessages = new Map(state.messages);
          const roomMessages = newMessages.get(roomId) || [];
          newMessages.set(roomId, roomMessages.filter((m) => m.id !== messageId));
          return { messages: newMessages };
        });
      },
      ...
      removeParticipant: (roomId, userId) => {
        set((state) => {
          const newParticipants = new Map(state.participants);
          const roomParticipants = newParticipants.get(roomId) || [];
          newParticipants.set(roomId, roomParticipants.filter((p) => p.id !== userId));
          return { participants: newParticipants };
        });
      },
...
export const selectCurrentParticipants = (state: WebSocketRoomStore) => state.getCurrentParticipants();
