...
import type { TraceId, SpanId, TraceContext } from "@/lib/tracing/types";
...
export async function withTrace<T>(
  ...
  if (!span) {
    ...
    return handler({
      ...tm.getContext()!,
      traceManager: tm,
      span: { spanId: "unknown", name: "no-span" },
      request,
    });
  }
 ...
 * 将 traceId 注入到响应头
 */
export function injectTraceIdToResponse(
