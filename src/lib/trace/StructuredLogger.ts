...
const defaultOptions: Required<Omit<StructuredLoggerOptions, 'transport'>> & { transport?: (entry: LogEntry) => void | Promise<void> | undefined } = {
  serviceName: "unknown",
...
export class StructuredLogger {
  private options: Required<Omit<StructuredLoggerOptions, 'transport'>> & { transport?: (entry: LogEntry) => void | Promise<void> | undefined };
  ...
  fatal(message: string, errorOrFields?: Error | Record<string, unknown>, fields?: Record<string, unknown>): void {
    let error: Error | undefined;
    let extraFields: Record<string, unknown> | undefined;
  ...
  private log(
    ...
    if (error) {
      ...
      if ("cause" in error && error.cause) {
        entry.error.cause = error.cause instanceof Error 
          ? error.cause.message 
          : String(error.cause);
      }
    }
