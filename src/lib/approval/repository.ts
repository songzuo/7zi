...
class ApprovalStore {
  ...
  list(query: ApprovalListQuery): ApprovalListResult {
    ...
    // 过滤审批人
    if (query.approverId) {
      items = items.filter(item => item.approvers.some(a => a.userId === query.approverId))
    }
  ...
  getByRequester(requesterId: string): ApprovalRequest[] {
    return Array.from(this.approvals.values()).filter(item => item.requesterId === requesterId)
  }
  ...
  getPendingForApprover(approverId: string): ApprovalRequest[] {
    return Array.from(this.approvals.values()).filter(
      item =>
        ...
        item.approvers.some(a => a.userId === approverId && a.status === ApprovalStatus.PENDING)
    )
  }
...
export class ApprovalRepository {
  ...
  static approve(
    ...
    // 检查是否所有审批人都已批准
    const allApproved = updated.approvers.every(a => a.status === ApprovalStatus.APPROVED)
  ...
  static reject(
    ...
    // 更新审批人状态
    let updated = this.updateApproverStatus(approvalId, approverId, ApprovalStatus.REJECTED, reason)
    if (!updated) return null
