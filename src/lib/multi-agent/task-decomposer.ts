...
export class TaskDecomposer extends EventEmitter {
  ...
  async decomposeWithTemplate(
    ...
    for (const templateSubTask of template.subTasks) {
      const subTask: SubTask = {
        ...
        dependencies: templateSubTask.dependencies?.map(depId => ({
   ...
   * 生成子任务模板
   */
  private async generateSubTaskTemplates(
    task: Task,
    taskType: string
   ...
   * 执行子任务
   */
  private async executeSubTasks(task: Task): Promise<Map<string, any>> {
    ...
    while (completed.size < task.subTasks.length) {
      ...
      const readyTasks = task.subTasks.filter(
        subTask =>
          ...
          this.areDependenciesMet(subTask, completed)
      );
      ...
      if (readyTasks.length === 0) {
        ...
        // 检查是否有失败的任务
        const failedTask = task.subTasks.find(
          subTask => subTask.status === TaskStatus.FAILED
        );
        if (failedTask) {
          throw new MultiAgentError(
            MultiAgentErrorType.DEPENDENCY_FAILED,
  ...
  private prepareSubTaskInput(
    ...
    for (const dep of subTask.dependencies) {
      ...
      if (result) {
        input.dependencyResults[dep.taskId] = result;
      }
    }
  ...
  private aggregateResults(results: Map<string, any>): any {
    ...
    results.forEach((value, key) => {
      aggregated.subTaskResults[key] = value;
    });
