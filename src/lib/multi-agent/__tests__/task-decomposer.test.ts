...
import { TaskStatus, MessagePriority, TransportType } from '../types'
...
describe('TaskDecomposer', () => {
  ...
  describe('任务创建', () => {
    it('应该成功创建任务', async () => {
      const task = await taskDecomposer.createTask('Test Task', 'A test task', { data: 'test' })
  ...
  describe('任务分解', () => {
    it('应该自动分解分析任务', async () => {
      ...
      expect(task.subTasks.some(st => st.name.includes('收集') || st.name.includes('分析'))).toBe(
        true
      )
    })
    ...
    it('应该自动分解开发任务', async () => {
      ...
      expect(task.subTasks.length).toBeGreaterThan(0)
      expect(task.subTasks.some(st => st.name.includes('设计') || st.name.includes('实现'))).toBe(
        true
      )
    })
  ...
  describe('任务执行', () => {
    ...
    it('应该处理任务执行失败', async () => {
      vi.spyOn(messageBus, 'request').mockRejectedValueOnce(new Error('Agent error'))
