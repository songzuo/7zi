...
import { MessageType, MessagePriority, MultiAgentError, TransportType } from '../types'
...
describe('MessageBus', () => {
  ...
  describe('请求-响应模式', () => {
    it('应该发送请求并等待响应', async () => {
      ...
      // 模拟响应 - 直接在 messageBus 上监听
      const handler = async ({ message }: { message: any }) => {
        if (message.headers.type === MessageType.REQUEST) {
          // 发送响应 - 注意 correlationId 应该是 message.headers.correlationId
          const response = {
            headers: {
              id: 'response-1',
              ...
              correlationId: message.headers.correlationId, // 使用 correlationId 而不是 id
              priority: MessagePriority.NORMAL,
              timestamp: Date.now(),
            },
            body: { result: 'success' },
      ...
      messageBus.on('message.received', handler)
