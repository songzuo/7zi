...
describe('AgentCollaborationProtocol', () => {
  ...
  beforeEach(async () => {
    ...
    protocol = new AgentCollaborationProtocol(
      agentId,
      messageBus,
      registry,
      taskDecomposer
  ...
  describe('任务取消', () => {
    it('应该取消委托的任务', async () => {
      ...
      // 验证任务被移除
      await expect(
        protocol.cancelTask('task-to-cancel', 'Again')
      ).rejects.toThrow('not found or not delegated');
    });

    it('应该拒绝取消不存在的任务', async () => {
      await expect(
        protocol.cancelTask('non-existent', 'Reason')
      ).rejects.toThrow('not found or not delegated');
    });
  });
  ...
  describe('能力查询', () => {
    ...
    it('应该返回具备指定能力的 Agent', async () => {
      ...
      const hasCapability = response.agents.some(
        agent => agent.capabilities.some(cap => cap.id === 'coding')
      );
