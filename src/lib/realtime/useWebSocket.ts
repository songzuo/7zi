...
export function useWebSocket(
  ...
  const sendMessage = useCallback((data: WebSocketMessage) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      ...
      logger.warn('[useWebSocket] WebSocket is not connected. Message not sent:', {
        data: String(data),
      })
    }
  }, [])
  ...
  // 添加一次性事件监听器
  const addOnceListener = useCallback(
    (event: string, handler: (data: unknown) => void): (() => void) => {
 ...
 * 创建标准 WebSocket 消息
 */
export function createMessage<T = unknown>(type: string, payload?: T): WebSocketMessage {
  return {
    type,
