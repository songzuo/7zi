...
export function NotificationProvider({
  ...
  // 处理批量通知
  const handleBatchNotifications = useCallback(
    (notifications: RealtimeNotification[]) => {
      // 过滤出应该显示的通知
  ...
  const handleMessage = useCallback(
    ...
    [batchEnabled, addToBatch, handleBatchNotifications, handleNotification, preferences.batchDelay]
  )
  ...
  useEffect(() => {
    ...
    const unsubscribers = messageTypes.map(type => onMessage(type, handleMessage))
