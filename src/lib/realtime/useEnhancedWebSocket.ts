...
const getMessagePriority = (type: string): MessagePriority => {
  ...
  const lowPriorityTypes = ['heartbeat', 'read_status_updated']
...
export function useEnhancedWebSocket(config: WebSocketConfig): UseEnhancedWebSocketReturn {
  ...
  const messageHandlersRef = useRef<Map<string, Set<(message: WebSocketMessage) => void>>>(
    new Map()
  )
  ...
  const trimOfflineQueue = useCallback(() => {
    ...
    if (thresholdReached) {
      ...
      if (droppedCount > 0 && process.env.NODE_ENV === 'development') {
        logger.info(
          `[WebSocket] Dropped ${droppedCount} low priority messages from offline queue`,
          { category: 'system' }
        )
        setStats(prev => ({
          ...prev,
      ...
      if (droppedCount > 0 && process.env.NODE_ENV === 'development') {
        logger.info(`[WebSocket] Dropped ${droppedCount} old messages from offline queue (FIFO)`, {
          category: 'system',
        })
        setStats(prev => ({
          ...prev,
    ...
    offlineMessageIdsRef.current = new Set(
      Array.from(offlineMessageIdsRef.current).filter(id => keptIds.has(id))
    )
  ...
  const processOfflineQueue = useCallback(() => {
    ...
    if (sentCount > 0 && process.env.NODE_ENV === 'development') {
      logger.info(`[WebSocket] Processed offline queue: ${sentCount} sent, ${failedCount} failed`, {
        category: 'system',
      })
    }
  ...
  const sendMessage = useCallback(
    (type: string, payload?: unknown, priority?: MessagePriority) => {
      const messageId = generateMessageId()

      // 检查是否已在离线队列中（去重）
      if (offlineMessageIdsRef.current.has(messageId)) {
        if (process.env.NODE_ENV === 'development') {
          logger.info(`[WebSocket] Message ${messageId} already in offline queue, skipping`, {
            category: 'system',
          })
        }
        setStats(prev => ({
  ...
  const addMessageListener = useCallback(
    <T extends WebSocketMessage>(type: string, handler: (message: T) => void): (() => void) => {
