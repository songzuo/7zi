...
export async function getOfflineData<T>(
  ...
  try {
    const result = await dbOperation(STORES.DATA, 'readonly', store => store.get([entityType, id]))
    return result || null
  } catch (error) {
    return null
  }
 ...
 * 获取未同步的数据
 */
export async function getUnsyncedData<T>(entityType: OfflineEntityType): Promise<OfflineData<T>[]> {
  const db = await getDBConnection().getDB()
 ...
 * 删除离线数据
 */
export async function deleteOfflineData(entityType: OfflineEntityType, id: string): Promise<void> {
...
export async function saveNetworkStatus(status: NetworkStatus): Promise<void> {
  await dbOperation(STORES.NETWORK, 'readwrite', store => store.put({ id: 'current', ...status }))
}
...
export async function getNetworkStatus(): Promise<NetworkStatus | null> {
  try {
    const result = await dbOperation(STORES.NETWORK, 'readonly', store => store.get('current'))
    return result || null
  } catch (error) {
    return null
  }
