...
describe('RCAEngine', () => {
  ...
  describe('analyzeTrace', () => {
    it('should analyze trace with database timeout', async () => {
      const traceData: TraceData = {
        ...
        spans: [
          createMockSpan(
            'db.query.users',
            { code: SpanStatusCode.ERROR, message: 'Connection timeout' },
            5000
          ),
        ],
    ...
    it('should detect slow spans', async () => {
      const traceData: TraceData = {
        traceId: 'trace-slow',
        spans: [createMockSpan('api.request', { code: 1 }, 6000)],
  ...
  describe('registerTrace', () => {
    it('should register trace data correctly', () => {
      const spans: Span[] = [
        ...
        createMockSpan('db.query', { code: SpanStatusCode.ERROR, message: 'Error' }, 200, 'span-1'),
      ]
    ...
    it('should detect database timeout pattern', async () => {
      const traceData: TraceData = {
        ...
        spans: [
          createMockSpan(
            'db.query.slow',
            { code: SpanStatusCode.ERROR, message: 'ETIMEDOUT' },
            10000
          ),
        ],
    ...
    it('should generate recommendations', async () => {
      const traceData: TraceData = {
        ...
        spans: [
          createMockSpan(
            'db.query',
            { code: SpanStatusCode.ERROR, message: 'deadlock detected' },
            500
          ),
        ],
    ...
    it('should build event timeline', async () => {
      const traceData: TraceData = {
        ...
        spans: [
          ...
          createMockSpan('operation.error', { code: SpanStatusCode.ERROR, message: 'Failed' }, 50),
        ],
        metadata: {},
        errors: [
          {
  ...
  describe('Caching', () => {
    it('should cache analysis results', async () => {
      const traceData: TraceData = {
        traceId: 'trace-cache',
        spans: [
          createMockSpan('test-operation', { code: SpanStatusCode.ERROR, message: 'Error' }, 100),
        ],
        metadata: {},
        errors: [
          {
