...
import type { Span, TraceId, SpanId } from '../tracing/types'
import { SpanStatusCode } from '../tracing/types'
...
const ERROR_PATTERNS: ErrorPattern[] = [
  ...
  },
  {
    ...
    rootCauseType: 'database-issue',
    signatures: [{ type: 'span-name', pattern: 'db.query', matchType: 'contains' }],
  ...
  private matchErrorPatterns(logs: LogEntry[]): RootCauseHypothesis[] {
    ...
    for (const pattern of ERROR_PATTERNS) {
      const matchingLogs = logs.filter(log =>
        pattern.signatures.some(
          sig => sig.type === 'error-message' && this.matchValue(sig, log.message.toLowerCase())
        )
      )
