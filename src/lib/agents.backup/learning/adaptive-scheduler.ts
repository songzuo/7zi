...
export class AdaptiveScheduler {
  ...
  async schedule(task: SchedulableTask): Promise<SchedulingDecision> {
    ...
    const decision: SchedulingDecision = {
      ...
      reasoning: this.generateReasoning(best.score, alternatives),
      alternatives,
  ...
  getStats(): {
    ...
    const avgPredicted = completed
      .reduce((s, r) => s + r.decision.predictedTime.estimatedMinutes, 0) / completed.length;
    const avgActual = completed
      .reduce((s, r) => s + (r.actualTime ?? 0), 0) / completed.length;
  ...
  private async scoreAgent(
    ...
    const capabilityScore = this.config.scheduling.preferFasterAgent ? capability * 0.3 : capability * 0.4;
  ...
  private generateReasoning(
    best: { score: ReturnType<typeof this.scoreAgent> extends Promise<infer T> ? T : never },
   ...
   */
  private isLoadBalanced(
    ranked: Array<{ agent: AgentState; score: { total: number } }>
  ): boolean {
  ...
  private adjustWeightBasedOnPerformance(
    ...
    const avgPredicted = history.reduce((s, h) => 
      s + h.decision.predictedTime.estimatedMinutes, 0) / history.length;
    const avgActual = history.reduce((s, h) => 
      s + (h.actualTime ?? 0), 0) / history.length;
  ...
  private checkWeightAdjustments(): void {
    ...
    if (lb.imbalance > this.config.loadBalance.rebalanceThreshold) {
      ...
      const underloaded = lb.agents.filter(a => a.load < this.config.loadBalance.targetUtilization * 0.5);
