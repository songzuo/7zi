...
export class LoadBalancer {
  ...
  getAvailableAgents(
    ...
    return Array.from(agents.values()).filter(agent =>
      !this.isAgentAtCapacity(agent, task) && agent.availability
    );
  }
  ...
  balanceLoad(
    ...
    const recommendedAgents = sortedByLoad
  ...
  redistributeTasks(
    ...
    const capableAgents = otherAgents.filter(agent => 
      !this.isAgentAtCapacity(agent, task)
    );
  ...
  getLoadStats(agents: Map<string, AgentCapability>): {
    ...
    const idleAgents = agentArray
      .filter(a => a.currentLoad < 20)
      .map(a => a.agentId);
  ...
  isSystemOverloaded(agents: Map<string, AgentCapability>): boolean {
    ...
    return stats.averageLoad > 80 || 
           stats.overloadedAgents.length > (agents.size / 2);
  }
