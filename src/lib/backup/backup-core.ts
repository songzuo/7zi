          ...
          try {
            ...
            try {
              ...
              if (file.endsWith('.gz') || file.endsWith('.br')) {
                content = await decompressBackup(content, file.endsWith('.gz') ? CompressionAlgorithm.GZIP : CompressionAlgorithm.BROTLI);
              }
 ...
 */
export async function getBackup(backupId: string): Promise<{ metadata: BackupMetadata; data: Record<string, unknown[]> } | null> {
  try {
...
export async function restoreBackup(
  ...
  try {
    ...
    const filePath = path.join(BACKUP_DIR, `${options.backupId}.json${options.backupId.endsWith('.gz') ? '.gz' : options.backupId.endsWith('.br') ? '.br' : ''}`);
    let content = await fs.readFile(filePath, 'utf-8');
...
export async function getBackupStatistics(): Promise<BackupStatistics> {
  ...
  const totalRecords = backups.reduce((sum, b) =>
    sum + Object.values(b.recordCounts).reduce((s, count) => s + count, 0), 0
  );
