...
export class ResourcePrefetcher {
  ...
  async prefetchResources(resources: ResourceConfig[]): Promise<PrefetchResult[]> {
    ...
    if (grouped.high.length > 0) {
      const highResults = await Promise.all(grouped.high.map(r => this.prefetchResource(r)))
    ...
    if (grouped.medium.length > 0) {
      ...
      const mediumResults = await Promise.all(grouped.medium.map(r => this.prefetchResource(r)))
    ...
    if (grouped.low.length > 0) {
      ...
      const lowResults = await Promise.all(grouped.low.map(r => this.prefetchResource(r)))
...
export const PREFETCH_PRESETS = {
  /** 关键资源预加载 */
  critical: {
    fonts: ['/fonts/inter-var.woff2', '/fonts/inter-bold.woff2'],
    styles: ['/styles/critical.css'],
    scripts: ['/scripts/polyfills.js'],
  },

  /** 首页预加载 */
  home: {
    images: ['/images/hero-bg.webp', '/images/hero-mobile-bg.webp'],
    data: ['/api/featured-content', '/api/recent-updates'],
  },

  /** Dashboard 预加载 */
  dashboard: {
    data: ['/api/dashboard/stats', '/api/user/preferences', '/api/notifications'],
    images: ['/images/dashboard-placeholder.webp'],
  },
} as const
