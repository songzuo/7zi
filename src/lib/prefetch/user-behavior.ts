...
export class UserBehaviorAnalyzer {
    ...
    if (pattern) {
      ...
      if (timeSpent && timeSpent > 0) {
        ...
        pattern.avgSessionDuration = 
          pattern.avgSessionDuration * (1 - weight) + timeSpent * weight;
      }
    } else {
   ...
   * 获取下一个可能访问的页面（基于历史模式）
   */
  getNextPages(currentPath: string, limit: number = 5): Array<{
    path: string;
    probability: number;
  }> {
        ...
        if (transitionCount > 0) {
          ...
          if (currentPattern) {
            for (const [nextPath, frequency] of currentPattern.nextPages) {
              // 计算置信度：基于转移概率
              const confidence = (transitionCount / pattern.visitCount) * 
                               (frequency / currentPattern.visitCount);
  ...
  getStats(): {
    ...
    const avgSessionDuration = this.data.sessions.length > 0
      ? this.data.sessions.reduce((sum, s) => sum + s.totalTime, 0) / 
