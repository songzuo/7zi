...
import { ResourcePrefetcher, globalResourcePrefetcher, type ResourceConfig } from './resource-prefetcher';
...
export function PrefetchProvider({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      metricsRef.current = {
        ...
        avgLoadTime: loadTimesRef.current.length > 0
  ...
  const prefetchResources = useCallback(async (resources: ResourceConfig[]) => {
  ...
  const recordVisit = useCallback((path: string, timeSpent?: number) => {
  ...
  const recordNavigation = useCallback((fromPath: string, toPath: string) => {
