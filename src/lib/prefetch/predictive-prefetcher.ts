      ...
      if (transition) {
        ...
        if (currentPattern) {
          for (const [nextPath, frequency] of currentPattern.nextPages) {
            const confidence = (transition / prevPattern.visitCount) * (frequency / currentPattern.visitCount);
            if (confidence > 0.2) {
              predictions.push({
                path: nextPath,
  ...
  private predictFromHeuristics(context: UserContext): PrefetchPrediction[] {
    ...
    const heuristics: Record<string, PrefetchPrediction[]> = {
      ...
      '/settings': [
        ...
        { path: '/settings/notifications', confidence: 0.5, reason: 'heuristics', priority: 'medium' },
      ],
    };
  ...
  importPatterns(patterns: UserBehaviorPattern[]): void {
    for (const pattern of patterns) {
      this.behaviorPatterns.set(pattern.path, {
        ...pattern,
        nextPages: new Map(Object.entries(pattern.nextPages as any)),
      });
    }
  }
}
...
export function usePredictivePrefetch() {
  return {
    ...
    recordNavigation: (from: string, to: string) => 
      globalPrefetcher.recordNavigation(from, to),
    predict: (context: UserContext) => 
      globalPrefetcher.predictNextPages(context),
    prefetch: (paths: string[]) => 
      globalPrefetcher.prefetch(paths),
  };
}
