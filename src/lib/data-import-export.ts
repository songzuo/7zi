...
async function importRow(
  ...
  if (mode === 'insert') {
    ...
    if (skipDuplicates && primaryKey) {
      ...
      const existing = db.queryRows(`SELECT 1 FROM ${table} WHERE ${primaryKey} = ? LIMIT 1`, [
        pkValue,
      ])
    ...
    if (setClause) {
      db.exec(`UPDATE ${table} SET ${setClause} WHERE ${primaryKey} = ?`, [...setValues, pkValue])
    }
  } else if (mode === 'upsert') {
    if (!primaryKey) {
...
export function getExportFileName(format: ExportFormat, tables: string[]): string {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-')
  const tablesStr = tables.length > 3 ? `${tables.length}-tables` : tables.join('-')
 ...
 */
export async function importData(data: string, options: ImportOptions): Promise<ImportResult> {
