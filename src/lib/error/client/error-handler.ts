...
export function getErrorSeverity(error: Error | AppError): ErrorSeverity {
  if (isUnifiedError(error)) {
    ...
    if (error.type === UnifiedErrorType.UNAUTHORIZED || 
        error.type === UnifiedErrorType.FORBIDDEN) {
      return "critical";
    }
    if (error.type === UnifiedErrorType.NETWORK_ERROR ||
        ...
        React.createElement("div", { className: "p-6 text-center" }, [
          ...
          React.createElement(
            "button",
            {
              ...
              className:
                "mt-4 px-4 py-2 bg-cyan-500 text-white rounded-lg hover:bg-cyan-600",
            },
            "刷新页面",
          ),
        ])
