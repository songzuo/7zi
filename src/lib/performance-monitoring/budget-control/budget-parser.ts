...
export class BudgetParser {
  ...
  private validateTiming(timing: unknown, path: string): string[] {
    ...
    if (!('metric' in t) || typeof t.metric !== 'string') {
      ...
      errors.push(
        `${path}.metric must be one of: ${VALID_METRICS.join(', ')} (got: ${t.metric})`
      );
    }
   ...
   */
  static generateBudgetJson(
    paths: string[] = ['/'],
    includeAllMetrics: boolean = true
  ): string {
    const budgets: Budget[] = paths.map((path) => ({
      ...
      timings: includeAllMetrics
        ...
        : [
            { metric: 'LCP' as const, budget: RECOMMENDED_BUDGETS.LCP, tolerance: DEFAULT_TOLERANCES.LCP },
            { metric: 'FID' as const, budget: RECOMMENDED_BUDGETS.FID, tolerance: DEFAULT_TOLERANCES.FID },
            { metric: 'CLS' as const, budget: RECOMMENDED_BUDGETS.CLS, tolerance: DEFAULT_TOLERANCES.CLS },
          ],
    }));
