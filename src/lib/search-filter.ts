 ...
 */
function fuzzyMatch(text: string, query: string, threshold = 1): {
  match: boolean;
  score: number;
  start: number;
} {
 ...
 * @returns 匹配结果
 */
function pinyinMatch(text: string, query: string): {
  match: boolean;
  score: number;
  start: number;
} {
 ...
 * @returns 过滤器选项列表
 */
export function extractFilterOptions<T extends object>(
  items: T[],
  field: keyof T
): FilterOption[] {
  // 早期退出
