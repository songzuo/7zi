...
export class RenderingTracker {
        ...
        const tbtObserver = new PerformanceObserver((list) => {
          for (const entry of list.getEntries()) {
            if (entry.duration > this.config.rendering.longTaskThreshold) {
              ...
              const currentLongTaskCount = this.metricsHistory[this.metricsHistory.length - 1]?.longTaskCount || 0;
              const currentLongTaskDuration = this.metricsHistory[this.metricsHistory.length - 1]?.longTaskDuration || 0;
              this.trackMetrics({
   ...
   */
  private detectIssues(metrics: Omit<RenderingMetrics, 'id' | 'timestamp' | 'issues'>): RenderingIssueType[] {
  ...
  calculateCLS(): number {
    ...
    for (const entry of this.layoutShifts) {
      ...
      sessionEntries = sessionEntries.filter(
        e => e.timestamp >= lastEntryTime - 1000
      );
  ...
  private identifyBottlenecks(): RenderingBottleneck[] {
    ...
    // Styling bottleneck
    const averageLongTaskDuration = this.longTasks.length > 0
   ...
   */
  private getComponentInvolved(type: RenderingIssueType, metrics: RenderingMetrics[]): string | undefined {
