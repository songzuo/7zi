...
export class APITracker {
  ...
  private calculateErrorRate(): ErrorRateAnalysis {
    ...
    // Detect spike
    const spikeDetected =
      trend === 'increasing' && overallErrorRate > this.config.api.errorRateThreshold * 2
  ...
  private calculateAveragePayload(requests: APIRequest[]): number {
    const withPayload = requests.filter(
      r => r.payloadSize !== undefined || r.responseSize !== undefined
    )
    if (withPayload.length === 0) return 0
    return (
      withPayload.reduce((sum, r) => sum + (r.payloadSize || r.responseSize || 0), 0) /
      withPayload.length
    )
  }
