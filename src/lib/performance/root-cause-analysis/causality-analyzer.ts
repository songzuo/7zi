...
export class CausalityAnalyzer {
  ...
  private getValueAtTime(
    ...
    const candidates = data.filter(p => Math.abs(p.timestamp - timestamp) <= tolerance)
      ...
      if (rootCauses.length > 0) {
        ...
        const multiHopChain: CausalChain = {
          ...
          timeline: {
            ...
            intervals: [...bestRootCause.timeline.intervals, ...immediateCause.timeline.intervals],
          },
          analysis: {
            method: 'rule-based',
            ...
            evidence: [...bestRootCause.analysis.evidence, ...immediateCause.analysis.evidence],
          },
        }
