...
import { DatabaseTracker, DatabaseQuery } from './database-tracker';
import { APITracker, APIRequest } from './api-tracker';
import { RenderingTracker, RenderingMetrics } from './rendering-tracker';
...
export class RootCauseAnalyzer {
  ...
  private identifyOptimizationOpportunities(entries: WaterfallEntry[]): OptimizationOpportunity[] {
    ...
    const criticalResources = entries.filter(e =>
      e.type === 'script' || e.type === 'stylesheet'
    );
  ...
  private calculateWaterfallSummary(
    entries: WaterfallEntry[],
    criticalPath: CriticalPathNode[]
  ): { totalDuration: number; criticalPathDuration: number; potentialImprovement: number; parallelizationScore: number } {
    const totalDuration = entries.length > 0
      ? Math.max(...entries.map(e => e.startTime + e.duration))
      : 0;

    const criticalPathDuration = criticalPath.length > 0
      ? criticalPath.reduce((sum, node) => sum + node.entry.duration, 0)
      : 0;

    const potentialImprovement = entries.length > 0
      ? Math.min(
          (entries.filter(e => e.duration > 500).length / entries.length) * 40,
          50
        )
      : 0;
    ...
    const parallelizationScore = entries.length > 0
      ? Math.round((overlapping.length / entries.length) * 100)
      : 100;
  ...
  traceSlowRequest(requestId: string): SlowRequestTrace {
    ...
    const relatedQueries = this.databaseTracker.getHistory().filter(q =>
      Math.abs(q.timestamp - request.timestamp) < 1000
    );
  ...
  private identifyRequestRootCauses(
    ...
    const slowQueries = relatedQueries.filter(q => q.duration > this.config.database.slowQueryThreshold);
    ...
    if (request.issues.includes('timeout')) {
      causes.push({
        ...
        fixRecommendations: [{
          ...
          actionItems: ['Profile the operation', 'Optimize bottlenecks', 'Increase timeout if needed'],
          complexity: 'medium',
   ...
   */
  private identifyCPUBottlenecks(longTasks: Array<{ duration: number; attribution?: string }>): ResourceBottleneck[] {
    ...
    if (totalBlockingTime > this.config.rendering.tbtThreshold) {
      bottlenecks.push({
        type: 'javascript',
        severity: totalBlockingTime > 600 ? 'critical' : totalBlockingTime > 300 ? 'high' : 'medium',
  ...
  private identifyMemoryBottlenecks(dbAnalysis: any, apiAnalysis: any): ResourceBottleneck[] {
    ...
    const largeResults = dbAnalysis.criticalIssues?.filter(
      (issue: any) => issue.type === 'large-result'
    ) || [];
   ...
   */
  private identifyNetworkBottlenecks(apiAnalysis: any): ResourceBottleneck[] {
    const bottlenecks: ResourceBottleneck[] = [];
   ...
   */
  private estimateMemoryUsage(dbAnalysis: any, apiAnalysis: any): number {
    const dbRows = dbAnalysis.queryStatistics?.topSlowQueries?.reduce(
      (sum: number, q: any) => sum + (q.rowCount || 0),
      0
    ) || 0;
   ...
   */
  private calculateAverageLatency(apiAnalysis: any): number {
  ...
  identifyHotPath(): HotPath[] {
    ...
    const dbAnalysis = this.databaseTracker.analyze();
    dbAnalysis.queryStatistics?.topSlowQueries?.slice(0, 5).forEach((query: DatabaseQuery, index: number) => {
      hotPaths.push({
        id: `hotpath-db-${index}`,
    ...
    longTasks.slice(0, 5).forEach((task, index: number) => {
      hotPaths.push({
        ...
        recommendations: ['Optimize JavaScript execution', 'Use Web Workers for CPU-intensive work']
      });
    });
  ...
  private generateQueryHotPathRecommendations(query: DatabaseQuery): string[] {
    ...
    return recommendations.length > 0 ? recommendations : ['Review query for optimization opportunities'];
  }
  ...
  private generateAPIHotPathRecommendations(request: APIRequest): string[] {
    ...
    return recommendations.length > 0 ? recommendations : ['Review endpoint for optimization opportunities'];
  }
  ...
  generateReport(): AnalysisReport {
    ...
    const metrics: PerformanceMetrics = {
      ...
      cls: resourceAnalysis.rendering.metrics.reduce((sum: number, m: any) => sum + (m.cls || 0), 0),
      fid: resourceAnalysis.rendering.metrics[resourceAnalysis.rendering.metrics.length - 1]?.fid,
      tbt: resourceAnalysis.rendering.metrics.reduce((sum: number, m: any) => sum + (m.tbt || 0), 0),
      ...
      totalBlockingTime: resourceAnalysis.rendering.metrics.reduce(
        (sum: number, m: any) => sum + m.longTaskDuration,
        0
      )
    };
  ...
  private compileRootCauses(resourceAnalysis: ResourceAnalysis, hotPaths: HotPath[]): RootCause[] {
    ...
    resourceAnalysis.database.criticalIssues.forEach((issue: any) => {
      causes.push({
        id: `cause-${issue.id}`,
        type: 'database',
        ...
        evidence: issue.affectedQueries.map((q: any) => q.query.substring(0, 50)),
        impact: {
    ...
    // Add API root causes
    resourceAnalysis.api.criticalIssues.forEach((issue: any) => {
      causes.push({
        id: `cause-${issue.id}`,
        type: 'api',
        severity: issue.severity,
  ...
  private calculateDatabaseSeverity(queries: DatabaseQuery[]): Severity {
    ...
    const level: SeverityLevel = avgDuration > 5000 ? 'critical' : avgDuration > 2000 ? 'high' : 'medium';
   ...
   */
  trackDatabaseQuery(query: Parameters<DatabaseTracker['trackQuery']>[0]): ReturnType<DatabaseTracker['trackQuery']> {
   ...
   */
  trackAPIRequest(request: Parameters<APITracker['trackRequest']>[0]): ReturnType<APITracker['trackRequest']> {
   ...
   */
  trackRenderingMetrics(metrics: Parameters<RenderingTracker['trackMetrics']>[0]): ReturnType<RenderingTracker['trackMetrics']> {
