...
export class CorrelationEngine {
  ...
  analyzeCorrelations(): Correlation[] {
    ...
    const filtered = correlations.filter(c => c.strength >= this.config.minCorrelationStrength)
      ...
      if (relatedEvents.length >= 2) {
        const correlation: Correlation = {
          ...
          timeframe: {
            ...
            duration:
              relatedEvents[relatedEvents.length - 1].timestamp - relatedEvents[0].timestamp,
          },
          rootCauseHypothesis:
            'Related events suggest a common trigger or shared resource bottleneck',
        }
  ...
  private calculateCausalStrength(event1: AnomalyEvent, event2: AnomalyEvent): number {
    ...
    const timeDiff = event2.timestamp - event1.timestamp
    strength +=
      Math.max(0, 1 - timeDiff / this.config.maxTimeBetweenEvents) * this.config.causalWeight
  ...
  private calculateContextSimilarity(context1: EventContext, context2: EventContext): number {
  ...
  private findRelatedCorrelations(correlation: Correlation): Correlation[] {
    ...
    for (const other of this.correlations.values()) {
      ...
      const eventOverlap = correlation.events.some(e => other.events.some(oe => oe.id === e.id))

      // Check for similar timeframes
      const timeOverlap =
        Math.min(correlation.timeframe.end, other.timeframe.end) -
          Math.max(correlation.timeframe.start, other.timeframe.start) >
        0
  ...
  private determinePrimaryCorrelation(correlations: Correlation[]): Correlation {
    ...
    return correlations.reduce((max, curr) => (curr.strength > max.strength ? curr : max))
  }
  ...
  private generateGroupSummary(correlations: Correlation[], primary: Correlation): string {
    const totalEvents = new Set(correlations.flatMap(c => c.events.map(e => e.id))).size
  ...
  private inferRootCause(correlations: Correlation[], primary: Correlation): string {
    ...
    return Object.entries(frequencyMap).sort(([, a], [, b]) => b - a)[0][0]
  }
