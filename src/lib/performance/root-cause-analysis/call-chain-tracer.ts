...
export interface CallChainTreeNode {
  id: string
  name: string
  type: CallNodeType
  duration: number
  status: 'success' | 'error' | 'timeout'
  children: CallChainTreeNode[]
}

export interface CallChainTree {
  id: string
  traceId: string
  duration: number
  status: 'error' | 'success' | 'timeout' | 'partial'
  root: CallChainTreeNode | null
}
...
export class CallChainTracer {
  ...
  getChain(chainId: string): CallChain | null {
    return this.activeChains.get(chainId) || this.completedChains.get(chainId) || null
  }
  ...
  private calculateChainSummary(chain: CallChain): CallChainSummary {
    ...
    const slowest = nodes.reduce((max, node) => (node.duration > max.duration ? node : max))
   ...
   */
  private identifyBottlenecks(chain: CallChain, summary: CallChainSummary): CallBottleneck[] {
   ...
   */
  private calculateBottleneckSeverity(duration: number, threshold: number): SeverityLevel {
   ...
   */
  getChainTree(chainId: string): CallChainTree | null {
    const chain = this.getChain(chainId)
    if (!chain) {
      return null
    }

    const toTree = (nodeId: string): CallChainTreeNode | null => {
      const node = chain.nodes.get(nodeId)
      if (!node) {
        return null
      }
      ...
      return {
        ...
        children: node.children
          ...
          .filter((child): child is CallChainTreeNode => child !== null),
      }
    }
