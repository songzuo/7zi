...
describe('Alert System Integration', () => {
  it('should handle complete alert lifecycle', async () => {
    ...
    const alert = await alerter.createAlert(
      createPerformanceAlert('High CPU Usage', 'CPU usage exceeded 90%', 'warning', {
        category: 'resource',
        source: 'system',
        metric: 'cpu-usage',
        currentValue: 92,
...
describe('Edge Cases', () => {
  ...
  it('should throw error when disabled', async () => {
    ...
    await expect(alerter.createAlert(createTestAlert())).rejects.toThrow('Alerting is disabled')
  })
