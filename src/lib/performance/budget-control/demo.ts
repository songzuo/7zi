...
async function demoBudgetChecker() {
  ...
  if (result2.violations.length > 0) {
    ...
    for (const violation of result2.violations) {
      console.log(
        `     - ${violation.metric}: ${violation.actual.toFixed(0)}ms (budget: ${violation.budget}ms, ${violation.percentOver.toFixed(1)}% over) [${violation.severity}]`
      )
    }
  }
  console.log()
...
async function demoBudgetConfig() {
  ...
  for (const timing of budget?.timings || []) {
    ...
    console.log(
      `     - ${timing.metric}: ${timing.budget}${timing.metric === 'CLS' ? '' : 'ms'} (threshold: ${threshold.toFixed(1)}${timing.metric === 'CLS' ? '' : 'ms'})`
    )
  }
  console.log()
  ...
  console.log(
    `   LCP: ${DEFAULT_THRESHOLDS.LCP.budget}ms (${DEFAULT_THRESHOLDS.LCP.tolerance * 100}% tolerance)`
  )
  console.log(
    `   FID: ${DEFAULT_THRESHOLDS.FID.budget}ms (${DEFAULT_THRESHOLDS.FID.tolerance * 100}% tolerance)`
  )
  console.log(
    `   CLS: ${DEFAULT_THRESHOLDS.CLS.budget} (${DEFAULT_THRESHOLDS.CLS.tolerance * 100}% tolerance)`
  )
  console.log()
}
