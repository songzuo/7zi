  ...
  private mergeWithDefaults(
    ...
    for (const budget of config.budgets) {
      ...
      if (existingIndex >= 0) {
        ...
        for (const defaultTiming of defaults.budgets.find(b => b.path === budget.path)?.timings ||
          []) {
...
export async function checkMetricsWithinBudget(
  ...
  passed: boolean
  violations: Array<{ metric: string; actual: number; budget: number; percentOver: number }>
}> {
  const manager = new BudgetConfigManager(options)
