...
export class AdvancedSearchManager<T extends Record<string, unknown>> {
   ...
   */
  addToHistory(query: string, resultCount: number, target: SearchTarget = 'all'): void {
  ...
  getRecentHistory(limit: number): AutocompleteSuggestion[] {
    return this.searchHistory.slice(0, limit).map(entry => ({
      text: entry.query,
      type: 'history' as const,
      ...
      if (result.matches) {
        for (const match of result.matches) {
          ...
          if (config?.includeHighlights && match.indices && match.value) {
            ...
            const textHighlights = this.generateHighlights(value, match.indices, 40)
            highlights.push({
              field: match.key || '',
   ...
   * Generate cache key
   */
  private generateCacheKey(query: string, indices: string[], config?: SearchConfig): string {
 ...
 */
export function buildSearchQuery(text: string, filters: Record<string, string>): string {
