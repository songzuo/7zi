...
└── __tests__/
    ├── template-engine.test.ts   # 模板引擎测试 (14 个测试)
    ├── data-aggregator.test.ts   # 数据聚合器测试 (13 个测试)
    ├── nlg-processor.test.ts     # NLG 处理器测试 (15 个测试)
    └── integration.test.ts       # 集成测试 (22 个测试)
...
1. **template-engine.test.ts** ✅ 14/14 通过
   - 模板获取和注册
   - 变量插值功能
   - 条件渲染
   - 循环渲染
...
2. **data-aggregator.test.ts** ✅ 13/13 通过
   - 各类型数据聚合
   - 自定义时间范围
   - 缓存机制
   - 错误处理

3. **nlg-processor.test.ts** ✅ 15/15 通过
   - 报表生成
   - 多语言支持
   - 语气风格
   - 洞察生成
