...
export { undoRedo, undoRedoImpl } from './middleware'

export type { UndoRedoStoreActions } from './middleware'
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (canUndo && undoShortcut) {
        ...
        const isUndo = shortcuts.some(shortcut => matchesShortcut(event, shortcut))
        if (isUndo) {
      ...
      if (canRedo && redoShortcut) {
        ...
        const isRedo = shortcuts.some(shortcut => matchesShortcut(event, shortcut))
        if (isRedo) {
          event.preventDefault()
          redo()
        }
