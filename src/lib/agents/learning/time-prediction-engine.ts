...
export class TimePredictionEngine {
  ...
  private ruleBasedPredict(input: PredictionInput): TimePrediction {
    ...
    const interval: [number, number] = [Math.max(1, estimatedTime * 0.7), estimatedTime * 1.3]
  ...
  private statisticalPredict(input: PredictionInput): TimePrediction {
    ...
    const interval: [number, number] = [Math.max(1, avgTime - stdDev), avgTime + stdDev]
  ...
  private async adaptivePredict(input: PredictionInput): Promise<TimePrediction> {
    ...
    return {
      ...
      confidenceInterval: [
        Math.round(
          Math.min(statisticalResult.confidenceInterval[0], ruleBasedResult.confidenceInterval[0])
        ),
        Math.round(
          Math.max(statisticalResult.confidenceInterval[1], ruleBasedResult.confidenceInterval[1])
        ),
      ],
      factors,
   ...
   */
  recordPrediction(agentId: AgentId, predictedTime: number, actualTime: number): void {
  ...
  getStats(): {
    ...
    return {
      ...
      totalHistories: Array.from(this.agentHistories.values()).reduce(
        (sum, h) => sum + h.tasks.length,
        0
      ),
