...
export class LearningOptimizer {
  ...
  getAggregatedStats(period: 'hour' | 'day' | 'week' | 'month'): AggregatedStats {
    ...
    const avgAgentUtilization =
      agentUtilizations.length > 0
  ...
  private async detectTimeOfDayPatterns(): Promise<void> {
    ...
    for (const record of this.history) {
      ...
      stats.successRate =
        (stats.successRate * (stats.count - 1) + (record.status === 'completed' ? 1 : 0)) /
        stats.count
  ...
  private async detectTaskSequencePatterns(): Promise<void> {
    ...
    for (const [, records] of agentRecords) {
      for (let i = 1; i < records.length; i++) {
        ...
        transition.count++
        transition.avgTimeRatio =
          (transition.avgTimeRatio * (transition.count - 1) + timeRatio) / transition.count
    ...
    const significantTransitions = Array.from(typeTransitions.entries()).filter(
      ([, t]) => t.count >= 5 && (t.avgTimeRatio < 0.8 || t.avgTimeRatio > 1.2)
    )
  ...
  private async detectAgentPreferencePatterns(): Promise<void> {
    const agentTaskStats = new Map<
      AgentId,
      Map<TaskType, { count: number; avgTime: number; success: number }>
    >()
    ...
    for (const [agentId, taskStats] of agentTaskStats) {
      ...
      const strengths = stats
        .filter(
          ([, t]) => t.count >= 3 && t.avgTime < avgTime * 0.85 && t.success / t.count > avgSuccess
        )
        .map(([type]) => type)
      ...
      const weaknesses = stats
        .filter(
          ([, t]) => t.count >= 3 && t.avgTime > avgTime * 1.15 && t.success / t.count < avgSuccess
        )
        .map(([type]) => type)
  ...
  private async calculateMetrics(): Promise<void> {
    ...
    // Throughput
    const timeRange =
      Math.max(...this.history.map(r => r.createdAt)) -
    ...
    this.metrics.agentUtilizationVariance =
