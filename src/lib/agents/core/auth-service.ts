...
): Promise<{ agent: Agent; plainApiKey: string }> {
...
export async function verifyAgentToken(
  token: string
): Promise<{ agentId: string; role: string; permissions: string[] } | null> {
  try {
    const secret = new TextEncoder().encode(getJwtSecret())
...
function getDefaultPermissions(role: AgentRole): string[] {
  ...
  switch (role) {
    ...
    case AgentRole.EXECUTOR:
      return [...basePermissions, 'write:tasks', 'update:tasks', 'complete:tasks']
    ...
    case AgentRole.DESIGNER:
      return [...basePermissions, 'write:tasks', 'update:tasks', 'access:design']

    case AgentRole.TESTER:
      return [...basePermissions, 'write:tests', 'access:reports', 'access:logs']

    case AgentRole.FINANCE:
      return [...basePermissions, 'manage:wallet', 'access:reports', 'access:transactions']
