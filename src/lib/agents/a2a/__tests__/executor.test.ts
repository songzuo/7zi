...
import { SimpleEventBus, SevenZiExecutor, createSevenZiExecutor, RequestContext } from '../executor'
...
describe('SimpleEventBus', () => {
  ...
  describe('publish', () => {
    ...
    it('should throw error when publishing after finished() is called', () => {
      ...
      expect(() => eventBus.publish(task)).toThrow(
        'Cannot publish events after finished() has been called'
      )
    })
  })
...
describe('SevenZiExecutor', () => {
    ...
    it('should handle messages with non-text parts', async () => {
      const context: RequestContext = {
        ...
        userMessage: {
          ...
          messageId: 'msg-1',
          role: 'user',
          parts: [{ kind: 'file', file: { name: 'test.txt', mimeType: 'text/plain' } }],
          createdAt: new Date().toISOString(),
        },
      }
    ...
    it('should handle messages with undefined text parts', async () => {
      const context: RequestContext = {
        ...
        userMessage: {
          ...
          messageId: 'msg-1',
          role: 'user',
          parts: [{ kind: 'text' as const }, { kind: 'text', text: undefined }],
          createdAt: new Date().toISOString(),
        },
      }
  ...
  describe('state transitions', () => {
    ...
    it('should ensure final state is always terminal', async () => {
      ...
      expect(['completed', 'failed', 'canceled', 'rejected']).toContain(
        lastStatusUpdate?.status.state
      )
    })
  })
  ...
  describe('timeout scenarios', () => {
    it('should handle execution timeout', async () => {
      ...
      await expect(
        Promise.race([slowExecutor.execute(context, eventBus), timeoutPromise])
