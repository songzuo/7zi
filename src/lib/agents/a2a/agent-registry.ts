...
export class InMemoryAgentRegistry implements AgentRegistry {
  ...
  getByCapability(capability: string): AgentRegistration[] {
    return this.getAll().filter(agent => this.agentCapabilities.get(agent.id)?.has(capability))
  }
  ...
  getBySkill(skill: string): AgentRegistration[] {
    return this.getAll().filter(agent => this.agentSkills.get(agent.id)?.has(skill))
  }
  ...
  findBestAgent(options: {
    ...
    if (options.capabilities && options.capabilities.length > 0) {
      candidates = candidates.filter(agent =>
        options.capabilities!.every(cap => this.agentCapabilities.get(agent.id)?.has(cap))
      )
    }
    ...
    if (options.skills && options.skills.length > 0) {
      candidates = candidates.filter(agent =>
        options.skills!.every(skill => this.agentSkills.get(agent.id)?.has(skill))
      )
    }
