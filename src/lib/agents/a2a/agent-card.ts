...
export function createAgentCard(baseUrl: string = 'http://localhost:3000'): AgentCard {
  ...
  return {
    name: '7zi Agent',
    description:
      'A2A-compliant AI agent for intelligent task processing, chat, and analysis. Part of the 7zi ecosystem.',
    ...
    skills: [
      ...
      },
      {
        ...
        examples: ['Analyze this document', 'What are the key points?', 'Summarize this for me'],
      ...
      },
      {
        ...
        examples: ['Create a new task', 'Check my task status', 'Cancel my task'],
        inputModes: ['text/plain'],
        outputModes: ['text/plain', 'application/json'],
      },
    ],
  ...
  return {
    ...
    skills: [
      ...baseCard.skills,
      {
        ...
        examples: ['List all tasks', 'Cleanup old tasks', 'View system metrics'],
        inputModes: ['text/plain', 'application/json'],
...
export function getAgentCard(baseUrl?: string): AgentCard {
  if (!defaultAgentCard || baseUrl) {
    defaultAgentCard = createAgentCard(
