...
export class InMemoryTaskStore implements TaskStore {
  ...
  listTasks(options: {
    ...
    const nextPageToken =
      endIndex < totalSize ? Buffer.from(String(endIndex)).toString('base64') : ''
  ...
  getHighestPriorityTasks(limit: number = 10): TaskWithPriority[] {
    const allTasks = Array.from(this.taskWithPriority.values()).filter(t =>
      ['submitted', 'working'].includes(t.status.state)
    )
   ...
   */
  updateAsyncTaskProgress(taskId: string, progress: number, currentStep?: string): boolean {
    const status = this.asyncTaskStatus.get(taskId)
