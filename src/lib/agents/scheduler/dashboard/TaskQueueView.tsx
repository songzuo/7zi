...
function TaskCard({ task, agents, onReassign, onCancel, onTaskClick, selected }: TaskCardProps) {
  ...
  const availableAgents = agents.filter(
    a => a.availability && a.capabilities.taskTypes.includes(task.type)
  )

  return (
    <div
      className={`cursor-pointer rounded-lg border-2 p-4 transition-all hover:shadow-md ${selected ? 'border-blue-500 ring-2 ring-blue-500' : priorityColor.border} ${isOverdue ? 'bg-red-50 dark:bg-red-900/20' : 'bg-white dark:bg-gray-800'} `}
    ...
    >
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span
            className={`rounded-md px-2 py-1 text-xs font-semibold ${priorityColor.bg} ${priorityColor.text} border ${priorityColor.border}`}
          >
            {task.priority.toUpperCase()}
          </span>
          <span
            className={`rounded-md px-2 py-1 text-xs font-medium ${statusColor.bg} ${statusColor.text}`}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          {isOverdue && (
            <span className="text-xs font-semibold text-red-500" title="Overdue">
              ⚠️ OVERDUE
            </span>
          )}
          {isDeadlineSoon && !isOverdue && (
            <span className="text-xs font-semibold text-orange-500" title="Deadline within 1 hour">
              ⏰ SOON
            </span>
          )}
        </div>
      ...
      <div className="mb-3 grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-gray-400">
        ...
        {deadline && (
          <div
            className={`col-span-2 ${isOverdue ? 'font-semibold text-red-600 dark:text-red-400' : ''}`}
          >
        ...
        {task.dependencies.length > 0 && (
          <div className="col-span-2">
            <span className="font-medium">Dependencies:</span> {task.dependencies.length} task
      ...
      <div className="flex items-center gap-2 border-t border-gray-200 pt-2 dark:border-gray-700">
        {task.status === 'pending' || task.status === 'assigned' ? (
          <>
            ...
            <button
              className="rounded px-3 py-1 text-sm font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
              onClick={e => {
                e.stopPropagation()
                onCancel(task.id)
              }}
...
export function TaskQueueView() {
  const { tasks, agents, selectedTaskId, selectTask, manualAssign, isLoading, error, refresh } =
    useSchedulerStore()
  ...
  const handleReassign = useCallback(
    async (taskId: string, newAgentId: string) => {
      try {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="mb-2 text-2xl font-bold text-gray-900 dark:text-white">Task Queue</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          {(filterStatus !== 'all' ||
            ...
            <button
              ...
              className="mt-6 rounded-lg px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"
            >
              Clear Filters
            </button>
          )}
      ...
      {sortedTasks.length === 0 ? (
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center dark:border-gray-700 dark:bg-gray-800">
          ...
          <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">No tasks found</h3>
          <p className="text-gray-600 dark:text-gray-400">
          ...
          {(Object.keys(groupedTasks) as TaskPriority[]).map(priority => {
            ...
            return (
              <div key={priority}>
                <div className="mb-3 flex items-center gap-2">
                  <span
                    className={`rounded-md px-3 py-1 text-sm font-semibold ${priorityColor.bg} ${priorityColor.text} border ${priorityColor.border}`}
                  >
