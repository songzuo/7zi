...
export class TaskMatcher {
  ...
  private hasRequiredCapabilities(agent: AgentCapability, required: string[]): boolean {
    ...
    const agentCapabilities = new Set(agent.capabilities.techStack.map(t => t.toLowerCase()))

    return required.every(
      req =>
        agentCapabilities.has(req.toLowerCase()) ||
        agent.capabilities.specializations?.some(s => s.toLowerCase().includes(req.toLowerCase()))
    )
  }
  ...
  calculateCapabilityScore(agent: AgentCapability, task: Task): number {
    ...
    if (task.requiredCapabilities.length > 0) {
      ...
      reasons.push(
        `Matched ${matchedCount}/${task.requiredCapabilities.length} required capabilities`
      )
    } else {
  ...
  getAlternativeCandidates(candidates: MatchResult[], count: number = 3): string[] {
    return candidates.slice(1, count + 1).map(c => c.agentId)
  }
