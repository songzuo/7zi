  ...
  describe("工作流验证", () => {
    ...
    it("应该检测不存在的边连接节点", () => {
      const invalidWorkflow = {
        ...
        edges: [
          { ...workflow.edges[0] },
          { id: "bad-edge", source: "non-existent", target: workflow.nodes[1].id, type: EdgeType.SEQUENCE },
        ],
      };
      const result = engine.validateWorkflow(invalidWorkflow);
      expect(result.valid).toBe(false);
  ...
  describe("工作流验证错误", () => {
    ...
    it("应该检测边连接到不存在的目标节点", () => {
      const invalidWorkflow = {
        ...workflow,
        edges: [
          { id: "bad-edge", source: workflow.nodes[0].id, target: "non-existent", type: EdgeType.SEQUENCE },
        ],
      };
