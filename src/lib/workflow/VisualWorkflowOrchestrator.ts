...
export interface NodeExecutorHandler {
   ...
   * 执行节点
   */
  execute(node: WorkflowNode, context: ExecutionContext): Promise<OrchestratorExecutionResult>
...
export class VisualWorkflowOrchestrator {
  ...
  private async executeNode(
    ...
    try {
      ...
      if (node.type === NodeType.CONDITION) {
        ...
        // 并行节点：并行执行所有分支
        await Promise.all(nextNodes.map(nextNode => this.executeNode(workflow, nextNode, instance)))
      } else {
        // 顺序执行
  ...
  getStatistics(workflowId: string): {
    ...
    const instances = this.getAllInstances().filter(i => i.workflowId === workflowId)
    ...
    return {
      ...
      avgDuration:
        instances.length > 0
          ? instances.reduce((sum, i) => sum + (i.metadata.duration || 0), 0) / instances.length
          : 0,
    }
  }
}
