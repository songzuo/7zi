...
export class WaitNodeExecutor implements NodeExecutor {
  ...
  validate(node: WorkflowNode): { valid: boolean; errors: string[] } {
    ...
    if (!node.waitConfig) {
      ...
      if (!node.waitConfig.duration && !node.waitConfig.waitForEvent) {
   ...
   * 定时等待
   */
  private async waitWithDuration(duration: number, context: ExecutionContext): Promise<void> {
   ...
   * 等待事件（模拟）
   */
  private async waitForEvent(eventName: string, context: ExecutionContext): Promise<void> {
