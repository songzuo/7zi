 ...
 * Render detail row
 */
function renderDetailRow(label: string, value: string, customValue?: string): string {
  return `
...
function renderMetadata(metadata: Record<string, unknown> | undefined): string {
  ...
  for (const [key, value] of Object.entries(metadata)) {
    rows += renderDetailRow(key, typeof value === 'object' ? JSON.stringify(value) : String(value))
  }
