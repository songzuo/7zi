...
export class CreditScoreService {
  ...
  constructor(creditRepo?: ICreditScoreRepository, weights?: CreditWeights) {
  ...
  async recordTaskCompletion(
    ...
    const currentCompletedTasks = Math.round(
      totalTasks * (currentScore.factors.taskCompletionRate / 100)
    )
    ...
    const currentOnTimeDeliveries = Math.round(
      totalDeliveries * (currentScore.factors.onTimeDeliveryRate / 100)
    )
    ...
    const newResponseSpeed = Math.round(currentResponseSpeed * 0.7 + responseSpeedScore * 0.3)
  ...
  async recordUserRating(
    ...
    // 使用0.3的权重平滑更新
    const newRatingScore = Math.round(currentRating * 0.7 + ratingScore * 0.3)
