 ...
 * Stream handler
 */
export type StreamHandler = (event: SSEEvent, context: StreamingContext) => Promise<void> | void
...
export class MCPStreamServer extends EventEmitter {
   ...
   */
  createStream(
    context: Omit<StreamingContext, 'startedAt' | 'streamId'> & { streamId?: string }
  ): string {
   ...
   * Send tool result
   */
  async sendToolResult(streamId: string, toolName: string, result: unknown): Promise<void> {
    await this.sendEvent(streamId, {
      event: 'tool/result',
      data: {
        toolName,
...
export class StreamingToolExecutor {
  ...
  async executeWithStreaming(
    ...
    const { toolName, arguments: args, onProgress, signal, timeout } = options
