...
export class PerformanceBudget {
  ...
  checkMetric(
    ...
    for (const budget of this.budgets.values()) {
      ...
      for (const threshold of budget.thresholds) {
        ...
        const isRelated = thresholdLower.includes(metricLower) || metricLower.includes(thresholdLower);
   ...
   */
  checkMetrics(metrics: Record<string, { value: number; unit: 'ms' | 'bytes' | 'count' | 'score' }>): BudgetReport {
    const results: BudgetResult[] = [];
