...
function aggregateTrend(
  ...
  const confidence: 'high' | 'medium' | 'low' = 
    filteredSnapshots.length >= 5 && timeSpread >= timeWindow * 0.75 ? 'high' :
    filteredSnapshots.length >= 3 && timeSpread >= timeWindow * 0.5 ? 'medium' :
    'low';
...
function calculateTrendVelocity(
  ...
  const valueChange = (latest.metrics[metricName] as number) - (earliest.metrics[metricName] as number);
...
describe('aggregateTrend', () => {
  ...
  it('should aggregate trend for a single metric', () => {
    const result = aggregateTrend('tasksPerHour', snapshots);
  ...
  it('should handle negative trends', () => {
    ...
    const result = aggregateTrend('tasksPerHour', decreasingSnapshots);
  ...
  it('should detect stable trend for small changes', () => {
    ...
    const result = aggregateTrend('tasksPerHour', stableSnapshots);
...
describe('aggregateMultipleTrends', () => {
  ...
  it('should aggregate trends for multiple metrics', () => {
    ...
    const results = aggregateMultipleTrends(snapshots, metricNames);
...
describe('Trend Aggregation Integration', () => {
  ...
  it('should provide comprehensive trend analysis', () => {
    ...
    const trends = aggregateMultipleTrends(snapshots, [
      'tasksPerHour',
      'taskSuccessRate',
      'throughput'
    ]);
    ...
    // Calculate velocity
    const velocity = calculateTrendVelocity('tasksPerHour', snapshots);
