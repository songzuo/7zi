...
export class WebSocketMonitor {
  ...
  private checkLatencyThreshold(namespace: string, latency: number): void {
    ...
    if (latency >= this.config.latencyCriticalThreshold) {
      this.log(`[${namespace}] CRITICAL: Latency ${latency}ms exceeds threshold ${this.config.latencyCriticalThreshold}ms`);
      ...
      this.log(`[${namespace}] WARNING: Latency ${latency}ms exceeds threshold ${this.config.latencyWarningThreshold}ms`);
    }
  }
