...
describe('calculateTrend', () => {
  ...
  describe('edge cases', () => {
    it('should handle negative values', () => {
      // Negative values: from -100 to -50 means moving towards 0 (increase)
    ...
    it('should handle decimal values', () => {
      const result = calculateTrend(0.15, 0.1)
      expect(result).toBe('up')
    })
  })
})
...
describe('calculateEfficiencyTrend', () => {
  ...
  it('should calculate negative efficiency trend', () => {
    const decreasingMetrics: TeamEfficiencyMetrics = {
      ...
      agentsIdle: 2,
      agentsWorking: 8,
      tasksPerHour: 60, // Decreased from 80
      averageTaskDuration: 45, // Increased from 35 (inverse metric, worse)
      taskSuccessRate: 80, // Decreased from 90
      throughput: 600, // Decreased from 800
      queueSize: 10, // Increased (worse)
    }
...
describe('Trend Calculation Integration', () => {
  ...
  it('should calculate all metrics trends correctly', () => {
    ...
    const tasksTrend = calculateTrendData(
      currentMetrics.tasksPerHour,
      previousMetrics.tasksPerHour,
      'en'
    )
    const successTrend = calculateTrendData(
      currentMetrics.taskSuccessRate,
      previousMetrics.taskSuccessRate,
      'en'
    )
    const throughputTrend = calculateTrendData(
      currentMetrics.throughput,
      previousMetrics.throughput,
      'en'
    )
    const durationTrend = calculateTrendData(
      currentMetrics.averageTaskDuration,
      previousMetrics.averageTaskDuration,
      'en'
    )
