...
export function useRenderPerformance(componentName: string): {
  ...
  useEffect(() => {
    ...
    if (duration > config.warning) {
      console.warn(
        `[Performance] Slow render: ${componentNameRef.current} took ${duration.toFixed(1)}ms`
      )
    }
  }, [renderTime])
  ...
  const isSlowRender =
    renderTime !== null && renderTime > CUSTOM_METRICS_CONFIG.rendering.componentRenderTime.warning
...
export function useMemoryUsage(): MemoryInfo & { isHighMemory: boolean } {
  ...
  const isHighMemory =
    memoryInfo.usedJSHeapSize !== null && memoryInfo.usedJSHeapSize > config.warning
