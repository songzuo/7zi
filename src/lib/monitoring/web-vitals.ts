...
export function optimizeLCP() {
  ...
  const lcpImage = document.querySelector(
    'img[data-lcp], .hero-image img, main img'
  ) as HTMLImageElement
...
function logToConsole(metric: Metric, config: WebVitalsConfig) {
  ...
  const emoji =
    metric.rating === 'poor' ? '🚨' : metric.rating === 'needs-improvement' ? '⚠️' : '✅'
  const method =
    metric.rating === 'poor' ? 'error' : metric.rating === 'needs-improvement' ? 'warn' : 'log'
