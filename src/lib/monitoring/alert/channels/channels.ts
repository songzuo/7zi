...
export class AlertChannelManager {
   ...
   */
  registerChannels(
    configs: Array<{ id: string; config: AlertChannelConfig }>,
  ): void {
  ...
  ): Array<{ id: string; config: AlertChannelConfig }> {
    ...
    for (const rule of this.routingRules) {
      if (this.matchesRule(rule.match, severity, tags, alertName)) {
        return rule.channels.filter((channel) =>
          this.hasEnabledChannel(channel),
        );
      }
    }
  ...
  private matchesRule(
    ...
    if (
      matcher.severity &&
      matcher.severity.length > 0 &&
      !matcher.severity.includes(severity)
    ) {
      return false;
    }
    ...
    if (
      ...
      !matcher.tags.some((tag) => tags.includes(tag))
    ) {
      return false;
    }
  ...
  static fromEnvironment(): AlertChannelManager {
    ...
    if (process.env.TELEGRAM_ENABLED === "true") {
      manager.registerChannel("default-telegram", {
        ...
        parseMode: (process.env.TELEGRAM_PARSE_MODE as any) || "Markdown",
    ...
    if (process.env.WEBHOOK_ENABLED === "true") {
      manager.registerChannel("default-webhook", {
        ...
        method: (process.env.WEBHOOK_METHOD as any) || "POST",
    ...
    if (process.env.FEISHU_ENABLED === "true") {
      manager.registerChannel("default-feishu", {
        ...
        msgType: (process.env.FEISHU_MSG_TYPE as any) || "interactive",
        retryAttempts: 3,
        timeoutMs: 10000,
      });
    }
    ...
    if (process.env.DINGTALK_ENABLED === "true") {
      manager.registerChannel("default-dingtalk", {
        ...
        msgType: (process.env.DINGTALK_MSG_TYPE as any) || "markdown",
        retryAttempts: 3,
        timeoutMs: 10000,
      });
    }
...
<body>
  ...
  private generateSlackAttachments(
    payload: Record<string, unknown>,
...
export function createChannelManager(
  config?: { channels?: Record<string, AlertChannelConfig>; routingRules?: ChannelRoutingRule[] },
): AlertChannelManager {
  const manager = new AlertChannelManager();
  if (config) {
