...
export class PerformanceWaterfall {
  ...
  estimateFCP(): FirstContentfulPaintData {
    ...
    return {
      ...
      blockingResources: blockingResources.filter((r) => 
  ...
  analyzeRenderBlocking(): {
    ...
    const blockingStyles = blockingResources.filter((r) => r.initiatorType === 'link' && r.name.includes('css'));
  ...
  private calculateTimingBreakdown(): {
    ...
    processing = Math.max(0, this.resources.reduce((sum, r) => sum + r.duration, 0) - totalNetworkTime);
