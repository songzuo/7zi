...
export class PerformanceRootCauseAnalyzer {
  ...
  private diagnoseFCP(fcp: number): SlowPageDiagnosis {
    ...
    const severity = fcp <= threshold.good ? 'good' 
      : fcp <= threshold.needsImprovement ? 'needs-improvement' 
      : 'poor';
  ...
  private diagnoseLCP(lcp: number, ttfb: number): SlowPageDiagnosis {
    ...
    const severity = lcp <= threshold.good ? 'good' 
      : lcp <= threshold.needsImprovement ? 'needs-improvement' 
      : 'poor';
  ...
  private diagnoseCLS(cls: number): SlowPageDiagnosis {
    ...
    const severity = cls <= threshold.good ? 'good' 
      : cls <= threshold.needsImprovement ? 'needs-improvement' 
      : 'poor';
  ...
  private diagnoseINP(inp: number): SlowPageDiagnosis {
    ...
    const severity = inp <= threshold.good ? 'good' 
      : inp <= threshold.needsImprovement ? 'needs-improvement' 
      : 'poor';
  ...
  detectMemoryLeak(currentMetrics: MemoryMetrics): MemoryLeakIndication {
    ...
    if (growthRate > MEMORY_THRESHOLDS.growthRateCritical || usageRatio > MEMORY_THRESHOLDS.usageRatioCritical) {
      severity = 'critical';
      detected = true;
    } else if (growthRate > MEMORY_THRESHOLDS.growthRateWarning || usageRatio > MEMORY_THRESHOLDS.usageRatioWarning) {
      severity = 'high';
      detected = true;
  ...
  diagnoseRenderIssues(performanceEntries: PerformanceEntry[]): RenderIssue[] {
    ...
    const layoutShifts = performanceEntries.filter(
  ...
  private determineOverallHealth(
    ...
    const hasMemoryLeak = memoryAnalysis.severity === 'critical' || memoryAnalysis.severity === 'high';
    ...
    if (hasCriticalDiagnosis || hasMemoryLeak || hasCriticalBottlenecks || hasCriticalRenderIssues) {
      return 'critical';
    }
  ...
  private generatePriorityActions(
    ...
    if (memoryAnalysis.detected && (memoryAnalysis.severity === 'high' || memoryAnalysis.severity === 'critical')) {
      actions.push({
        rank: rank++,
        category: 'immediate',
    ...
    for (const bottleneck of networkBottlenecks.filter((b) => b.severity === 'high' || b.severity === 'critical')) {
      actions.push({
        rank: rank++,
        category: 'short-term',
  ...
  private generateSummary(
    ...
    if (diagnoses.length > 0) {
      ...
      if (poorCount > 0 || needsImprovementCount > 0) {
        parts.push(`${poorCount} poor and ${needsImprovementCount} needs-improvement Core Web Vitals metrics`);
      }
    }
 ...
 */
export function createMockMemoryMetrics(
