...
export class RootCauseAnalyzer {
  ...
  analyze(profile: PerformanceProfile): RootCauseAnalysis {
    ...
    // Categorize issues
    const criticalIssues = indicators.filter(i => 'severity' in i && i.severity === 'critical')
  ...
  private detectMemoryLeak(profile: PerformanceProfile): MemoryLeakIndicator | null {
    ...
    if (isIncreasing && (highGrowthRate || highMemoryUsage)) {
      const severity = this.determineMemorySeverity(
        growthRate,
        profile.memoryUsed / profile.memoryLimit
      )
  ...
  private detectSlowQueries(profile: PerformanceProfile): SlowQueryIndicator | null {
    ...
    const severity = this.determineQuerySeverity(profile.slowRequests, profile.averageResponseTime)

    const estimatedQps = profile.requestCount > 0 ? 1000 / profile.averageResponseTime : 0
  ...
  private detectCacheIssues(profile: PerformanceProfile): CacheHitRateIndicator | null {
    ...
    if (estimatedHitRate < 0.8) {
      const severity =
        estimatedHitRate < 0.5 ? 'critical' : estimatedHitRate < 0.7 ? 'high' : 'medium'
  ...
  private generateDiagnosis(
    ...
    // Find primary issue
    const criticalIndicators = indicators.filter(i => 'severity' in i && i.severity === 'critical')
    ...
    const contributingFactors = indicators.slice(1, 4).map(i => this.formatIndicatorTitle(i))
      ...
      if ('recommendations' in indicator && Array.isArray(indicator.recommendations)) {
        ...
        indicator.recommendations.slice(0, 3).forEach((rec, index) => {
          actions.push({
            ...
            category:
              priority === 'p0' ? 'immediate' : priority === 'p1' ? 'short-term' : 'long-term',
          })
        })
      }
    }
  ...
  private determineMemorySeverity(
    growthRate: number,
    usageRatio: number
  ...
  private determineQuerySeverity(
    slowCount: number,
    avgTime: number
  ): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private generateSummary(
    ...
    const highCount = indicators.filter(i => 'severity' in i && i.severity === 'high').length
