...
describe('data-converter', () => {
  ...
  describe('cleanArray', () => {
    ...
    it('should filter empty values when filterEmpty is true', () => {
      expect(cleanArray([1, null, 2, undefined, '', 3], { filterEmpty: true })).toEqual([1, 2, 3])
    })

    it('should remove duplicates when unique is true', () => {
      expect(cleanArray([1, 2, 2, 3, 1], { unique: true })).toEqual([1, 2, 3])
    })

    it('should combine unique and filterEmpty', () => {
      expect(cleanArray([1, 2, 2, null, 3, '', 1], { unique: true, filterEmpty: true })).toEqual([
        1, 2, 3,
      ])
    })
  })
