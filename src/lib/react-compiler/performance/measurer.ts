...
export class PerformanceMeasurer {
  ...
  comparePerformance(
    ...
    const improvement = {
      ...
      avgRenderTime: this.calculateImprovement(
        withoutCompiler.avgRenderTime,
        withCompiler.avgRenderTime
      ),
      totalRenderTime: this.calculateImprovement(
        withoutCompiler.totalRenderTime,
        withCompiler.totalRenderTime
      ),
      memoryUsage: this.calculateImprovement(withoutCompiler.memoryUsage, withCompiler.memoryUsage),
      firstRenderTime: this.calculateImprovement(
        withoutCompiler.firstRenderTime,
        withCompiler.firstRenderTime
      ),
      updateRenderTime: this.calculateImprovement(
        withoutCompiler.updateRenderTime,
        withCompiler.updateRenderTime
      ),
    }
...
export function usePerformanceTracker(componentName: string) {
  ...
  const getMetrics = (): PerformanceMetrics | null => {
    ...
    return {
      ...
      updateRenderTime:
        renderTimes.slice(1).reduce((a, b) => a + b, 0) / Math.max(renderTimes.length - 1, 1),
    }
  }
