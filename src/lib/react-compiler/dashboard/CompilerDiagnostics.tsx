...
function ProgressBar({
  ...
  return (
    <div className="w-full">
      ...
      {showLabel && (
        <div className="mt-1 text-right text-xs text-gray-600">{percentage.toFixed(1)}%</div>
      )}
    </div>
  )
}
...
function IssueBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-xs font-medium ${colorClasses[severity]}`}
    >
...
function ComponentCard({
  ...
  return (
    <div
      className={`cursor-pointer rounded-lg border p-3 transition-all hover:shadow-md ${severityColor} ${isSelected ? 'ring-2 ring-blue-500' : ''}`}
      onClick={onClick}
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          ...
          {report.issues.filter(i => i.severity === 'info').length > 0 && (
            <IssueBadge
              severity="info"
              count={report.issues.filter(i => i.severity === 'info').length}
            />
          )}
        </div>
      </div>
      <div className="mt-1 truncate text-xs text-gray-500">{report.path}</div>
    </div>
  )
}
...
function IssueDetail({ issue }: { issue: ComponentIssue }) {
  ...
  return (
    <div className={`rounded border-l-4 p-3 ${severityColors[issue.severity]}`}>
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="mb-1 flex justify-between text-sm">
            ...
            <span
              className={metrics.improvement.renderCount > 0 ? 'text-green-600' : 'text-red-600'}
            >
        ...
        <div>
          <div className="mb-1 flex justify-between text-sm">
            ...
            <span
              className={metrics.improvement.avgRenderTime > 0 ? 'text-green-600' : 'text-red-600'}
            >
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="rounded bg-gray-100 p-2">
              ...
              {metrics.beforeCompiler.avgRenderTime.toFixed(2)}ms
            </div>
            <div className="rounded bg-green-100 p-2">
              <span className="text-gray-500">After:</span>{' '}
              {metrics.afterCompiler.avgRenderTime.toFixed(2)}ms
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="mb-1 flex justify-between text-sm">
            ...
            <span
              className={metrics.improvement.memoryUsage > 0 ? 'text-green-600' : 'text-red-600'}
            >
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="rounded bg-gray-100 p-2">
              <span className="text-gray-500">Before:</span>{' '}
              {(metrics.beforeCompiler.memoryUsage / 1024).toFixed(2)}KB
            </div>
            <div className="rounded bg-green-100 p-2">
              <span className="text-gray-500">After:</span>{' '}
              {(metrics.afterCompiler.memoryUsage / 1024).toFixed(2)}KB
            </div>
          </div>
        </div>
      </div>
...
export function CompilerDiagnostics() {
  ...
  const stats = {
    ...
    errors: components.reduce(
      (sum, c) => sum + c.issues.filter(i => i.severity === 'error').length,
      0
    ),
    warnings: components.reduce(
      (sum, c) => sum + c.issues.filter(i => i.severity === 'warning').length,
      0
    ),
    info: components.reduce(
      (sum, c) => sum + c.issues.filter(i => i.severity === 'info').length,
      0
    ),
  }
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <StatsCard title="Total Components" value={stats.total} color="blue" icon="📦" />
          <StatsCard
            title="Compatible"
        ...
        <div className="mb-6 rounded-lg border bg-white p-4">
          ...
          <ProgressBar
            ...
            max={stats.total}
            color={
              stats.compatible / stats.total > 0.8
                ? 'green'
                : stats.compatible / stats.total > 0.5
                  ? 'yellow'
                  : 'red'
            }
          />
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-lg bg-gray-50 p-3">
                    ...
                    <div className="mt-1 text-sm">
                      ...
                      <span
                        className={
                          selectedComponent.isCompatible ? 'text-green-600' : 'text-red-600'
                        }
                      >
                  ...
                  {selectedComponent.issues.length > 0 ? (
                    <div>
                      <h4 className="mb-2 text-sm font-medium">
                        Issues ({selectedComponent.issues.length})
                      </h4>
