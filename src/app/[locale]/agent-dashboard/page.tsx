...
export default function AgentDashboardPage() {
  ...
  const derivedStats = useMemo(() => {
    ...
    const avgResponseTime = agents.length > 0
      ? Math.round(agents.reduce((sum, a) => sum + a.capabilities.avgResponseTime, 0) / agents.length)
      : 0;

    // Calculate team efficiency
    const teamEfficiency = agents.length > 0
      ? Math.round(agents.reduce((sum, a) => sum + a.capabilities.successRate, 0) / agents.length * 100)
      : 0;
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-zinc-900' : 'bg-zinc-50'} p-4 md:p-6`}>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Loading State */}
        {isLoading && (
          <div className={`p-8 text-center rounded-lg border ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-zinc-200'}`}>
        ...
        {!isLoading && (
          <>
            ...
            <div className={`p-4 rounded-lg border ${isDark ? 'bg-zinc-800/50 border-zinc-700/50' : 'bg-white/50 border-zinc-200/50'}`}>
              <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
                ...
                <div className={isDark ? 'text-zinc-500' : 'text-zinc-500'}>
                  总任务: {tasks.length} | 待处理: {tasks.filter(t => t.status === 'pending').length} | 完成: {stats.completedTasks}
                </div>
              </div>
            </div>
          </>
