 ...
 */
function validateAndParseFormData(formData: FormData): { success: true; data: ImageProcessingFormData } | { success: false; error: string } {
...
function logImageProcessingError(
  ...
  logger.error(`Image processing error at ${stage}`, error instanceof Error ? error : new Error(String(error)), context);
}
  ...
  try {
    ...
    if (!validation_result.valid) {
      ...
      if (validation_result.error?.includes('format')) {
        return createValidationError(
          `Invalid image format: ${validation_result.error}`,
    ...
    if (compress) {
      try {
        ...
        return createBadRequestError('Failed to compress image - try without compression or lower quality');
      }
    }
    ...
    if (!result.success) {
      ...
      if (errorLower.includes('format') || errorLower.includes('unsupported')) {
        return createValidationError(
          `Unsupported image format: ${result.error}`,
          { filename: image.name, fileType: image.type }
        );
      }
      ...
      if (errorLower.includes('size') || errorLower.includes('too large')) {
        return createValidationError(
          `Image size issue: ${result.error}`,
          { filename: image.name, fileSize: buffer.length }
        );
      }
...
export async function GET() {
  try {
    ...
    return createErrorResponse(error instanceof Error ? error : new Error('Failed to list image processing providers'));
  }
}
