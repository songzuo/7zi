...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!taskId || !status || !state) {
      return NextResponse.json({ error: 'taskId, status, and state are required' }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: 'Failed to broadcast task status update' }, { status: 500 })
  }
}
