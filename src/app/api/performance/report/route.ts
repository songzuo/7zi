...
function generateReport(startTime: number, endTime: number, routes?: string[]): Report {
  ...
  const criticalAlerts = Object.values(metrics).reduce((sum, m) => sum + m.stats.poor, 0)
...
function calculateStats(metrics: PerformanceMetric[]): MetricStats {
  ...
  const ratingCounts = metrics.reduce(
    (acc, m) => {
      acc[m.rating]++
      return acc
    },
    ...
    if (poorPercentage > 20) {
      ...
      switch (m.name) {
        ...
        case 'CLS':
          ...
          recommendation =
            'Reserve space for images/ads, avoid inserting content above existing content'
          break
        case 'INP':
