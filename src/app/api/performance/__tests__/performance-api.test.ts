...
} from '@/app/api/performance/metrics/route'
import {
  GET as getAlerts,
  POST as postAlerts,
  PUT as putAlerts,
  DELETE as deleteAlerts,
} from '@/app/api/performance/alerts/route'
...
function createMockRequest(method: string, url: string, body?: MockRequestBody): NextRequest {
  return {
    method,
    url,
    json: async () => body,
...
describe('Performance Metrics API', () => {
  beforeEach(async () => {
    ...
    const req = createMockRequest(
      'DELETE',
      'http://localhost/api/performance/metrics?before=9999999999999'
    )
  ...
  describe('DELETE /api/performance/metrics', () => {
    it('should delete old metrics by timestamp', async () => {
      ...
      const req = createMockRequest(
        'DELETE',
        `http://localhost/api/performance/metrics?before=${oldTimestamp}`
      )
...
describe('Performance Alerts API', () => {
  ...
  describe('DELETE /api/performance/alerts', () => {
    it('should clear acknowledged alerts', async () => {
      const req = createMockRequest(
        'DELETE',
        'http://localhost/api/performance/alerts?clearAcknowledged=true'
      )
  ...
  describe('GET /api/performance/report', () => {
    ...
    it('should generate report for different time periods', async () => {
      ...
      for (const period of periods) {
        const req = createMockRequest(
          'GET',
          `http://localhost/api/performance/report?period=${period}`
        )
        const res = await getReport(req)
        const data = await res.json()
