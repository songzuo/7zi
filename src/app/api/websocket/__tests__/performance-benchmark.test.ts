...
describe('WebSocket Performance Benchmark Tests', () => {
  ...
  describe('Memory Usage Under Load', () => {
    ...
    it('should implement message TTL cleanup', () => {
      ...
      const expiredMessages = oldMessages.filter(m => Date.now() - m.createdAt > ttl)
    ...
    it('should monitor memory growth over time', () => {
      ...
      const growthRate =
        (memorySnapshots[3].memoryMB - memorySnapshots[0].memoryMB) /
    ...
    it('should implement memory pressure handling', () => {
      ...
      memoryPressure.pressureLevel =
        pressureRatio > 0.8 ? 'high' : pressureRatio > 0.5 ? 'medium' : 'low'
    ...
    it('should clean up stale connections', () => {
      ...
      const staleConnections = connections.filter(c => Date.now() - c.lastActivity > staleThreshold)
  ...
  describe('Stress Tests', () => {
    ...
    it('should maintain performance under sustained load', () => {
      ...
      const degradation =
        ((loadTest.expectedThroughput - loadTest.actualThroughput) / loadTest.expectedThroughput) *
        100
