...
function validateProgress(progress: unknown): asserts progress is number {
  ...
  if (typeof progress !== 'number') {
    throw createAppError('progress must be a number', ErrorCodes.VALIDATION_ERROR, 400);
  }

  if (progress < 0 || progress > 100) {
    throw createAppError('progress must be between 0 and 100', ErrorCodes.VALIDATION_ERROR, 400);
  }

  if (!Number.isInteger(progress)) {
    throw createAppError('progress must be an integer', ErrorCodes.VALIDATION_ERROR, 400);
  }
}
...
function validateDate(date: unknown): asserts date is string | null {
  ...
  if (typeof date !== 'string') {
    throw createAppError('Date must be a string', ErrorCodes.VALIDATION_ERROR);
  }

  const parsed = new Date(date);
  if (isNaN(parsed.getTime())) {
    throw createAppError('Invalid date format', ErrorCodes.VALIDATION_ERROR);
  }
}
...
export function validateCreateProjectRequest(data: unknown): CreateProjectRequest {
  if (!data || typeof data !== 'object') {
    throw createAppError('Request body must be an object', ErrorCodes.VALIDATION_ERROR);
  }
...
export function validateListProjectsQuery(params: URLSearchParams): ListProjectsQuery {
  ...
  if (search) {
    if (search.length > 100) {
      throw createAppError('Search term cannot exceed 100 characters', ErrorCodes.VALIDATION_ERROR);
    }
    query.search = search;
  }
...
export function validateProjectId(id: string): number {
  ...
  if (isNaN(numId)) {
    throw createAppError('Invalid project ID', ErrorCodes.VALIDATION_ERROR);
  }

  if (numId <= 0) {
    throw createAppError('Project ID must be a positive integer', ErrorCodes.VALIDATION_ERROR);
  }
