...
export async function GET(request: NextRequest) {
  try {
    ...
    const { owner, repo, per_page, page, state, labels, sort, direction, since } = validation.data
    ...
    if (!response.ok) {
      switch (response.status) {
        case 404:
          return createNotFoundError(`Repository ${owner}/${repo} not found or does not exist`, {
            owner,
            repo,
          })
        ...
        default:
          ...
          return createErrorResponse(new Error(`GitHub API error: ${errorText}`), response.status)
      }
    }
    ...
    if (!Array.isArray(data)) {
      ...
      return createErrorResponse(new Error('Invalid response format'), 502, {
