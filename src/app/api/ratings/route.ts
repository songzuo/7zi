  ...
  try {
    ...
    if (existingRating) {
      ...
      const response = createSuccessResponse({
        ...updatedRating,
        metadata: updatedRating.metadata ? JSON.parse(updatedRating.metadata as unknown as string) : undefined,
    ...
    // Get created rating
    const newRating = db.queryRows('SELECT * FROM ratings WHERE id = ?', [ratingId])[0] as unknown as Rating;

    const response = createSuccessResponse({
      ...newRating,
      metadata: newRating.metadata ? JSON.parse(newRating.metadata as unknown as string) : undefined,
 ...
 * Delete rating
 */
export async function DELETE_RATING(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  const startTime = Date.now();
 ...
 */
export async function POST_HELPFUL(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  const startTime = Date.now();
  ...
  try {
    ...
    if (existingVote) {
      ...
      if (existingVote.is_helpful !== is_helpful) {
        ...
        db.exec(
          'UPDATE helpful_votes SET is_helpful = ? WHERE rating_id = ? AND user_id = ?',
          [is_helpful ? 1 : 0, id, userId]
        );
      }
    } else {
      // Create new vote
    ...
    // Get updated rating
    const updatedRating = db.queryRows('SELECT * FROM ratings WHERE id = ?', [id])[0] as unknown as Rating;
