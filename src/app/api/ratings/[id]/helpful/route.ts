...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rating = db.queryRows('SELECT * FROM ratings WHERE id = ?', [id])[0] as
      | { id: string; helpful_count: number; not_helpful_count: number }
      | undefined
    ...
    if (helpful) {
      db.query('UPDATE ratings SET helpful_count = helpful_count + 1 WHERE id = ?', [id])
    } else {
      db.query('UPDATE ratings SET not_helpful_count = not_helpful_count + 1 WHERE id = ?', [id])
    }

    // Get updated rating
    const updatedRating = db.queryRows('SELECT * FROM ratings WHERE id = ?', [id])[0]
