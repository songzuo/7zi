...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { type, rating, title, description, email, images, metadata: feedbackMetadata } =
...
export async function PATCH(
  ...
  try {
    ...
    db.exec(
      `UPDATE feedbacks SET ${setStatements.join(', ')} WHERE id = ?`,
      updateParams
    );

    // Get updated feedback
    const feedback = db.queryRows('SELECT * FROM feedbacks WHERE id = ?', [id])[0] as unknown as Feedback;
