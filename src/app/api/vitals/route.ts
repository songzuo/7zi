...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error(
      '[Web Vitals API] Error processing request:',
...
export async function GET(request: NextRequest) {
  ...
  if (route) {
    filtered = filtered.filter(entry => entry.metrics.some(m => m.route === route))
  }

  if (name) {
    filtered = filtered.filter(entry => entry.metrics.some(m => m.name === name))
  }
