...
describe('GET /api/health', () => {
  ...
  describe('error handling', () => {
    it('should return 503 when cache fails', async () => {
      ...
      mockCacheManager.getOrSet = vi.fn().mockRejectedValue(new Error('Cache connection failed'))
      ...
      expect(createErrorResponse).toHaveBeenCalledWith(expect.any(Error), 503)
    })

    it('should log error when health check fails', async () => {
      const mockCacheManager = getCacheManager()

      mockCacheManager.getOrSet = vi.fn().mockRejectedValue(new Error('Health check failed'))
      ;(createErrorResponse as ReturnType<typeof vi.fn>).mockReturnValue(
        NextResponse.json(
          {
            success: false,
