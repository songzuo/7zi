...
describe('Server Actions 缓存 API - cacheLife profile', () => {
  ...
  describe('cacheLife profile 验证', () => {
    it('posts 应该使用 max profile（最大缓存）', async () => {
      ...
      const postsCalls = (revalidateTag as ReturnType<typeof vi.fn>).mock.calls.filter(
    ...
    it('projects 应该使用 max profile（最大缓存）', async () => {
      ...
      const projectsCalls = (revalidateTag as ReturnType<typeof vi.fn>).mock.calls.filter(
        (call: unknown[]) => call[0] === 'projects'
      )
