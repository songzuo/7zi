#!/usr/bin/env node
/**
 * Polymarket 自我进化修复 - 使用 hybrid 模式测试个人端点
 */
...
async function testGetBalanceAllowance() {
  ...
  try {
    ...
    console.log('getBalanceAllowance 失败:', e.message, e.stack);
    return { success: false, error: e.message };
  }
}

async function testUpdateBalanceAllowance() {
  console.log('\n' + '='.repeat(70));
  console.log('测试：updateBalanceAllowance({ amount: \'99.58\' })');
  console.log('='.repeat(70));

  try {
    const result = await client.updateBalanceAllowance({ amount: '99.58' });
    console.log('updateBalanceAllowance 成功:', JSON.stringify(result, null, 2));
    return { success: true, result };
  } catch (e) {
    console.log('updateBalanceAllowance 失败:', e.message, e.stack);
    return { success: false, error: e.message };
  }
}

async function main() {
  console.log('🧬 Polymarket 自我进化修复 - Hybrid 模式测试\n');
  ...
  // 测试 update allowance（如果前面失败）
  if (!results.getBalanceAllowance.success) {
    console.log('\n注意：getBalanceAllowance 失败，尝试 updateBalanceAllowance...');
    results.updateBalanceAllowance = await testUpdateBalanceAllowance();
  }
  ...
  if (results.updateBalanceAllowance) {
    console.log('\nupdateBalanceAllowance()');
    if (results.updateBalanceAllowance.success) {
      console.log('结果: 成功');
      console.log('返回值:', JSON.stringify(results.updateBalanceAllowance.result, null, 2));
    } else {
      console.log('结果: 失败');
      console.log('错误:', results.updateBalanceAllowance.error);
    }
  }
  ...
  const balanceEndpointRecovered = results.getBalanceAllowance.success || 
                                   (results.updateBalanceAllowance && results.updateBalanceAllowance.success);
