...
function fixFile(filePath) {
  ...
  asyncFunctions.forEach(funcName => {
    ...
    const regex = new RegExp(`(const\\s+\\w+\\s*=\\s+)(${funcName}\\()`, 'g')
    ...
    if (countBefore > countAfter) {
      modified = true
      console.log(
        `  ✅ Fixed ${funcName} in ${path.basename(filePath)} (${countBefore - countAfter} occurrences)`
      )
    }
  })
  ...
  asyncFunctions.forEach(funcName => {
    ...
    const regex = new RegExp(`(return\\s+)(${funcName}\\()`, 'g')
...
function findApiFiles(dir) {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      ...
      if (!['node_modules', '__tests__', '.next', '.git', 'coverage'].includes(item.name)) {
