...
const UNUSED_IMPORT_PATTERNS = [
  ...
  { regex: /import\s*\{\s*fireEvent\s*\}\s*from\s*['"]@testing-library\/react['"]/, name: 'fireEvent' },
  { regex: /import\s*\{\s*userEvent\s*\}\s*from\s*['"]@testing-library\/user-event['"]/, name: 'userEvent' },
    ...
    for (const pattern of UNUSED_IMPORT_PATTERNS) {
      ...
      if (match) {
        ...
        if (usages.length === 0) {
          ...
          modified = true;
          console.log(`    - Removed unused '${pattern.name}' from ${path.relative(PROJECT_DIR, filePath)}`);
        }
      }
    }
