...
function auditRoute(filePath: string): RouteAudit {
  ...
  if (
    ROUTE_METHODS.some(m => content.includes(`${m}(async`)) &&
...
function main() {
  ...
  console.log(
    `  Routes with error handling: ${audits.length - withNoErrorHandling} (${Math.round(((audits.length - withNoErrorHandling) / audits.length) * 100)}%)`
  )
  console.log(
    `  Routes with validation: ${audits.length - withNoValidation} (${Math.round(((audits.length - withNoValidation) / audits.length) * 100)}%)`
  )
  ...
  audits.forEach(audit => {
    if (audit.issues.length > 0) {
      console.log(`\n${audit.filePath}`)
      console.log(
        `  Error Handling: ${audit.hasErrorHandling ? '✅' : '❌'} (${audit.errorHandlingMethods.join(', ') || 'none'})`
      )
      console.log(
        `  Validation: ${audit.hasAnyValidation ? '✅' : '❌'} (${audit.validationLibraries.join(', ') || 'none'})`
      )
  ...
  const withSecretsOnly = audits.filter(
    a => a.hasHardcodedSecrets && a.issues.filter(i => i.includes('hardcoded')).length > 0
  )
