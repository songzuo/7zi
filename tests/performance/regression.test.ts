...
describe('Performance Score Tests', () => {
  ...
  it('should calculate performance score correctly', () => {
    ...
    const calculateScore = (
      metrics: { name: string; rating: 'good' | 'needs-improvement' | 'poor' }[]
    ) => {
      let score = 100
...
describe('Comprehensive Regression Tests', () => {
  ...
  it('should detect performance regression scenario', () => {
    ...
    // 验证退化被正确检测
    const poorCount = degradedMetrics.filter(
      m => getMetricRating(m.name, m.value) === 'poor'
    ).length
  ...
  it('should detect partial regression scenario', () => {
    ...
    // 验证部分退化被正确检测
    const needsImprovementCount = partialMetrics.filter(
      m => getMetricRating(m.name, m.value) === 'needs-improvement'
    ).length
