...
function generateTasks(count: number, startId: number = 0): Task[] {
  const taskTypes = ['architecture', 'research', 'implementation', 'testing', 'devops', 'design', 'marketing', 'sales', 'finance', 'media', 'general'] as const;
...
describe('AgentScheduler Performance', () => {
  ...
  describe('Matching Algorithm Performance', () => {
    ...
    it('should find best candidate within 15ms', () => {
      ...
      if (result) {
        console.log(`  - Best: ${result.agentName} (${(result.confidence * 100).toFixed(1)}% confidence)`);
      }
  ...
  describe('Scheduling Decision Performance', () => {
    ...
    it('should handle full scheduling cycle efficiently', async () => {
      ...
      const highTasks = generateTasks(20).map(t => ({ ...t, priority: 'high' as TaskPriority, id: `high-${t.id}` }));
      const normalTasks = generateTasks(70).map(t => ({ ...t, priority: 'medium' as TaskPriority, id: `medium-${t.id}` }));
...
describe('Performance Benchmark Suite', () => {
  it('should generate benchmark report', () => {
    ...
    results.push({ name: 'Find best candidate', target: THRESHOLDS.findCandidates, actual: matchTime });
    
    // Test scheduling
    const scheduleStart = performance.now();
    testScheduler.scheduleNextBatch();
    const scheduleTime = performance.now() - scheduleStart;
    results.push({ name: 'Schedule batch', target: THRESHOLDS.fullScheduleCycle, actual: scheduleTime });
