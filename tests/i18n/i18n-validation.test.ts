    ...
    it('中文占位符应与其他语言一致', () => {
      ...
      for (const lang of LANGUAGES) {
        ...
        for (const [key, zhPhs] of Object.entries(zhPlaceholders)) {
          ...
          expect(
            missing,
            `${lang.toUpperCase()} [${key}] 缺少占位符: ${missing.join(', ')}`
          ).toHaveLength(0)
          expect(
            extra,
            `${lang.toUpperCase()} [${key}] 多余占位符: ${extra.join(', ')}`
          ).toHaveLength(0)
        }
      }
    })
    ...
    it('关键命名空间应该存在于所有语言', () => {
      for (const lang of LANGUAGES) {
        for (const ns of criticalNamespaces) {
          ...
          if (zhHasNamespace) {
            expect(messages[lang], `${lang.toUpperCase()} 应该包含 ${ns} 命名空间`).toHaveProperty(
              ns
            )
          }
        }
