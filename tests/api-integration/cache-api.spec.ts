...
beforeEach(() => {
  ...
  vi.mocked(unstable_cache).mockImplementation(
    (fn: (...args: unknown[]) => unknown, tags: string[]) => {
...
async function simulateDataFetch(
  userId: string
): Promise<{ id: string; name: string; timestamp: number }> {
  return {
    id: userId,
    name: `User ${userId}`,
    timestamp: Date.now(),
...
async function simulateExpensiveQuery(
  query: string
): Promise<{ results: number[]; duration: number }> {
  // Simulate delay
...
describe('updateTag() - Cache Tag Invalidation', () => {
  ...
  it('should handle tag-based invalidation for multiple entries', async () => {
    ...
    for (const entry of entries) {
      const cachedFn = unstable_cache(async (id: string) => ({ id, data: `data-${id}` }), [tag])
...
describe('Cache Invalidation Behavior', () => {
  ...
  it('should maintain cache isolation between different tags', async () => {
    ...
    const cacheFns = tags.map(tag => unstable_cache(async (id: string) => ({ id, tag }), [tag]))
  ...
  it('should handle invalidation during concurrent requests', async () => {
    ...
    const promises = [cachedFn('123'), cachedFn('123'), cachedFn('123')]
...
describe('Integration with Server Actions', () => {
  ...
  it('should handle error scenarios gracefully', async () => {
    ...
    const errorFn = unstable_cache(async () => {
...
describe('Edge Cases and Error Handling', () => {
  ...
  it('should work with undefined and null values', async () => {
    const cachedFn = unstable_cache(async (val: unknown) => ({ value: val }), ['test-tag'])
