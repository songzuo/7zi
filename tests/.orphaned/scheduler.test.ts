...
describe("AgentScheduler", () => {
  ...
  describe("scheduleNextBatch", () => {
    it("should schedule multiple tasks", async () => {
      ...
      expect(
        result.stats.totalScheduled + result.stats.totalFailed,
      ).toBeGreaterThan(0);
    });
  ...
  describe("manualAssign", () => {
    ...
    it("should throw when manual override not allowed", () => {
      ...
      expect(() =>
        restrictedScheduler.manualAssign("task1", "executor", "user1"),
      ).toThrow();
    });

    it("should throw for non-existent task", () => {
      expect(() =>
        scheduler.manualAssign("nonexistent", "executor", "user1"),
      ).toThrow();
    });
    ...
    it("should throw for non-existent agent", () => {
      ...
      scheduler.addTask(task);
      expect(() =>
        scheduler.manualAssign("task1", "nonexistent", "user1"),
      ).toThrow();
    });
    ...
    it("should throw when agent at capacity", () => {
      ...
      scheduler.addTask(task);
      expect(() =>
        scheduler.manualAssign("task1", "executor", "user1"),
      ).toThrow();
    });
  });
  ...
  describe("reassignTask", () => {
    it("should reassign a failed task", async () => {
      ...
      expect(
        newDecision === null || newDecision.assignedAgent !== undefined,
      ).toBe(true);
    });
