...
describe("Agent Availability Integration Tests", () => {
  ...
  describe("Task Reassignment After Agent Offline", () => {
    it("should fail task when assigned agent goes offline before completion", async () => {
      ...
      expect(scheduler.getAgent(decision!.assignedAgent)?.availability).toBe(
        false,
      );
    });
    ...
    it("should handle multiple agents going offline", async () => {
      ...
      if (decision) {
        ...
        expect([implAgents[0].agentId, implAgents[1].agentId]).not.toContain(
          decision.assignedAgent,
        );
      }
    });
  ...
  describe("Multi-Agent Collaboration", () => {
    ...
    it("should handle agent specialization in scheduling", async () => {
      ...
      // Get the decisions
      const researchDecision = result.scheduled.find(
        (d) => d.taskId === researchTask.id,
      );
      const testingDecision = result.scheduled.find(
        (d) => d.taskId === testingTask.id,
      );

      // Check that appropriate agents were selected
      if (researchDecision) {
        const researchAgent = scheduler.getAgent(
          researchDecision.assignedAgent,
        );
