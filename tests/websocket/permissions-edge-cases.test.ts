...
describe('PermissionManager - Edge Cases & Boundary Tests', () => {
  ...
  describe('Permission Granting - Edge Cases', () => {
    it('should grant permission to non-existent user', () => {
      ...
      permissionManager.grantPermission(nonExistentUser, roomId, 'room:manage', ownerUserId)

      const hasPermission = permissionManager.hasPermission(nonExistentUser, roomId, 'room:manage')
    ...
    it('should grant permission with past expiration date', () => {
      ...
      permissionManager.grantPermission(userId, roomId, 'admin:manage_users', ownerUserId, pastDate)

      const hasPermission = permissionManager.hasPermission(userId, roomId, 'admin:manage_users')
    ...
    it('should grant permission with expiration exactly at current time', () => {
      ...
      permissionManager.grantPermission(userId, roomId, 'admin:manage_users', ownerUserId, now)
  ...
  describe('Permission Revocation - Edge Cases', () => {
    it('should revoke permission from non-existent user', () => {
      ...
      permissionManager.revokePermission(nonExistentUser, roomId, 'room:manage', ownerUserId)
  ...
  describe('Hierarchy Management - Edge Cases', () => {
    ...
    it('should handle canManageUser for non-existent users', () => {
      const canManage1 = permissionManager.canManageUser(ownerUserId, 'non-existent-user', roomId)

      const canManage2 = permissionManager.canManageUser('non-existent-user', userId, roomId)
