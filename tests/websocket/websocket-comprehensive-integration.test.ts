...
class MockWebSocketServer {
  ...
  constructor(config?: { maxConnections?: number; maxRoomsPerUser?: number }) {
    if (config) {
  ...
  joinRoom(socketId: string, roomId: string): { success: boolean; error?: string } {
    ...
    if (
      ...
      !room.invites.has(socket.userId) &&
      room.ownerId !== socket.userId
    ) {
  ...
  sendToRoom(
    ...
    roomId: string,
    content: string
  ): { success: boolean; error?: string; message?: MockMessage } {
    const socket = this.sockets.get(socketId)
    if (!socket || !socket.connected) {
      return { success: false, error: 'Socket not connected' }
    }
...
describe('WebSocket Connection Management', () => {
  ...
  describe('Connection Limits', () => {
    it('should enforce maximum connections limit', async () => {
      ...
      await expect(limitedServer.connect('user-6', 'User 6', 'token')).rejects.toThrow(
...
describe('Error Handling', () => {
  ...
  describe('Connection Errors', () => {
    it('should handle connection limit exceeded', async () => {
      ...
      await expect(limitedServer.connect('user-2', 'Bob', 'token')).rejects.toThrow(
        'Maximum connections reached'
      )
      limitedServer.reset()
    })
