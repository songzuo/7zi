...
async function runTests() {
  ...
  try {
    ...
    logger.info(`  By Device: Mobile=${stats.byDevice.mobile}, Tablet=${stats.byDevice.tablet}, Desktop=${stats.byDevice.desktop}`);
    logger.info(`  Top Routes: ${Object.entries(stats.byRoute).slice(0, 3).map(([r, c]) => `${r} (${c})`).join(', ')}\n`);

    // Test 6: Get percentiles
    logger.info('Test 6: Getting LCP percentiles...');
    const lcpPercentiles = db.getPercentiles('LCP', { hours: 24 });
    logger.info(`✓ LCP Percentiles: p50=${lcpPercentiles.p50}ms, p75=${lcpPercentiles.p75}ms, p95=${lcpPercentiles.p95}ms\n`);
