...
describe('CreditScoreService', () => {
  ...
  describe('getCreditDiscountRate', () => {
    ...
    it('良好信用应该有中等折扣', async () => {
      ...
      const discountRate = await creditService.getCreditDiscountRate(testAgentId);
      expect(discountRate).toBe(0.10); // 10%
    });
  ...
  describe('getTopAgents', () => {
    it('应该返回按信用分数排序的 Agent 列表', async () => {
      ...
      await creditService.calculateCreditScore(agent1, {
        taskCompletionRate: 100, userRating: 100, violationCount: 0,
        disputeRate: 0, onTimeDeliveryRate: 100, repeatCustomerRate: 100, responseSpeed: 100,
      });

      await creditService.calculateCreditScore(agent2, {
        taskCompletionRate: 50, userRating: 50, violationCount: 0,
        disputeRate: 0, onTimeDeliveryRate: 50, repeatCustomerRate: 50, responseSpeed: 50,
      });

      await creditService.calculateCreditScore(agent3, {
        taskCompletionRate: 80, userRating: 80, violationCount: 0,
        disputeRate: 0, onTimeDeliveryRate: 80, repeatCustomerRate: 80, responseSpeed: 80,
      });
