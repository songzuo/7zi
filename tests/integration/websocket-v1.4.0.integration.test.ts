...
describe('WebSocket v1.4.0 Integration Tests', () => {
  ...
  describe('房间系统集成', () => {
    describe('房间创建流程', () => {
      ...
      it('应该支持不同类型的房间', () => {
        const types: Array<'task' | 'project' | 'chat' | 'document'> = ['task', 'project', 'chat', 'document'];
  ...
  describe('权限控制集成', () => {
    ...
    describe('权限检查', () => {
      it('应该根据角色正确检查权限', () => {
        ...
        expect(permissionManager.hasPermission('user-1', 'perm-test-room', 'room:manage')).toBe(true);
        expect(permissionManager.hasPermission('user-2', 'perm-test-room', 'room:manage')).toBe(false);
        expect(permissionManager.hasPermission('user-2', 'perm-test-room', 'message:send')).toBe(true);
      });
