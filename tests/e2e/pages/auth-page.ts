...
export class AuthPage {
  ...
  constructor(page: Page) {
    ...
    this.loginEmailInput = page.locator(
      'input[type="email"][name*="email"], input[placeholder*="邮箱"], input[placeholder*="Email"]'
    )
    this.loginPasswordInput = page.locator(
      'input[type="password"][name*="password"], input[placeholder*="密码"], input[placeholder*="Password"]'
    )
    this.loginButton = page.locator(
      'button[type="submit"]:has-text("登录"), button:has-text("Login"), button:has-text("Sign In")'
    )
    this.rememberMeCheckbox = page.locator('input[type="checkbox"][name*="remember"]')

    // Registration form
    this.registerNameInput = page.locator(
    ...
    this.registerButton = page.locator(
      'button[type="submit"]:has-text("注册"), button:has-text("Register"), button:has-text("Sign Up")'
    )

    // Navigation
    this.loginLink = page.locator('a:has-text("登录"), a:has-text("Login"), a:has-text("Sign In")')
    this.registerLink = page.locator(
      'a:has-text("注册"), a:has-text("Register"), a:has-text("Sign Up")'
    )
  ...
  async logout() {
    await this.page.click(
      'button:has-text("退出"), button:has-text("Logout"), [aria-label*="logout"]'
    )
  }
