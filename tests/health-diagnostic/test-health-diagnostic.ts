...
if (existsSync(envExamplePath)) {
  ...
  log('ENV', 'pass', '.env.example 定义了变量', {
    count: requiredVars.length,
    vars: requiredVars.slice(0, 10),
  })
}

// 检查关键环境变量
const criticalEnvVars = ['DATABASE_URL', 'NEXT_PUBLIC_APP_URL', 'NODE_ENV']
...
try {
  ...
  if (existsSync(healthRoutePath)) {
    ...
    // 检查实现细节
    const usesCacheManager =
      routeContent.includes('getCacheManager') || routeContent.includes('CacheManager')
    ...
    const checksDb =
      ...
      routeContent.includes('db') ||
      routeContent.includes('getDatabase')
...
try {
  ...
  const testExpectsWrapped =
    testContent.includes('data.data.status') || testContent.includes('data.checks')
  ...
  // 检查缓存使用差异
  const routeUsesCache =
    routeContent.includes('getCacheManager') || routeContent.includes('getOrSet')
