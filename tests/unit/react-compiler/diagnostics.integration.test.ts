...
describe('ReactCompilerDiagnostics Integration Tests', () => {
  ...
  describe('批量扫描功能', () => {
    ...
    it('应该正确计算兼容性百分比', async () => {
      ...
      const incompatibleCode = 'function Bad() { const ref = { current: null }; ref.current = value; return <div />; }';
  ...
  describe('便捷函数测试', () => {
    it('quickScan 应该扫描项目', async () => {
      const code = 'function Test() { const ref = { current: null }; ref.current = value; return <div />; }';
      await fs.writeFile(path.join(testDir, 'src', 'Test.tsx'), code, 'utf-8');
