...
describe('Database Module (Direct)', () => {
  ...
  describe('data types', () => {
    it('should handle INTEGER types', () => {
      ...
      const result = db.prepare(`SELECT * FROM ${tableName}`).all() as { id: number; int_val: number }[];
    ...
    it('should handle TEXT types', () => {
      ...
      const result = db.prepare(`SELECT * FROM ${tableName}`).all() as { id: number; text_val: string }[];
      expect(result).toHaveLength(1);
    ...
    it('should handle REAL/float types', () => {
      ...
      const result = db.prepare(`SELECT * FROM ${tableName}`).all() as { id: number; real_val: number }[];
      expect(result).toHaveLength(1);
    ...
    it('should handle BLOB types', () => {
      ...
      const result = db.prepare(`SELECT * FROM ${tableName}`).all() as { id: number; blob_val: Buffer }[];
      expect(result).toHaveLength(1);
