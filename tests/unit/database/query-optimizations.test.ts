...
} from '../src/lib/db/query-optimizations'
...
describe('getOptimizedFeedbackStats', () => {
  ...
  it('should use single optimized query', async () => {
    ...
    expect((mockDb.queryRows as ReturnType<typeof vi.fn>).mock.calls[0][0]).toContain(
      'COUNT(*) as total'
    )
    expect((mockDb.queryRows as ReturnType<typeof vi.fn>).mock.calls[0][0]).toContain(
...
describe('batchLoad', () => {
  ...
  it('should load items in single batch', async () => {
    ...
    expect(mockDb.queryRows).toHaveBeenCalledWith(expect.stringContaining('WHERE id IN'), [
      '1',
      '2',
      '3',
    ])
  ...
  it('should split into batches for large id lists', async () => {
    ...
    const batch1 = ids.slice(0, 100).map(id => ({ id, name: `Item ${id}` }))
    const batch2 = ids.slice(100, 200).map(id => ({ id, name: `Item ${id}` }))
    const batch3 = ids.slice(200, 250).map(id => ({ id, name: `Item ${id}` }))
    ;(mockDb.queryRows as ReturnType<typeof vi.fn>)
  ...
  it('should use custom id column', async () => {
    ...
    expect(mockDb.queryRows).toHaveBeenCalledWith(expect.stringContaining('WHERE user_id IN'), [
      '1',
      '2',
    ])
  })
...
describe('paginate', () => {
  ...
  it('should include WHERE clause and params', async () => {
    ...
    expect(mockDb.queryRows).toHaveBeenCalledWith(expect.stringContaining('WHERE status = ?'), [
      'active',
      10,
      0,
    ])
...
describe('getFeedbacksWithAttachments', () => {
  ...
  it('should use single query for all attachments', async () => {
    ...
    expect(mockDb.queryRows).toHaveBeenCalledTimes(1)
    expect(mockDb.queryRows).toHaveBeenCalledWith(expect.stringContaining('WHERE feedback_id IN'), [
      'fb1',
      'fb2',
      'fb3',
    ])
...
describe('getRatingWithVotes', () => {
  ...
  it('should use single query for all votes', async () => {
    ...
    expect(mockDb.queryRows).toHaveBeenCalledTimes(1)
    expect(mockDb.queryRows).toHaveBeenCalledWith(expect.stringContaining('WHERE rating_id IN'), [
      'r1',
      'r2',
    ])
  })
...
describe('getFeedbackStatsByStatuses', () => {
  ...
  it('should use single query with IN clause', async () => {
    ...
    expect(mockDb.queryRows).toHaveBeenCalledWith(expect.stringContaining('WHERE status IN'))
  })
...
describe('getRatingStatsByValues', () => {
  ...
  it('should use single query with IN clause', async () => {
    ...
    expect(mockDb.queryRows).toHaveBeenCalledWith(expect.stringContaining('WHERE rating IN'))
  })

  it('should return Record<number, number> type', async () => {
    ;(mockDb.queryRows as ReturnType<typeof vi.fn>).mockReturnValue([{ rating: 5, count: 100 }])
