...
import { withRetry, RetryPresets, getRetryInfo, type RetryConfig } from '@/lib/api/retry-decorator'
  ...
  describe('withRetry - retryable error codes', () => {
    ...
    it('should retry on network error codes', async () => {
      ...
      for (const code of errorCodes) {
        const mockFn = vi.fn().mockRejectedValueOnce(new Error(code)).mockResolvedValue('success')
