...
async function runConcurrentTest(name, path, method, bodyFactory, concurrency, totalRequests) {
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    process.stdout.write(
      `\r   进度: ${progress}% (${stats.successes + stats.failures}/${totalRequests})`
    )
  }
...
async function runTests() {
  ...
  for (const scenario of scenarios) {
    ...
    console.log(`\n┌${'─'.repeat(68)}┐`)
    console.log(
      `│  场景: ${scenario.name.toUpperCase().padEnd(20)} 并发: ${scenario.concurrency.toString().padEnd(10)} 请求: ${scenario.requests}  │`
    )
    ...
    for (const r of scenarioResults) {
      ...
      console.log(
        `     延迟: P50=${r.latency.p50.toFixed(0)}ms P90=${r.latency.p90.toFixed(0)}ms P99=${r.latency.p99.toFixed(0)}ms`
      )
      console.log(
        `           Min=${r.latency.min.toFixed(0)}ms Max=${r.latency.max.toFixed(0)}ms Mean=${r.latency.mean.toFixed(0)}ms`
      )

      if (Object.keys(r.errors).length > 0 && r.errorRate > 0) {
        console.log(
          `     错误: ${Object.entries(r.errors)
            .map(([k, v]) => `${k}(${v})`)
            .join(', ')}`
        )
      }
  ...
  console.log(
    `    增长: ${((memoryAfter.heapUsed - memoryBefore.heapUsed) / 1024 / 1024).toFixed(2)} MB`
  )
  ...
  if (highLatency.length > 0) {
    ...
    for (const r of highLatency) {
      console.log(
        `    - ${r.endpoint}: P95=${r.latency.p95.toFixed(0)}ms, P99=${r.latency.p99.toFixed(0)}ms`
      )
    }
  }
